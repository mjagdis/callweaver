...
KOpenPBXConsole::KOpenPBXConsole() : KTMainWindow()
{
	QVBoxLayout *box;
	QFrame *f;
	...
	help = kapp->getHelpMenu(TRUE, "KDE OpenPBX Console\nby Mark Spencer");
	
	setCaption("OpenPBX Console");
...
void KOpenPBXConsole::slotExit()
{
	close();
}

void KOpenPBXConsole::closeEvent(QCloseEvent *)
{
	kapp->quit();
}
