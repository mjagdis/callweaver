...
int cw_park_call(struct cw_channel *chan, struct cw_channel *peer, int timeout, int *extout)
{
	...
	if (!(i < parking_range)) {
		cw_log(CW_LOG_WARNING, "No more parking spaces\n");
		free(pu);
	...
	cw_manager_event(CW_EVENT_FLAG_CALL, "ParkedCall",
		...
		cw_msg_tuple("From",         "%s",  peer->name),
		cw_msg_tuple("Timeout",      "%ld", (long) pu->start.tv_sec + (long) (pu->parkingtime/1000) - (long) time(NULL)),
		cw_msg_tuple("CallerID",     "%s",  (pu->chan->cid.cid_num ? pu->chan->cid.cid_num : "<unknown>")),
		cw_msg_tuple("CallerIDName", "%s",  (pu->chan->cid.cid_name ? pu->chan->cid.cid_name : "<unknown>"))
	);
	...
	if (!con) {
		...
		if (!con) {
			cw_log(CW_LOG_ERROR, "Parking context '%s' does not exist and unable to create\n", parking_con);
		}
	}
	if (con) {
...
static int builtin_automonitor(struct cw_channel *chan, struct cw_channel *peer, struct cw_bridge_config *config, char *code, int sense)
{
	...
	if (!monitor_ok) {
		cw_log(CW_LOG_ERROR,"Cannot record the call. The monitor application is disabled.\n");
		return -1;
	}
	...
	cw_log(CW_LOG_NOTICE,"Cannot record the call. One or both channels have gone away.\n");	
	return -1;
}
...
static int feature_exec_app(struct cw_channel *chan, struct cw_channel *peer, struct cw_bridge_config *config, char *code, int sense)
{
	...
	if (!feature) { /* shouldn't ever happen! */
		cw_log(CW_LOG_NOTICE, "Found feature before, but at execing we've lost it??\n");
		return -1; 
	}
	...
	res = cw_function_exec_str(work, cw_hash_string(0, feature->app), feature->app, args, NULL);
	if (res < 0) {
		if (errno == ENOENT)
...
static int remap_feature(const char *name, const char *value)
{
	...
	for (x = 0; x < FEATURES_COUNT; x++) {
		if (!strcasecmp(name, builtin_features[x].sname)) {
			...
			cw_log(CW_LOG_WARNING, "Sequence '%s' already mapped to function %s (%s) while assigning to %s\n", value, builtin_features[x].fname, builtin_features[x].sname, name);
	}
	return res;
}
		...
		if (!cw_call(chan, data)) {
			...
			while (!cw_check_hangup(caller) && timeout && (chan->_state != CW_STATE_UP)) {
				...
				if(cw_tvdiff_ms(cw_tvnow(), started) > timeout) {
					...
					cw_log(CW_LOG_NOTICE, "We exceeded our AT-timeout\n");
					break; /*doh! timeout*/
				}
				...
				if (chan && (chan == active_channel)){
					...
					if (f->frametype == CW_FRAME_CONTROL || f->frametype == CW_FRAME_DTMF || f->frametype == CW_FRAME_TEXT) {
						if (f->subclass == CW_CONTROL_RINGING) {
							...
							cw_log(CW_LOG_NOTICE, "Don't know what to do about control frame: %d\n", f->subclass);
						}
						/* else who cares */
					}
...
int cw_bridge_call(struct cw_channel *chan,struct cw_channel *peer,struct cw_bridge_config *config)
{
	...
	for (;;) {
		...
		if (config->feature_timer) {
			...
			if (hasfeatures) {
				...
				if (backup_config.feature_timer && ((backup_config.feature_timer -= diff) <= 0)) {
					cw_log(CW_LOG_DEBUG, "Timed out, realtime this time!\n");
					...
					cw_log(CW_LOG_DEBUG, "Timed out for feature!\n");
		...
		if (res < 0) {
			cw_log(CW_LOG_WARNING, "Bridge failed on channels %s and %s\n", chan->name, peer->name);
			return -1;
		}
			...
			if (tms > pu->parkingtime) {
				...
				if (pu->peername[0]) {
					...
					if (!con) {
						...
						if (!con) {
							cw_log(CW_LOG_ERROR, "Parking dial context '%s' does not exist and unable to create\n", parking_con_dial);
						}
					}
					if (con) {
				...
				if (cw_pbx_start(pu->chan))  {
					cw_log(CW_LOG_WARNING, "Unable to restart the PBX for user on '%s', hanging them up...\n", pu->chan->name);
					cw_hangup(pu->chan);
				}
				...
				if (con) {
					...
					if (cw_context_remove_extension2(con, exten, 1, NULL))
						cw_log(CW_LOG_WARNING, "Whoa, failed to remove the extension!\n");
				} else
					...
					if ((pu->chan->fds[x] > -1) && (FD_ISSET(pu->chan->fds[x], &rfds) || FD_ISSET(pu->chan->fds[x], &efds))) {
						...
						if (!f || ((f->frametype == CW_FRAME_CONTROL) && (f->subclass ==  CW_CONTROL_HANGUP))) {
							...
							if (pu->moh_trys < 3 && !cw_generator_is_active(pu->chan)) {
								cw_log(CW_LOG_DEBUG, "MOH on parked call stopped by outside source.  Restarting.\n");
								cw_moh_start(pu->chan, NULL);
								pu->moh_trys++;
							}
...
static int park_exec(struct cw_channel *chan, int argc, char **argv, struct cw_dynstr *result)
{
	...
	if (peer) {
		...
		if (res < 0) {
			cw_log(CW_LOG_WARNING, "Could not make channels %s and %s compatible for bridge\n", chan->name, peer->name);
			cw_hangup(peer);
			return -1;
		}
		...
		if (!dres)
			...
			cw_log(CW_LOG_WARNING, "cw_streamfile of %s failed on %s\n", "pbx-invalidpark", chan->name);
			dres = 0;
		}
		if (option_verbose > 2) 
...
static struct cw_clicmd showfeatures = {
...
static struct cw_clicmd showparked = {
...
int cw_features_reload(void) 
{
	...
	if (!cfg) {
		...
		if (cfg)
			cw_log(CW_LOG_NOTICE, "parking.conf is deprecated in favor of 'features.conf'.  Please rename it.\n");
	}
	if (cfg) {
		...
		while(var) {
			if (!strcasecmp(var->name, "parkext")) {
				...
				if ((sscanf(var->value, "%d", &parkingtime) != 1) || (parkingtime < 1)) {
					cw_log(CW_LOG_WARNING, "%s is not a valid parkingtime\n", var->value);
					parkingtime = DEFAULT_PARK_TIME;
				} else
					parkingtime = parkingtime * 1000;
			} else if (!strcasecmp(var->name, "parkpos")) {
				if (sscanf(var->value, "%d-%d", &start, &end) != 2) {
					cw_log(CW_LOG_WARNING, "Format for parking positions is a-b, where a and b are numbers at line %d of parking.conf\n", var->lineno);
				} else {
					parking_start = start;
					parking_stop = end;
				}
				...
				if ((sscanf(var->value, "%d", &transferdigittimeout) != 1) || (transferdigittimeout < 1)) {
					cw_log(CW_LOG_WARNING, "%s is not a valid transferdigittimeout\n", var->value);
					transferdigittimeout = DEFAULT_TRANSFER_DIGIT_TIMEOUT;
				} else
					transferdigittimeout = transferdigittimeout * 1000;
			} else if (!strcasecmp(var->name, "featuredigittimeout")) {
				if ((sscanf(var->value, "%d", &featuredigittimeout) != 1) || (featuredigittimeout < 1)) {
					cw_log(CW_LOG_WARNING, "%s is not a valid featuredigittimeout\n", var->value);
					featuredigittimeout = DEFAULT_FEATURE_DIGIT_TIMEOUT;
				}
		...
		while(var) {
			if (remap_feature(var->name, var->value))
				cw_log(CW_LOG_NOTICE, "Unknown feature '%s'\n", var->name);
			var = var->next;
		}
		...
		while(var) {
			...
			if (!(app && strlen(app)) || !(exten && strlen(exten)) || !(party && strlen(party)) || !(var->name && strlen(var->name))) {
				cw_log(CW_LOG_NOTICE, "Please check the feature Mapping Syntax, either extension, name, or app aren't provided %s %s %s %s\n",app,exten,party,var->name);
				free(tmp_val);
				var = var->next;
				continue;
			}

			{
				...
				if (!strcasecmp(party,"caller"))
					...
					cw_log(CW_LOG_NOTICE, "Invalid party specification for feature '%s', must be caller, or callee\n", var->name);
					var = var->next;
					continue;
				}
	...
	if (!cw_strlen_zero(old_parking_con) && (con = cw_context_find(old_parking_con)))   {
		...
		cw_log(CW_LOG_DEBUG, "Removed old parking extension %s@%s\n", old_parking_ext, old_parking_con);
	}
