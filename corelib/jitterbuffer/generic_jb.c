...
int cw_jb_put(struct cw_channel *chan, struct cw_frame *f, int codec)
{
	...
	if(frr == NULL)
	{
		cw_log(CW_LOG_ERROR, "Failed to isolate frame for the jitterbuffer on channel '%s'\n", chan->name);
		return -1;
	}
...
static void jb_get_and_deliver(struct cw_channel *chan)
{
	...
	while(now >= jb->next)
	{
		...
		case JB_IMPL_NOFRAME:
			...
			return;
		default:
			cw_log(CW_LOG_ERROR, "This should never happen!\n");
			CRASH;
			break;
		}
...
static int create_jb(struct cw_channel *chan, struct cw_frame *frr, int codec)
{
	...
	if (chan) {
		...
		cw_log(CW_LOG_ERROR, "No channel provided!\n");
		return 0;
	}
	...
	if(jbobj == NULL)
	{
		cw_log(CW_LOG_WARNING, "Failed to create jitterbuffer on channel '%s'\n", chan->name);
		return -1;
	}
	...
	if(res != JB_IMPL_OK)
	{
		cw_log(CW_LOG_WARNING, "Failed to put first frame in the jitterbuffer on channel '%s'\n", chan->name);
		/*
	...
	if(cw_test_flag(jbconf, CW_GENERIC_JB_LOG))
	{
		...
		if(jb->logfile == NULL)
		{
			cw_log(CW_LOG_WARNING, "Failed to create frame log file with pathname '%s'\n", logfile_pathname);
		}
...
void cw_jb_destroy(struct cw_channel *chan)
{
	...
	if (chan) {
		...
		cw_log(CW_LOG_ERROR, "Channel/jitterbuffer data is broken!\n");
		return;
	}
...
void cw_jb_default_config(struct cw_jb_conf *conf)
{
	if (conf) {
		...
		cw_log(CW_LOG_ERROR, "No jitterbuffer conf struct provided!\n");
}
...
int cw_jb_is_active(struct cw_channel *chan)
{
	...
	if (chan) {
		...
		cw_log(CW_LOG_ERROR, "Trying to retreive flag but structs are freed");	
		return 0;
	}
}
