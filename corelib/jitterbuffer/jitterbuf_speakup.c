...
//private functions
static void set_default_settings(speakup_jitterbuffer *jb); 
static void reset(speakup_jitterbuffer *jb); 
static long find_pointer(long *array, long max_index, long value); static void frame_free(jb_frame *frame);

static void put_control(speakup_jitterbuffer *jb, void *data, int type, long ts); 
static void put_voice(speakup_jitterbuffer *jb, void *data, int type, long ms, long ts, int codec); 
static void put_history(speakup_jitterbuffer *jb, long ts, long now, long ms, int codec); 
static void calculate_info(speakup_jitterbuffer *jb, long ts, long now, int codec);

static int get_control(speakup_jitterbuffer *jb, void **data); 
static int get_voice(speakup_jitterbuffer *jb, void **data, long now, long interpl); 
static int get_voicecase(speakup_jitterbuffer *jb, void **data, long now, long interpl, long diff);

static int get_next_frametype(speakup_jitterbuffer *jb, long ts); 
static long get_next_framets(speakup_jitterbuffer *jb); 
static jb_frame *get_frame(speakup_jitterbuffer *jb, long ts); 
static jb_frame *get_all_frames(speakup_jitterbuffer *jb);
...
{
  speakup_jitterbuffer *jb;
  
  jb_dbg("N");
  jb = calloc(1, sizeof(speakup_jitterbuffer));
  if (!jb) {
    jb_err("cannot allocate speakup_jitterbuffer\n");
    return NULL;
  }
