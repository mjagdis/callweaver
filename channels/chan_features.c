...
static inline int indexof(struct feature_pvt *p, struct cw_channel *owner, int nullok)
{
	int x;
	if (!owner) {
		cw_log(CW_LOG_WARNING, "indexof called on NULL owner??\n");
		return -1;
	}
	for (x=0; x<3; x++) {
...
static int features_call(struct cw_channel *ast, char *dest, int timeout)
{
	...
	if (dest2) {
		...
		if (!x && p->subchan) {
			...
			cw_log(CW_LOG_NOTICE, "Uhm yah, not quite there with the call waiting...\n");
		cw_mutex_unlock(&p->lock);
	}
	return res;
}
...
static struct feature_pvt *features_alloc(char *data, int format)
{
	...
	if (!dest) {
		cw_log(CW_LOG_NOTICE, "Format for feature channel is Feature/Tech/Dest ('%s' not valid)!\n", 
			data);
		return NULL;
	}
	cw_mutex_lock(&featurelock);
	...
	if (!tmp) {
		...
		if (!chan) {
			cw_log(CW_LOG_NOTICE, "Unable to allocate subchannel '%s/%s'\n", tech, dest);
			return NULL;
		}
...
static struct cw_channel *features_new(struct feature_pvt *p, int state, int index)
{
	...
	int x,y;
	if (!p->subchan) {
		cw_log(CW_LOG_WARNING, "Called upon channel with no subchan:(\n");
		return NULL;
	}
	if (p->subs[index].owner) {
		cw_log(CW_LOG_WARNING, "Called to put index %d already there!\n", index);
		return NULL;
	}
...
static struct cw_clicmd cli_show_features = {
