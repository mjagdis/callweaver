...
OPENPBX_FILE_VERSION(__FILE__, "$Revision: 1.48 $")
...
int load_module()
{
	...
	if (res < 0) {
		if (option_verbose > 1) {
			...
			ast_verbose(VERBOSE_PREFIX_2 "Turn off ALSA support by adding 'noload=chan_alsa.so' in /etc/openpbx/modules.conf\n");
		}
		return 0;
	}
