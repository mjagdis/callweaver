...
OPENPBX_FILE_VERSION(__FILE__, "$Revision: 1.130 $")
...
struct oh323_pvt {
	...
	int needhangup;						/* Send hangup when OpenPBX is ready */
...
/**
  * Callback for sending digits from H.323 up to openpbx
  *
  */
...
/**
  * Call-back function passing remote ip/port information from H.323 to openpbx 
  *
  * Returns nothing 
  */
...
/**  
  *	Call-back function to signal openpbx that the channel has been answered 
  * Returns nothing
  */
void connection_made(unsigned call_reference, const char *token)
{
	...
	/* Inform openpbx about remote party connected only on outgoing calls */
	if (!pvt->outgoing) {
		ast_mutex_unlock(&pvt->lock);
		return;
	}
...
int progress(unsigned call_reference, const char *token, int inband)
{
	...
	if (!pvt->owner) {
		ast_mutex_unlock(&pvt->lock);
		ast_log(LOG_ERROR, "No OpenPBX channel associated with private structure.\n");
		return -1;
	}
...
static int answer_call(unsigned call_reference, const char *token)
{
	...
	if (h323debug)
		ast_log(LOG_DEBUG, "Preparing OpenPBX to answer for %s\n", token);
	...
	/* allocate a channel and tell openpbx about it */
...
/**
  *  Call-back function to signal openpbx that the channel is ringing
  *  Returns nothing
  */
