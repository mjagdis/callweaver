 ...
 * ast_h323.cpp
 *
 * OpenH323 Channel Driver for OPENPBX PBX.
 *			By  Jeremy McNamara
 ...
 *
 * This file is part of the chan_h323 driver for OpenPBX
 *
...
class POpenPBXLog : public PObject, public iostream {
	PCLASSINFO(POpenPBXLog, PObject);

	public:
	POpenPBXLog() : iostream(cout.rdbuf()) { init(&buffer); }
	~POpenPBXLog() { flush(); }

	private:
	POpenPBXLog(const POpenPBXLog &) : iostream(cout.rdbuf()) { }
	POpenPBXLog & operator=(const POpenPBXLog &) { return *this; }
...
static POpenPBXLog *logstream = NULL;

int POpenPBXLog::Buffer::overflow(int c)
{
	if (pptr() >= epptr()) {
		int ppos = pptr() - pbase();
...
int POpenPBXLog::Buffer::underflow()
{
	return EOF;
}

int POpenPBXLog::Buffer::sync()
{
...
MyProcess::MyProcess(): PProcess("The NuFone Network's", "H.323 Channel Driver for OpenPBX",
...
//	int gsmFrames  = 4;
	...
	if (cap & AST_FORMAT_SPEEX) {
		/* Not real sure if OpenPBX acutally supports all
...
BOOL MyH323_ExternalRTPChannel::Start(void)
{
	...
	if (GetDirection() == IsTransmitter) {
		...
		/* Notify OpenPBX of remote RTP information */
...
int h323_set_gk(int gatekeeper_discover, char *gatekeeper, char *secret)
{
	...
	if (gatekeeper_discover) {
		...
		if (endPoint->SetGatekeeper(gkName, rasChannel)) {
			...
			/* XXX Maybe we should fire a new thread to attempt to re-register later and not kill openpbx here? */
			return 1;
		}
	}
	return 0;
