...
static int sccp_pbx_call(struct cw_channel *ast, char *dest) {
	...
	if (!c) {
		cw_log(CW_LOG_WARNING, "SCCP: CallWeaver request to call %s channel: %s, but we don't have this channel!\n", dest, ast->name);
		return -1;
	}
	...
	if (!l || !d || !s) {
		cw_log(CW_LOG_WARNING, "SCCP: weird error. The channel %d has no line or device or session\n", (c ? c->callid : 0) );
		return -1;
	}
  ...
  /* Set the channel calledParty Name and Number 7910 compatibility*/
	...
	if ( sccp_channel_get_active(d) ) {
		...
		if (c->autoanswer_type) {
			...
			if (cw_pthread_create(&t, &global_attr_detached, sccp_pbx_call_autoanswer_thread, &c->callid)) {
...
static int sccp_pbx_answer(struct cw_channel *ast) {
	...
	if (!c || !c->device || !c->line) {
		cw_log(CW_LOG_ERROR, "SCCP: Answered %s but no SCCP channel\n", ast->name);
		return -1;
	}
...
/* when the bridged channel hold/unhold the call we are notified here */
	...
	default:
	  cw_log(CW_LOG_WARNING, "SCCP: Don't know how to indicate condition %d\n", ind);
	  res = -1;
	}
...
static int sccp_pbx_fixup(struct cw_channel *oldchan, struct cw_channel *newchan) {
	...
	if (!c) {
		cw_log(CW_LOG_WARNING, "sccp_pbx_fixup(old: %s(%p), new: %s(%p)). no SCCP channel to fix\n", oldchan->name, oldchan, newchan->name, newchan);
		return -1;
	}
	cw_mutex_lock(&c->lock);
	if (c->owner != oldchan) {
...
uint8_t sccp_pbx_channel_allocate(sccp_channel_t * c) {
	...
	if (!l || !d || !d->session) {
		cw_log(CW_LOG_ERROR, "SCCP: Unable to allocate asterisk channel\n");
		return 0;
	}
