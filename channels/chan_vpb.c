...
OPENPBX_FILE_VERSION(__FILE__, "$Revision: 1.94 $")
...
/* Use OpenPBX Indication or VPB */
static int use_ast_ind=0;

/* Use OpenPBX DTMF detection or VPB */
static int use_ast_dtmfdet=0;

static int relaxdtmf=0;

/* Use OpenPBX DTMF play back or VPB */
...
static struct vpb_pvt {
	...
	struct ast_frame f, fr;			/* OpenPBX frame interface */
...
*  Get openpbx to do the bridging
*/
#define BAD_V4PCI_BRIDGE

/* This one enables a half duplex bridge which may be required to prevent high pitched
 * feedback when getting openpbx to do the bridging and when using certain gain settings.
 */
/* #define HALF_DUPLEX_BRIDGE */

/* This is the Native bridge code, which OpenPBX will try before using its own bridging code */
static enum ast_bridge_result vpb_bridge(struct ast_channel *c0, struct ast_channel *c1, int flags, struct ast_frame **fo, struct ast_channel **rc)
{
...
static int vpb_digit(struct ast_channel *ast, char digit)
{
	...
	if (use_ast_dtmf){
		if (option_verbose > 3)
			ast_verbose(VERBOSE_PREFIX_4 "%s: vpb_digit: asked to play digit[%c] but we are using openpbx dtmf play back?!\n", p->dev, digit);
		return 0;
	}
...
static void *do_chanreads(void *pvt)
{
	...
	while (!p->stopreads && p->owner) {
		...
		if (p->bridge) {
			...
			if (p->owner->_bridge){
				if (option_verbose > 4){
					ast_verbose("%s: chanreads: Got OpenPBX bridge with [%s].\n", p->dev,p->owner->_bridge->name);
				}
				bridgerec = 1;
			}
			else {
...
/*		if ( (p->owner->_state != AST_STATE_UP) || !bridgerec) */
		...
		if( p->play_dtmf[0] ) {
			...
			/* This DTMF is played by openpbx and leads to an annoying trailing beep on CISCO phones */
			if( !ignore_dtmf) 
	...
	ast_mutex_lock(&iflock); {
		...
		while (v){
			if (strcasecmp(v->name, "cards") == 0) {
				...
				use_ast_ind = 1;
				ast_log(LOG_NOTICE,"VPB driver using OpenPBX Indication functions!\n");
			}
				...
				use_ast_dtmf = 1;
				ast_log(LOG_NOTICE,"VPB driver using OpenPBX DTMF play functions!\n");
			}
			else if (strcasecmp(v->name, "ast-dtmf-det") == 0) {
				use_ast_dtmfdet = 1;
				ast_log(LOG_NOTICE,"VPB driver using OpenPBX DTMF detection functions!\n");
			}
			else if (strcasecmp(v->name, "relaxdtmf") == 0) {
				relaxdtmf = 1;
				ast_log(LOG_NOTICE,"VPB driver using Relaxed DTMF with OpenPBX DTMF detections functions!\n");
			}
