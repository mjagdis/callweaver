...
static unsigned char compress_subclass(int subclass)
{
	...
	for (x = 0; x < IAX_MAX_SHIFT; x++) {
		if (subclass & (1 << x)) {
			if (power > -1) {
				cw_log(CW_LOG_WARNING, "Can't compress subclass %d\n", subclass);
				return 0;
			} else
				power = x;
		}
...
static void update_max_trunk(void)
{
	...
	if (option_debug && iaxdebug)
		cw_log(CW_LOG_DEBUG, "New max trunk callno is %d\n", max);
}
...
static int find_callno(unsigned short callno, unsigned short dcallno, struct sockaddr_in *sin, int new, int lockpeer, int sockfd)
{
	...
	if ((res < 1) && (new >= NEW_ALLOW)) {
		...
		if (iaxs[x]) {
			if (option_debug && iaxdebug)
				cw_log(CW_LOG_DEBUG, "Creating new call structure %d\n", x);
			...
			cw_log(CW_LOG_WARNING, "Out of resources\n");
			cw_mutex_unlock(&iaxsl[x]);
			return 0;
		}
		cw_mutex_unlock(&iaxsl[x]);
...
static int handle_error(void)
{
	...
	if (res < 0)
		cw_log(CW_LOG_WARNING, "Error detected, but unable to read error: %s\n", strerror(errno));
	else {
		if (m.msg_controllen) {
			sin = (struct sockaddr_in *)SO_EE_OFFENDER(&e);
			if (sin) 
				cw_log(CW_LOG_WARNING, "Receive error from %s\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr));
			else
				cw_log(CW_LOG_WARNING, "No address detected??\n");
		} else {
			cw_log(CW_LOG_WARNING, "Local error: %s\n", strerror(e.ee_errno));
		}
	}
#endif
	return 0;
...
static int transmit_trunk(struct iax_frame *f, struct sockaddr_in *sin, int sockfd)
{
	...
	if (res < 0) {
		if (option_debug)
			cw_log(CW_LOG_DEBUG, "Received error: %s\n", strerror(errno));
		handle_error();
	} else
		res = 0;
	return res;
...
static int forward_delivery(struct iax_frame *fr)
{
	...
	if (option_debug)
		cw_log(CW_LOG_DEBUG, "forward_delivery: Forwarding ts=%d on %d/%d to %d/%d on %s:%d\n",
				fr->ts,
	...
	*/
	if (fr->ts + 50000 <= p1->last) {
		...
		if (option_debug)
			cw_log(CW_LOG_DEBUG, "forward_delivery: pushed forward timestamp to %u\n", fr->ts);
	}
...
static void unwrap_timestamp(struct iax_frame *fr)
{
	...
	if ( (fr->ts & 0xFFFF0000) == (iaxs[fr->callno]->last & 0xFFFF0000) ) {
		...
		if (x < -50000) {
			...
			if (option_debug && iaxdebug)
				cw_log(CW_LOG_DEBUG, "schedule_delivery: pushed forward timestamp\n");
		}
		if (x > 50000) {
			...
			fr->ts = ( (iaxs[fr->callno]->last & 0xFFFF0000) - 0x10000) | (fr->ts & 0xFFFF);
			if (option_debug && iaxdebug)
				cw_log(CW_LOG_DEBUG, "schedule_delivery: pushed back timestamp\n");
		}
	}
}
...
#if 0
	if (option_debug && iaxdebug)
		cw_log(CW_LOG_DEBUG, "schedule_delivery: ts=%d, last=%d, update=%d\n",
...
#if 0
		cw_log(CW_LOG_DEBUG, "schedule_delivery: set delivery to 0 as we don't have an rxcore yet, or frame is from trunk.\n");
#endif
...
static int iax2_fixup(struct cw_channel *oldchannel, struct cw_channel *newchan)
{
	...
	if (iaxs[callno])
		...
		cw_log(CW_LOG_WARNING, "Uh, this isn't a good sign...\n");
	cw_mutex_unlock(&iaxsl[callno]);
	return 0;
}
...
static struct iax2_peer *realtime_peer(const char *peername, struct sockaddr_in *sin)
{
	...
	if (!cw_test_flag(&globalflags, IAX_RTIGNOREREGEXPIRE) && dynamic) {
		...
		if ((nowtime - regseconds) > IAX_DEFAULT_REG_EXPIRE) {
			...
			if (option_debug)
				cw_log(CW_LOG_DEBUG, "realtime_peer: Bah, '%s' is expired (%ld/%ld/%ld)!\n",
						peername, nowtime - regseconds, regseconds, nowtime);
		}
		else {
			if (option_debug)
				cw_log(CW_LOG_DEBUG, "realtime_peer: Registration for '%s' still active (%ld/%ld/%ld)!\n",
						peername, nowtime - regseconds, regseconds, nowtime);
		}
	}
...
static int create_addr(const char *peername, struct sockaddr_in *sin, struct create_addr_info *cai)
{
	...
	if (!(peer = find_peer(peername, 1))) {
		...
		if (hp) {
			...
			cw_log(CW_LOG_WARNING, "No such host: %s\n", peername);
			return -1;
		}
	}
...
static int iax2_call(struct cw_channel *c, char *dest, int timeout)
{
	...
	if ((c->_state != CW_STATE_DOWN) && (c->_state != CW_STATE_RESERVED)) {
		cw_log(CW_LOG_WARNING, "Channel is already in use (%s)?\n", c->name);
		return -1;
	}
	...
	if (create_addr(pds.peer, &sin, &cai)) {
		cw_log(CW_LOG_WARNING, "No address associated with '%s'\n", pds.peer);
		return -1;
	}
...
static int iax2_hangup(struct cw_channel *c) 
{
	...
	if (callno && iaxs[callno]) {
		cw_log(CW_LOG_DEBUG, "We're hanging up %s now...\n", c->name);
		...
		if (alreadygone) {
			cw_log(CW_LOG_DEBUG, "Really destroying %s now...\n", c->name);
	...
	for (/* ever */;;) {
		...
		!(flags & (CW_BRIDGE_DTMF_CHANNEL_0 | CW_BRIDGE_DTMF_CHANNEL_1))) {
			...
			if (iax2_start_transfer(callno0, callno1))
				cw_log(CW_LOG_WARNING, "Unable to start the transfer\n");
			transferstarted = 1;
		}
...
static int iax2_answer(struct cw_channel *c)
{
	unsigned short callno = PTR_TO_CALLNO(c->tech_pvt);
	if (option_debug)
		cw_log(CW_LOG_DEBUG, "Answering IAX2 call\n");
...
static int iax2_indicate(struct cw_channel *c, int condition)
{
	unsigned short callno = PTR_TO_CALLNO(c->tech_pvt);
	if (option_debug && iaxdebug)
		cw_log(CW_LOG_DEBUG, "Indicating condition %d\n", condition);
...
static unsigned int calc_timestamp(struct chan_iax2_pvt *p, unsigned int ts, struct cw_frame *f)
{
	...
	*/
	...
	if (delivery && !cw_tvzero(*delivery)) {
		...
		if (option_debug > 2 && iaxdebug)
			cw_log(CW_LOG_DEBUG, "calc_timestamp: call %d/%d: Timestamp slaved to delivery time\n", p->callno, iaxs[p->callno]->peercallno);
	} else {
...
/* 		p->rxcore = cw_tvnow(); */
/* 		if (option_debug && iaxdebug) */
/* 			cw_log(CW_LOG_DEBUG, "calc_rxstamp: call=%d: rxcore set to %d.%6.6d - %dms\n", */
/* 					p->callno, (int)(p->rxcore.tv_sec), (int)(p->rxcore.tv_usec), offset); */
/* 		p->rxcore = cw_tvsub(p->rxcore, cw_samp2tv(offset, 1000)); */
/* #if 1 */
/* 		if (option_debug && iaxdebug) */
/* 			cw_log(CW_LOG_DEBUG, "calc_rxstamp: call=%d: works out as %d.%6.6d\n", */
	...
	if (tpeer) {
		if (tpeer->trunkdatalen + f->datalen + 4 >= tpeer->trunkdataalloc) {
			...
			if (tpeer->trunkdataalloc < MAX_TRUNKDATA) {
				...
				cw_log(CW_LOG_WARNING, "Maximum trunk data space exceeded to %s:%d\n", cw_inet_ntoa(iabuf, sizeof(iabuf), tpeer->addr.sin_addr), ntohs(tpeer->addr.sin_port));
				cw_mutex_unlock(&tpeer->lock);
				return -1;
			}
		}
...
static void memcpy_decrypt(unsigned char *dst, const unsigned char *src, int len, aes_decrypt_ctx *dcx)
{
	...
	int x;
	if (len % 16)
		cw_log(CW_LOG_WARNING, "len should be multiple of 16, not %d!\n", len);
	for (x=0;x<len;x++)
		dst[x] = src[x] ^ 0xff;
#else	
...
static int decode_frame(aes_decrypt_ctx *dcx, struct cw_iax2_full_hdr *fh, struct cw_frame *f, int *datalen)
{
	...
	if (ntohs(fh->scallno) & IAX_FLAG_FULL) {
		...
		if (option_debug && iaxdebug)
			cw_log(CW_LOG_DEBUG, "Decoding full frame with length %d (padding = %d) (15=%02x)\n", *datalen, padding, workspace[15]);
		...
		*datalen -= padding;
		...
		if (option_debug && iaxdebug)
			cw_log(CW_LOG_DEBUG, "Decoding mini with length %d\n", *datalen);
...
static int encrypt_frame(aes_encrypt_ctx *ecx, struct cw_iax2_full_hdr *fh, unsigned char *poo, int *datalen)
{
	...
	if (ntohs(fh->scallno) & IAX_FLAG_FULL) {
		struct cw_iax2_full_enc_hdr *efh = (struct cw_iax2_full_enc_hdr *)fh;
		if (option_debug && iaxdebug)
			cw_log(CW_LOG_DEBUG, "Encoding full frame %d/%d with length %d\n", fh->type, fh->csub, *datalen);
		...
		if (option_debug && iaxdebug)
			cw_log(CW_LOG_DEBUG, "Encoding full frame %d/%d with length %d + %d padding (15=%02x)\n", fh->type, fh->csub, *datalen, padding, workspace[15]);
		*datalen += padding;
		...
		struct cw_iax2_mini_enc_hdr *efh = (struct cw_iax2_mini_enc_hdr *)fh;
		if (option_debug && iaxdebug)
			cw_log(CW_LOG_DEBUG, "Encoding mini frame with length %d\n", *datalen);
...
static int iax2_send(struct chan_iax2_pvt *pvt, struct cw_frame *f, unsigned int ts, int seqno, int now, int transfer, int final)
{
	...
	if (!pvt) {
		cw_log(CW_LOG_WARNING, "No private structure for packet?\n");
		return -1;
	}
	...
	if (!sendmini)
    {
		...
		if (cw_test_flag(pvt, IAX_ENCRYPTED))
        {
			if (cw_test_flag(pvt, IAX_KEYPOPULATED))
            {
				...
				cw_log(CW_LOG_WARNING, "Supposed to send packet encrypted, but no key?\n");
		}
...
static int iax2_write(struct cw_channel *c, struct cw_frame *f)
{
	...
	/* If there's an outstanding error, return failure now */
		if (!iaxs[callno]->error) {
			...
			cw_log(CW_LOG_DEBUG, "Write error: %s\n", strerror(errno));
		}
	}
...
static int check_access(int callno, struct sockaddr_in *sin, struct iax_ies *ies)
{
	...
	if (version > IAX_PROTO_VERSION) {
		cw_log(CW_LOG_WARNING, "Peer '%s' has too new a protocol version (%d) for me\n", 
...
static int raw_hangup(struct sockaddr_in *sin, unsigned short src, unsigned short dst, int sockfd)
{
	...
	if (option_debug)
#endif	
		cw_log(CW_LOG_DEBUG, "Raw Hangup %s:%d, src=%d, dst=%d\n",
	...
	if ((p->authmethods & IAX_AUTH_RSA) && !cw_strlen_zero(rsasecret) && !cw_strlen_zero(p->inkeys)) {
		...
		while(keyn) {
			...
			if (key && !cw_check_signature(key, p->challenge, rsasecret)) {
				...
				cw_log(CW_LOG_WARNING, "requested inkey '%s' for RSA authentication does not exist\n", keyn);
			keyn = strsep(&stringp, ":");
		}
...
static int register_verify(int callno, struct sockaddr_in *sin, struct iax_ies *ies)
{
	...
	if (cw_strlen_zero(peer)) {
		cw_log(CW_LOG_NOTICE, "Empty registration from %s\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr));
		return -1;
	}
	...
	if (!p) {
		if (authdebug)
			cw_log(CW_LOG_NOTICE, "No registration for peer '%s' (from %s)\n", peer, cw_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr));
		return -1;
	}

	if (!cw_test_flag(p, IAX_DYNAMIC)) {
		if (authdebug)
			cw_log(CW_LOG_NOTICE, "Peer '%s' is not dynamic (from %s)\n", peer, cw_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr));
	...
	if (!cw_strlen_zero(rsasecret) && (p->authmethods & IAX_AUTH_RSA) && !cw_strlen_zero(iaxs[callno]->challenge)) {
		if (!cw_strlen_zero(p->inkeys)) {
			...
			while(keyn) {
				...
				if (key && !cw_check_signature(key, iaxs[callno]->challenge, rsasecret)) {
					...
					cw_log(CW_LOG_WARNING, "requested inkey '%s' does not exist\n", keyn);
				keyn = strsep(&stringp, ":");
			}
			if (!keyn) {
				if (authdebug)
					cw_log(CW_LOG_NOTICE, "Host %s failed RSA authentication with inkeys '%s'\n", peer, p->inkeys);
				if (cw_test_flag(p, IAX_TEMPONLY))
					destroy_peer(p);
				return -1;
			}
			...
			if (authdebug)
				cw_log(CW_LOG_NOTICE, "Host '%s' trying to do RSA authentication, but we have no inkeys\n", peer);
			if (cw_test_flag(p, IAX_TEMPONLY))
				destroy_peer(p);
			return -1;
		}
		...
		if (strcmp(secret, p->secret)) {
			if (authdebug)
				cw_log(CW_LOG_NOTICE, "Host %s did not provide proper plaintext password for '%s'\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr), p->name);
			if (cw_test_flag(p, IAX_TEMPONLY))
				destroy_peer(p);
			return -1;
		} else
		...
		if (authdebug)
			cw_log(CW_LOG_NOTICE, "Inappropriate authentication received\n");
		if (cw_test_flag(p, IAX_TEMPONLY))
			destroy_peer(p);
		return -1;
	}
...
static int authenticate(char *challenge, char *secret, char *keyn, int authmethods, struct iax_ie_data *ied, struct sockaddr_in *sin, aes_encrypt_ctx *ecx, aes_decrypt_ctx *dcx)
{
	...
	if (!cw_strlen_zero(keyn)) {
		if (!(authmethods & IAX_AUTH_RSA)) {
			if (cw_strlen_zero(secret)) 
				cw_log(CW_LOG_NOTICE, "Asked to authenticate to %s with an RSA key, but they don't allow RSA authentication\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr));
		} else if (cw_strlen_zero(challenge)) {
			cw_log(CW_LOG_NOTICE, "No challenge provided for RSA authentication to %s\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr));
		} else {
			char sig[256];
			struct cw_key *key;
			key = cw_key_get(keyn, CW_KEY_PRIVATE);
			if (!key) {
				cw_log(CW_LOG_NOTICE, "Unable to find private key '%s'\n", keyn);
			} else {
				if (cw_sign(key, challenge, sig)) {
					cw_log(CW_LOG_NOTICE, "Unable to sign challenge withy key\n");
					res = -1;
				} else {
	...
	if (res && !cw_strlen_zero(secret)) {
		if ((authmethods & IAX_AUTH_MD5) && !cw_strlen_zero(challenge)) {
			...
			cw_log(CW_LOG_NOTICE, "No way to send secret to peer '%s' (their methods: %d)\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr), authmethods);
	}
	return res;
}
...
static int iax2_do_register_s(void *data)
{
	...
	cw_pthread_create(&tid, &global_attr_detached, iax2_do_register_thread, data);
	return 0;
}
...
static int try_transfer(struct chan_iax2_pvt *pvt, struct iax_ies *ies)
{
	...
	if (!newcall || !new.sin_addr.s_addr || !new.sin_port) {
		cw_log(CW_LOG_WARNING, "Invalid transfer request\n");
		return -1;
	}
...
static int iax2_ack_registry(struct iax_ies *ies, struct sockaddr_in *sin, int callno)
{
	...
	reg = iaxs[callno]->reg;
	if (!reg) {
		cw_log(CW_LOG_WARNING, "Registry acknowledge on unknown registry '%s'\n", peer);
		return -1;
	}
	if (inaddrcmp(&reg->addr, sin)) {
		cw_log(CW_LOG_WARNING, "Received unsolicited registry ack from '%s'\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr));
		return -1;
	}
...
static int expire_registry(void *data)
{
	...
	cw_log(CW_LOG_DEBUG, "Expiring registration for peer '%s'\n", p->name);
...
static int update_registry(char *name, struct sockaddr_in *sin, int callno, char *devtype, int fd, unsigned short refresh)
{
	...
	if (!(p = find_peer(name, 1))) {
		cw_log(CW_LOG_WARNING, "No such peer '%s'\n", name);
		return -1;
	}
	...
	if (refresh > max_reg_expire) {
		cw_log(CW_LOG_NOTICE, "Restricting registration for peer '%s' to %d seconds (requested %d)\n",
...
static int registry_rerequest(struct iax_ies *ies, int callno, struct sockaddr_in *sin)
{
	...
	if (reg) {
			if (inaddrcmp(&reg->addr, sin)) {
				cw_log(CW_LOG_WARNING, "Received unsolicited registry authenticate request from '%s'\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr));
				return -1;
			}
			if (cw_strlen_zero(reg->secret)) {
				cw_log(CW_LOG_NOTICE, "No secret associated with peer '%s'\n", reg->username);
...
static void spawn_dp_lookup(int callno, char *context, char *callednum, char *callerid)
{
	...
	if (dpr) {
		...
		if (cw_pthread_create(&newthread, &global_attr_detached, dp_lookup_thread, dpr)) {
...
static int iax_park(struct cw_channel *chan1, struct cw_channel *chan2)
{
	...
	if (d) {
		...
		d->chan1 = chan1m;
		d->chan2 = chan2m;
		if (!cw_pthread_create(&th, &global_attr_detached, iax_park_thread, d))
			return 0;
		free(d);
	}
	return -1;
...
static int socket_read(int *id, int fd, short events, void *cbdata)
{
	...
	if ((vh->zeros == 0) && (ntohs(vh->callno) & 0x8000)) {
		if (res < sizeof(*vh)) {
			cw_log(CW_LOG_WARNING, "Rejecting packet from '%s.%d' that is flagged as a video frame but is too short\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), ntohs(sin.sin_port));
			return 1;
		}
		...
		if (res < sizeof(*meta)) {
			cw_log(CW_LOG_WARNING, "Rejecting packet from '%s.%d' that is flagged as a meta frame but is too short\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), ntohs(sin.sin_port));
			return 1;
		}
		...
		case IAX_META_TRUNK:
			...
			if (!tpeer) {
				cw_log(CW_LOG_WARNING, "Unable to accept trunked packet from '%s:%d': No matching peer\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), ntohs(sin.sin_port));
				return 1;
			}
			tpeer->trunkact = cw_tvnow();
			...
			while(res >= sizeof(*mte)) {
				...
				if( metatype == IAX_META_TRUNK_MINI) {
					...
					cw_log(CW_LOG_WARNING, "Unknown meta trunk cmd from '%s:%d': dropping\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), ntohs(sin.sin_port));
					break;
				}
...
/* duped_fr doesn't exist any more
								...
								cw_log(CW_LOG_WARNING, "Datalen < 0?\n");
							}
						} else {
							cw_log(CW_LOG_WARNING, "Received trunked frame before first full voice frame\n ");
							iax2_vnak(fr.callno);
						}
					}
	...
	if (ntohs(mh->callno) & IAX_FLAG_FULL) {
		if (res < sizeof(*fh)) {
			cw_log(CW_LOG_WARNING, "Rejecting packet from '%s.%d' that is flagged as a full frame but is too short\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), ntohs(sin.sin_port));
			return 1;
		}
	...
	if (cw_test_flag(iaxs[fr.callno], IAX_ENCRYPTED)) {
		if (decrypt_frame(fr.callno, fh, &f, &res)) {
			cw_log(CW_LOG_NOTICE, "Packet Decrypt Failed!\n");
			cw_mutex_unlock(&iaxsl[fr.callno]);
			return 1;
		}
#ifdef DEBUG_SUPPORT
	...
	if (ntohs(mh->callno) & IAX_FLAG_FULL) {
		if (option_debug  && iaxdebug)
			cw_log(CW_LOG_DEBUG, "Received packet %d, (%d, %d)\n", fh->oseqno, f.frametype, f.subclass);
		...
		if (test_resync) {
			if (option_debug)
				cw_log(CW_LOG_DEBUG, "Simulating frame ts resync, was %u now %u\n", fr.ts, fr.ts + test_resync);
			fr.ts += test_resync;
		}
#endif /* IAXTESTS */
#if 0
			...
			if (
			 ((f.subclass != IAX_COMMAND_ACK) &&
			  ...
			  (f.frametype != CW_FRAME_IAX)) {
				...
				if (option_debug)
					cw_log(CW_LOG_DEBUG, "Packet arrived out of order (expecting %d, got %d) (frametype = %d, subclass = %d)\n", 
				...
				if (iaxs[fr.callno]->iseqno > fr.oseqno) {
					...
					if ((f.frametype != CW_FRAME_IAX) || 
						...
						if (option_debug)
							cw_log(CW_LOG_DEBUG, "Acking anyway\n");
						...
						send_command_immediate(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_ACK, fr.ts, NULL, 0,fr.iseqno);
					}
				} else {
		...
		if (!inaddrcmp(&sin, &iaxs[fr.callno]->addr) && 
			...
			if ((x != iaxs[fr.callno]->oseqno) || (iaxs[fr.callno]->oseqno == fr.iseqno)) {
				...
				for (x=iaxs[fr.callno]->rseqno; x != fr.iseqno; x++) {
					...
					if (option_debug && iaxdebug)
						cw_log(CW_LOG_DEBUG, "Cancelling transmission of packet %d\n", x);
					cw_mutex_lock(&iaxq.lock);
					for (cur = iaxq.head; cur ; cur = cur->next) {
						...
						if ((fr.callno == cur->callno) && (x == cur->oseqno)) {
							...
							if (cur->final) { 
								if (iaxdebug && option_debug)
									cw_log(CW_LOG_DEBUG, "Really destroying %d, having been acked on final message\n", fr.callno);
				...
				cw_log(CW_LOG_DEBUG, "Received iseqno %d not within window %d->%d\n", fr.iseqno, iaxs[fr.callno]->rseqno, iaxs[fr.callno]->oseqno);
		}
		if (inaddrcmp(&sin, &iaxs[fr.callno]->addr) && 
			((f.frametype != CW_FRAME_IAX) || 
			 ((f.subclass != IAX_COMMAND_TXACC) &&
		...
		if (f.datalen) {
			if (f.frametype == CW_FRAME_IAX) {
				if (iax_parse_ies(&ies, buf + sizeof(*fh), f.datalen)) {
					cw_log(CW_LOG_WARNING, "Undecodable frame received from '%s'\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr));
					cw_mutex_unlock(&iaxsl[fr.callno]);
					return 1;
				}
				f.data = NULL;
		...
		if (f.frametype == CW_FRAME_VOICE) {
			if (f.subclass != iaxs[fr.callno]->voiceformat) {
					...
					cw_log(CW_LOG_DEBUG, "Ooh, voice format changed to %d\n", f.subclass);
					if (iaxs[fr.callno]->owner) {
						int orignative;
retryowner:
						if (cw_mutex_trylock(&iaxs[fr.callno]->owner->lock)) {
							cw_mutex_unlock(&iaxsl[fr.callno]);
							usleep(1);
							cw_mutex_lock(&iaxsl[fr.callno]);
						...
						if (iaxs[fr.callno]) {
							if (iaxs[fr.callno]->owner) {
								...
								if (iaxs[fr.callno]->owner->readformat)
									cw_set_read_format(iaxs[fr.callno]->owner, iaxs[fr.callno]->owner->readformat);
								iaxs[fr.callno]->owner->nativeformats = orignative;
								cw_mutex_unlock(&iaxs[fr.callno]->owner->lock);
							}
						} else {
		...
		if (f.frametype == CW_FRAME_VIDEO) {
			if (f.subclass != iaxs[fr.callno]->videoformat) {
				cw_log(CW_LOG_DEBUG, "Ooh, video format changed to %d\n", f.subclass & ~0x1);
		...
		if (f.frametype == CW_FRAME_IAX) {
			if (iaxs[fr.callno]->initid > -1) {
				...
				cw_sched_del(sched, iaxs[fr.callno]->initid);
			...
			case IAX_COMMAND_QUELCH:
				if (iaxs[fr.callno]->state & IAX_STATE_STARTED) {
					...
					cw_set_flag(iaxs[fr.callno], IAX_QUELCH);
					if (ies.musiconhold) {
						if (iaxs[fr.callno]->owner &&
							cw_bridged_channel(iaxs[fr.callno]->owner))
								cw_moh_start(cw_bridged_channel(iaxs[fr.callno]->owner), NULL);
					}
				}
				break;
			case IAX_COMMAND_UNQUELCH:			 
				if (iaxs[fr.callno]->state & IAX_STATE_STARTED) {
					...
					if (iaxs[fr.callno]->owner && cw_test_flag(iaxs[fr.callno], IAX_QUELCH)) {
					...
					cw_clear_flag(iaxs[fr.callno], IAX_QUELCH);
					if (iaxs[fr.callno]->owner &&
						cw_bridged_channel(iaxs[fr.callno]->owner))
							cw_moh_stop(cw_bridged_channel(iaxs[fr.callno]->owner));
				}
				break;
			case IAX_COMMAND_TXACC:
				if (iaxs[fr.callno]->transferring == TRANSFER_BEGIN) {
					...
					send_command(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_TXREADY, 0, ied1.buf, ied1.pos, -1);
			...
			case IAX_COMMAND_NEW:
				...
				if (check_access(fr.callno, &sin, &ies)) {
					...
					if (authdebug)
						cw_log(CW_LOG_NOTICE, "Rejected connect attempt from %s, who was trying to reach '%s@%s'\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), iaxs[fr.callno]->exten, iaxs[fr.callno]->context);
					break;
				}
				...
				if (strcasecmp(iaxs[fr.callno]->exten, "TBD")) {
					cw_mutex_unlock(&iaxsl[fr.callno]);
					exists = cw_exists_extension(NULL, iaxs[fr.callno]->context, iaxs[fr.callno]->exten, 1, iaxs[fr.callno]->cid_num);
					cw_mutex_lock(&iaxsl[fr.callno]);
				} else
					exists = 0;
				if (cw_strlen_zero(iaxs[fr.callno]->secret) && cw_strlen_zero(iaxs[fr.callno]->inkeys)) {
					if (strcmp(iaxs[fr.callno]->exten, "TBD") && !exists) {
						...
						send_command_final(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_REJECT, 0, ied0.buf, ied0.pos, -1);
						if (authdebug)
							cw_log(CW_LOG_NOTICE, "Rejected connect attempt from %s, request '%s@%s' does not exist\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), iaxs[fr.callno]->exten, iaxs[fr.callno]->context);
					} else {
						/* Select an appropriate format */

						if(cw_test_flag(iaxs[fr.callno], IAX_CODEC_NOPREFS)) {
							if(cw_test_flag(iaxs[fr.callno], IAX_CODEC_NOCAP)) {
								using_prefs = "reqonly";
							} else {
							...
							if(ies.codec_prefs) {
								...
								if (cw_test_flag(iaxs[fr.callno], IAX_CODEC_USER_FIRST)) {
									pref = rpref;
									using_prefs = "caller";
								} else {
							...
							format = cw_codec_choose(&pref, iaxs[fr.callno]->capability & iaxs[fr.callno]->peercapability, 0);
							cw_codec_pref_string(&rpref, caller_pref_buf, sizeof(caller_pref_buf) - 1);
							cw_codec_pref_string(&iaxs[fr.callno]->prefs, host_pref_buf, sizeof(host_pref_buf) - 1);
						}
						if (!format) {
							if(!cw_test_flag(iaxs[fr.callno], IAX_CODEC_NOCAP))
							...
							if (!format) {
								...
								if (authdebug) {
									if(cw_test_flag(iaxs[fr.callno], IAX_CODEC_NOCAP))
										cw_log(CW_LOG_NOTICE, "Rejected connect attempt from %s, requested 0x%x incompatible with our capability 0x%x.\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), iaxs[fr.callno]->peerformat, iaxs[fr.callno]->capability);
									else 
										cw_log(CW_LOG_NOTICE, "Rejected connect attempt from %s, requested/capability 0x%x/0x%x incompatible with our capability 0x%x.\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), iaxs[fr.callno]->peerformat, iaxs[fr.callno]->peercapability, iaxs[fr.callno]->capability);
								}
							} else {
								/* Pick one... */
								if(cw_test_flag(iaxs[fr.callno], IAX_CODEC_NOCAP)) {
									...
									if(cw_test_flag(iaxs[fr.callno], IAX_CODEC_NOPREFS)) {
										using_prefs = cw_test_flag(iaxs[fr.callno], IAX_CODEC_NOCAP) ? "reqonly" : "disabled";
										memset(&pref, 0, sizeof(pref));
										format = cw_best_codec(iaxs[fr.callno]->peercapability & iaxs[fr.callno]->capability);
										strcpy(caller_pref_buf,"disabled");
										strcpy(host_pref_buf,"disabled");
									} else {
										using_prefs = "mine";
										if(ies.codec_prefs) {
											...
											format = cw_codec_choose(&pref, iaxs[fr.callno]->peercapability & iaxs[fr.callno]->capability, 1);
								...
								if (!format) {
									...
									iax_ie_append_str(&ied0, IAX_IE_CAUSE, "Unable to negotiate codec");
									iax_ie_append_byte(&ied0, IAX_IE_CAUSECODE, CW_CAUSE_BEARERCAPABILITY_NOTAVAIL);
									cw_log(CW_LOG_ERROR, "No best format in 0x%x???\n", iaxs[fr.callno]->peercapability & iaxs[fr.callno]->capability);
									send_command_final(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_REJECT, 0, ied0.buf, ied0.pos, -1);
									if (authdebug)
										cw_log(CW_LOG_NOTICE, "Rejected connect attempt from %s, requested/capability 0x%x/0x%x incompatible with our capability 0x%x.\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), iaxs[fr.callno]->peerformat, iaxs[fr.callno]->peercapability, iaxs[fr.callno]->capability);
									cw_set_flag(iaxs[fr.callno], IAX_ALREADYGONE);	
									break;
								}
							}
						}
						if (format) {
							...
							send_command(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_ACCEPT, 0, ied1.buf, ied1.pos, -1);
							if (strcmp(iaxs[fr.callno]->exten, "TBD")) {
								...
								if (option_verbose > 2) 
									cw_verbose(VERBOSE_PREFIX_3 "Accepting UNAUTHENTICATED call from %s:\n"
												...
												cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), 
												VERBOSE_PREFIX_4,
												cw_getformatname(iaxs[fr.callno]->peerformat), 
												VERBOSE_PREFIX_4,
												caller_pref_buf,
												VERBOSE_PREFIX_4,
								...
								if(!(c = cw_iax2_new(fr.callno, CW_STATE_RING, format)))
									iax2_destroy_nolock(fr.callno);
							} else {
			...
			case IAX_COMMAND_HANGUP:
				cw_set_flag(iaxs[fr.callno], IAX_ALREADYGONE);
				cw_log(CW_LOG_DEBUG, "Immediately destroying %d, having received hangup\n", fr.callno);
			...
			case IAX_COMMAND_REJECT:
				...
				if (iaxs[fr.callno]) {
					if (cw_test_flag(iaxs[fr.callno], IAX_PROVISION)) {
						/* Send ack immediately, before we destroy */
						send_command_immediate(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_ACK, fr.ts, NULL, 0,fr.iseqno);
						iax2_destroy_nolock(fr.callno);
						break;
					...
					if (iaxs[fr.callno]->owner) {
						if (authdebug)
							cw_log(CW_LOG_WARNING, "Call rejected by %s: %s\n", cw_inet_ntoa(iabuf, sizeof(iabuf), iaxs[fr.callno]->addr.sin_addr), ies.cause ? ies.cause : "<Unknown>");
					}
					cw_log(CW_LOG_DEBUG, "Immediately destroying %d, having received reject\n", fr.callno);
					/* Send ack immediately, before we destroy */
					send_command_immediate(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_ACK, fr.ts, NULL, 0,fr.iseqno);
			...
			case IAX_COMMAND_TRANSFER:
				if (iaxs[fr.callno]->owner && cw_bridged_channel(iaxs[fr.callno]->owner) && ies.called_number) {
					if (!strcmp(ies.called_number, cw_parking_ext())) {
						if (iax_park(cw_bridged_channel(iaxs[fr.callno]->owner), iaxs[fr.callno]->owner)) {
			...
			case IAX_COMMAND_ACCEPT:
				...
				if (option_verbose > 2)
					cw_verbose(VERBOSE_PREFIX_3 "Call accepted by %s (format %s)\n", cw_inet_ntoa(iabuf, sizeof(iabuf), iaxs[fr.callno]->addr.sin_addr), cw_getformatname(iaxs[fr.callno]->peerformat));
				if (!(iaxs[fr.callno]->peerformat & iaxs[fr.callno]->capability)) {
					memset(&ied0, 0, sizeof(ied0));
					iax_ie_append_str(&ied0, IAX_IE_CAUSE, "Unable to negotiate codec");
					iax_ie_append_byte(&ied0, IAX_IE_CAUSECODE, CW_CAUSE_BEARERCAPABILITY_NOTAVAIL);
					send_command_final(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_REJECT, 0, ied0.buf, ied0.pos, -1);
					if (authdebug)
						cw_log(CW_LOG_NOTICE, "Rejected call to %s, format 0x%x incompatible with our capability 0x%x.\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), iaxs[fr.callno]->peerformat, iaxs[fr.callno]->capability);
				} else {
					iaxs[fr.callno]->state |= IAX_STATE_STARTED;
					if (iaxs[fr.callno]->owner) {
						...
						if (option_verbose > 2)
							cw_verbose(VERBOSE_PREFIX_3 "Format for call is %s\n", cw_getformatname(iaxs[fr.callno]->owner->nativeformats));
retryowner2:
						if (cw_mutex_trylock(&iaxs[fr.callno]->owner->lock)) {
							cw_mutex_unlock(&iaxsl[fr.callno]);
							usleep(1);
						...
						if (iaxs[fr.callno] && iaxs[fr.callno]->owner) {
							...
							if (iaxs[fr.callno]->owner->writeformat)
								cw_set_write_format(iaxs[fr.callno]->owner, iaxs[fr.callno]->owner->writeformat);	
							if (iaxs[fr.callno]->owner->readformat)
								cw_set_read_format(iaxs[fr.callno]->owner, iaxs[fr.callno]->owner->readformat);	
							cw_mutex_unlock(&iaxs[fr.callno]->owner->lock);
						}
			...
			case IAX_COMMAND_POKE:
				...
				send_command_final(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_PONG, fr.ts, NULL, 0, -1);
				break;
			case IAX_COMMAND_PING:
#ifdef BRIDGE_OPTIMIZATION
				if (iaxs[fr.callno]->bridgecallno) {
					/* If we're in a bridged call, just forward this */
					forward_command(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_PING, fr.ts, NULL, 0, -1);
				} else {
					struct iax_ie_data pingied;
					construct_rr(iaxs[fr.callno], &pingied);
					/* Send back a pong packet with the original timestamp */
					send_command(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_PONG, fr.ts, pingied.buf, pingied.pos, -1);
				}
#else				
				{
					struct iax_ie_data pingied;
			...
			case IAX_COMMAND_PONG:
				...
				if (iaxs[fr.callno]->bridgecallno) {
					...
					forward_command(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_PONG, fr.ts, NULL, 0, -1);
				} else {
					/* Calculate ping time */
				 ...
				 */
				if ((peer = iaxs[fr.callno]->peerpoke) && !cw_sched_del(sched, peer->pokeexpire)) {
				...
				if (iaxs[fr.callno]->bridgecallno) {
					...
					if(f.subclass == IAX_COMMAND_LAGRQ) {
						...
						cw_log(CW_LOG_DEBUG, "Peer %s lag measured as %dms\n",
								cw_inet_ntoa(iabuf, sizeof(iabuf), iaxs[fr.callno]->addr.sin_addr), iaxs[fr.callno]->lag);
					}
#ifdef BRIDGE_OPTIMIZATION
				}
#endif				
			...
			case IAX_COMMAND_AUTHREQ:
				if (iaxs[fr.callno]->state & (IAX_STATE_STARTED | IAX_STATE_TBD)) {
					cw_log(CW_LOG_WARNING, "Call on %s is already up, can't start on it\n", iaxs[fr.callno]->owner ? iaxs[fr.callno]->owner->name : "<Unknown>");
					break;
				}
				if (authenticate_reply(iaxs[fr.callno], &iaxs[fr.callno]->addr, &ies, iaxs[fr.callno]->secret, iaxs[fr.callno]->outkey)) {
					cw_log(CW_LOG_WARNING, 
						...
						ies.username ? ies.username : "<unknown>", cw_inet_ntoa(iabuf, sizeof(iabuf), iaxs[fr.callno]->addr.sin_addr));
				}
				break;
			case IAX_COMMAND_AUTHREP:
				...
				if (authenticate_verify(iaxs[fr.callno], &ies)) {
					if (authdebug)
						cw_log(CW_LOG_NOTICE, "Host %s failed to authenticate as %s\n", cw_inet_ntoa(iabuf, sizeof(iabuf), iaxs[fr.callno]->addr.sin_addr), iaxs[fr.callno]->username);
					memset(&ied0, 0, sizeof(ied0));
					auth_fail(fr.callno, IAX_COMMAND_REJECT);
					break;
				}
				...
				if (strcmp(iaxs[fr.callno]->exten, "TBD") && !exists) {
					...
					if (!format) {
						if(!cw_test_flag(iaxs[fr.callno], IAX_CODEC_NOCAP)) {
							cw_log(CW_LOG_DEBUG, "We don't do requested format %s, falling back to peer capability %d\n", cw_getformatname(iaxs[fr.callno]->peerformat), iaxs[fr.callno]->peercapability);
							format = iaxs[fr.callno]->peercapability & iaxs[fr.callno]->capability;
						}
						if (!format) {
							if (authdebug) {
							...
							if(cw_test_flag(iaxs[fr.callno], IAX_CODEC_NOCAP)) {
								...
								if(cw_test_flag(iaxs[fr.callno], IAX_CODEC_NOPREFS)) {
									using_prefs = cw_test_flag(iaxs[fr.callno], IAX_CODEC_NOCAP) ? "reqonly" : "disabled";
									memset(&pref, 0, sizeof(pref));
									format = cw_test_flag(iaxs[fr.callno], IAX_CODEC_NOCAP) ?
										iaxs[fr.callno]->peerformat : cw_best_codec(iaxs[fr.callno]->peercapability & iaxs[fr.callno]->capability);
									strcpy(caller_pref_buf,"disabled");
									strcpy(host_pref_buf,"disabled");
								} else {
									using_prefs = "mine";
			...
			case IAX_COMMAND_DIAL:
				if (iaxs[fr.callno]->state & IAX_STATE_TBD) {
					iaxs[fr.callno]->state &= ~IAX_STATE_TBD;
					cw_copy_string(iaxs[fr.callno]->exten, ies.called_number ? ies.called_number : "s", sizeof(iaxs[fr.callno]->exten));	
					if (!cw_exists_extension(NULL, iaxs[fr.callno]->context, iaxs[fr.callno]->exten, 1, iaxs[fr.callno]->cid_num)) {
						if (authdebug)
							cw_log(CW_LOG_NOTICE, "Rejected dial attempt from %s, request '%s@%s' does not exist\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), iaxs[fr.callno]->exten, iaxs[fr.callno]->context);
						memset(&ied0, 0, sizeof(ied0));
						iax_ie_append_str(&ied0, IAX_IE_CAUSE, "No such context/extension");
						iax_ie_append_byte(&ied0, IAX_IE_CAUSECODE, CW_CAUSE_NO_ROUTE_DESTINATION);
						send_command_final(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_REJECT, 0, ied0.buf, ied0.pos, -1);
						...
						iaxs[fr.callno]->state |= IAX_STATE_STARTED;
						if (option_verbose > 2) 
							cw_verbose(VERBOSE_PREFIX_3 "Accepting DIAL from %s, formats = 0x%x\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), iaxs[fr.callno]->peerformat);
						iaxs[fr.callno]->state |= IAX_STATE_STARTED;
						send_command(iaxs[fr.callno], CW_FRAME_CONTROL, CW_CONTROL_PROGRESS, 0, NULL, 0, -1);
						if(!(c = cw_iax2_new(fr.callno, CW_STATE_RING, iaxs[fr.callno]->peerformat)))
							iax2_destroy_nolock(fr.callno);
					}
				}
				break;
			case IAX_COMMAND_INVAL:
				...
				cw_log(CW_LOG_DEBUG, "Immediately destroying %d, having received INVAL\n", fr.callno);
				iax2_destroy_nolock(fr.callno);
				if (option_debug)
					cw_log(CW_LOG_DEBUG, "Destroying call %d\n", fr.callno);
				break;
			case IAX_COMMAND_VNAK:
				cw_log(CW_LOG_DEBUG, "Received VNAK: resending outstanding frames\n");
			...
			case IAX_COMMAND_REGREL:
				...
				if ((cw_strlen_zero(iaxs[fr.callno]->secret) && cw_strlen_zero(iaxs[fr.callno]->inkeys)) || (iaxs[fr.callno]->state & IAX_STATE_AUTHENTICATED)) {
					if (f.subclass == IAX_COMMAND_REGREL)
						memset(&sin, 0, sizeof(sin));
					if (update_registry(iaxs[fr.callno]->peer, &sin, fr.callno, ies.devicetype, fd, ies.refresh))
						cw_log(CW_LOG_WARNING, "Registry error\n");
					break;
				}
			...
			case IAX_COMMAND_REGACK:
				if (iax2_ack_registry(&ies, &sin, fr.callno)) 
					cw_log(CW_LOG_WARNING, "Registration failure\n");
				/* Send ack immediately, before we destroy */
				send_command_immediate(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_ACK, fr.ts, NULL, 0,fr.iseqno);
				iax2_destroy_nolock(fr.callno);
				break;
			case IAX_COMMAND_REGREJ:
				if (iaxs[fr.callno]->reg) {
					if (authdebug) {
						cw_log(CW_LOG_NOTICE, "Registration of '%s' rejected: '%s' from: '%s'\n", iaxs[fr.callno]->reg->username, ies.cause ? ies.cause : "<unknown>", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr));
			...
			case IAX_COMMAND_TXREJ:
				...
				if (iaxs[fr.callno]->bridgecallno) {
					if (iaxs[iaxs[fr.callno]->bridgecallno]->transferring) {
						...
						send_command(iaxs[iaxs[fr.callno]->bridgecallno], CW_FRAME_IAX, IAX_COMMAND_TXREJ, 0, NULL, 0, -1);
					}
				}
				break;
				...
				if (iaxs[fr.callno]->transferring == TRANSFER_BEGIN) {
					...
					if (iaxs[fr.callno]->bridgecallno) {
						if (iaxs[iaxs[fr.callno]->bridgecallno]->transferring == TRANSFER_READY) {
							...
							cw_set_flag(iaxs[iaxs[fr.callno]->bridgecallno], IAX_ALREADYGONE);
							cw_set_flag(iaxs[fr.callno], IAX_ALREADYGONE);
							...
							send_command(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_TXREL, 0, ied0.buf, ied0.pos, -1);
							send_command(iaxs[iaxs[fr.callno]->bridgecallno], CW_FRAME_IAX, IAX_COMMAND_TXREL, 0, ied1.buf, ied1.pos, -1);
			...
			case IAX_COMMAND_TXCNT:
				if (iaxs[fr.callno]->transferring)
					send_command_transfer(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_TXACC, 0, NULL, 0);
				break;
			...
			case IAX_COMMAND_UNSUPPORT:
				cw_log(CW_LOG_NOTICE, "Peer did not understand our iax command '%d'\n", ies.iax_unknown);
				break;
			case IAX_COMMAND_FWDOWNL:
				/* Firmware download */
				cw_log(CW_LOG_DEBUG, "Rejecting firmware download request.");
				memset(&ied0, 0, sizeof(ied0));
				send_command_final(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_REJECT, 0, ied0.buf, ied0.pos, -1);
				break;
			default:
				cw_log(CW_LOG_DEBUG, "Unknown IAX command %d on %d/%d\n", f.subclass, fr.callno, iaxs[fr.callno]->peercallno);
				memset(&ied0, 0, sizeof(ied0));
				iax_ie_append_byte(&ied0, IAX_IE_IAX_UNKNOWN, f.subclass);
				send_command(iaxs[fr.callno], CW_FRAME_IAX, IAX_COMMAND_UNSUPPORT, 0, ied0.buf, ied0.pos, -1);
			}
		...
		if (iaxs[fr.callno]->videoformat > 0) 
			...
			cw_log(CW_LOG_WARNING, "Received mini frame before first full video frame\n ");
			iax2_vnak(fr.callno);
			cw_mutex_unlock(&iaxsl[fr.callno]);
			return 1;
		}
		...
		if (iaxs[fr.callno]->voiceformat > 0)
			...
			cw_log(CW_LOG_WARNING, "Received mini frame before first full voice frame\n ");
			iax2_vnak(fr.callno);
			cw_mutex_unlock(&iaxsl[fr.callno]);
			return 1;
		}
	...
	if (reg->addr.sin_addr.s_addr) {
		if (!reg->callno) {
			...
			if (reg->callno >= 0) {
				...
				cw_log(CW_LOG_WARNING, "Unable to create call for registration\n");
			}
		}
...
static void *iax2_poke_peer_thread(void *data)
{
	...
	if (peer->maxms && peer->addr.sin_addr.s_addr) {
		if (peer->callno > 0) {
			cw_log(CW_LOG_NOTICE, "Still have a callno...\n");
			iax2_destroy(peer->callno);
		}
...
static int iax2_poke_peer(void *data)
{
	...
	if (cw_test_flag(peer, IAX_DYNAMIC))
		...
		cw_pthread_create(&tid, &global_attr_detached, iax2_poke_peer_thread, data);
...
static struct cw_channel *iax2_request(const char *type, int format, void *data, int *cause)
{
	...
	if (!pds.peer) {
		cw_log(CW_LOG_WARNING, "No peer given\n");
		return NULL;
	}
	...
	if (c) {
		...
		if (c->nativeformats & format) 
			...
			if (res < 0) {
				cw_log(CW_LOG_WARNING, "Unable to create translator path for %s to %s on %s\n",
...
static int check_srcaddr(struct sockaddr *sa, socklen_t salen)
{
	...
	if (res < 0) {
		cw_log(CW_LOG_DEBUG, "Can't bind: %s\n", strerror(errno));
		close(sd);
		return 1;
	}
...
static int peer_set_srcaddr(struct iax2_peer *peer, const char *srcaddr)
{
	...
	if (nonlocal) {
		cw_log(CW_LOG_WARNING, "Non-local or unbound address specified (%s) in sourceaddress for '%s', reverting to default\n",
			srcaddr, peer->name);
		return -1;
	} else {
		cw_log(CW_LOG_DEBUG, "Using sourceaddress %s for '%s'\n", srcaddr, peer->name);
		return 0;
	}
}
			...
			if (!strcasecmp(v->name, "secret")) {
				...
				*/
				...
				if (cw_test_flag(peer, IAX_TRUNK)) {
					cw_log(CW_LOG_WARNING, "Unable to support trunking on peer '%s'Trunking support is not compiled!\n", peer->name);
				...
				if (!strcasecmp(v->value, "no")) {
					...
					cw_log(CW_LOG_WARNING, "Qualification of peer '%s' should be 'yes', 'no', or a number of milliseconds at line %d of iax.conf\n", peer->name, v->lineno);
					peer->maxms = 0;
				}
				...
				if (sscanf(v->value, "%d", &peer->pokefreqok) != 1) {
					cw_log(CW_LOG_WARNING, "Qualification testing frequency of peer '%s' when OK should a number of milliseconds at line %d of iax.conf\n", peer->name, v->lineno);
				}
			} else if (!strcasecmp(v->name, "qualifyfreqnotok")) {
				if (sscanf(v->value, "%d", &peer->pokefreqnotok) != 1) {
					cw_log(CW_LOG_WARNING, "Qualification testing frequency of peer '%s' when NOT OK should be a number of milliseconds at line %d of iax.conf\n", peer->name, v->lineno);
				} else cw_log(CW_LOG_WARNING, "Set peer->pokefreqnotok to %d\n", peer->pokefreqnotok);
		...
		while(v) {
			if (!strcasecmp(v->name, "context")) {
				...
				if (cw_test_flag(user, IAX_TRUNK)) {
					cw_log(CW_LOG_WARNING, "Unable to support trunking on user '%s': Trunking support is not compiled!\n", user->name);
...
static int set_config(char *config_file, int reload)
{
	...
	if (tosval) {
		if (cw_str2tos(tosval, &tos))
			cw_log(CW_LOG_WARNING, "Invalid tos value, should be 'lowdelay', 'throughput', 'reliability', 'mincost', or 'none'\n");
	}
	while(v) {
		/* handle jb conf */
		...
		if (!strcasecmp(v->name, "bindport")){ 
			if (reload)
				cw_log(CW_LOG_NOTICE, "Ignoring bindport on reload\n");
			else
			...
			if (reload) {
				cw_log(CW_LOG_NOTICE, "Ignoring bindaddr on reload\n");
			} else {
				if (!(ns = cw_netsock_bind(netsock, io, v->value, portno, tos, socket_read, NULL))) {
					cw_log(CW_LOG_WARNING, "Unable apply binding to '%s' at line %d\n", v->value, v->lineno);
				} else {
					if (option_verbose > 1) {
						if (strchr(v->value, ':'))
			...
			if (!strcasecmp(v->value, "low")) {
				...
				cw_log(CW_LOG_WARNING, "bandwidth must be either low, medium, or high\n");
		} else if (!strcasecmp(v->name, "allow")) {
	...
	if (min_reg_expire > max_reg_expire) {
		cw_log(CW_LOG_WARNING, "Minimum registration interval of %d is more than maximum of %d, resetting minimum to %d\n",
...
static struct iax2_dpcache *find_cache(struct cw_channel *chan, const char *data, const char *context, const char *exten, int priority)
{
	...
	while(dp) {
		...
		if (cw_tvcmp(tv, dp->expiry) > 0) {
				...
				if (!dp->peer && !(dp->flags & CACHE_FLAG_PENDING) && !dp->callno) {
					...
					cw_log(CW_LOG_WARNING, "DP still has peer field or pending or callno (flags = %d, peer = %p callno = %d)\n", dp->flags, dp->peer, dp->callno);
				}
				dp = next;
				continue;
		}
	...
	if (dp->flags & CACHE_FLAG_PENDING) {
		...
		if (x >= sizeof(dp->waiters) / sizeof(dp->waiters[0])) {
			cw_log(CW_LOG_WARNING, "No more waiter positions available\n");
			return NULL;
		}
		if (pipe(com)) {
			cw_log(CW_LOG_WARNING, "Unable to create pipe for comm\n");
			return NULL;
		}
		dp->waiters[x] = com[1];
		/* Okay, now we wait */
		...
		if (!timeout) {
			cw_log(CW_LOG_WARNING, "Timeout waiting for %s exten %s\n", data, exten);
		}
		cw_mutex_lock(&dpcache_lock);
		dp->waiters[x] = -1;
		close(com[1]);
...
static int iax2_exists(struct cw_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	...
	cw_mutex_unlock(&dpcache_lock);
	if (!dp) {
		cw_log(CW_LOG_WARNING, "Unable to make DP cache\n");
	}
	return res;
}
...
static int iax2_exec(struct cw_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	...
	if (dp) {
		if (dp->flags & CACHE_FLAG_EXISTS) {
			...
			cw_mutex_unlock(&dpcache_lock);
			cw_log(CW_LOG_WARNING, "Can't execute nonexistent extension '%s[@%s]' in data '%s'\n", exten, context, data);
			return -1;
		}
	}
	cw_mutex_unlock(&dpcache_lock);
...
static int iax2_devicestate(void *data) 
{
	...
	if (option_debug > 2)
		cw_log(CW_LOG_DEBUG, "Checking device state for device %s\n", dest);
	...
	if (p) {
		...
		if (option_debug > 2) 
			cw_log(CW_LOG_DEBUG, "Devicestate: Can't find peer %s.\n", host);
	}
...
static struct cw_clicmd iax2_cli[] = {
	{
...
static int load_module(void)
{
	...
	if (!netsock) {
		cw_log(CW_LOG_ERROR, "Could not allocate netsock list.\n");
		return -1;
	}
