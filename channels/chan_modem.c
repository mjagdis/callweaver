...
OPENPBX_FILE_VERSION(__FILE__, "$Revision: 1.46 $")
...
/* Default DTMF-detection mode (i4l/openpbx) */
static int dtmfmode = MODEM_DTMF_AST;
/* Default DTMF-generation mode (i4l (outband) / openpbx (inband) */
...
int load_module()
{
	...
	while(v) {
		...
		if (!strcasecmp(v->name, "device")) {
			...
			else if (!strcasecmp(tmp, "openpbx"))
				dtmfmode = MODEM_DTMF_AST;
			else if (!strcasecmp(tmp, "i4l"))
				dtmfmode = MODEM_DTMF_I4L;
			else {
				ast_log(LOG_WARNING, "Unknown dtmf detection mode '%s', using 'openpbx'\n", v->value);
				dtmfmode = MODEM_DTMF_AST;
			}
			if (alt) {
				if (!strcasecmp(alt, "none"))
					dtmfmodegen=MODEM_DTMF_NONE;
				else if (!strcasecmp(alt, "openpbx"))
					...
					ast_log(LOG_WARNING, "Unknown dtmf generation mode '%s', using 'openpbx'\n", v->value);
					dtmfmodegen = MODEM_DTMF_AST;
				}
			} else
