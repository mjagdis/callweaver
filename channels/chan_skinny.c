...
OPENPBX_FILE_VERSION(__FILE__, "$Revision: 1.91 $")
...
/************************************************************************************/
/*                         Skinny/OpenPBX Protocol Settings                        */
/************************************************************************************/
...
{
	return 4; /* ulaw (this is not the same as openpbx's '4'  */
...
static int handle_message(skinny_req *req, struct skinnysession *s)
{
	...
	case SPEED_DIAL_STAT_REQ_MESSAGE:
		...
		snprintf(req->data.speeddial.speedDialDisplayName,  sizeof(req->data.speeddial.speedDialDisplayName),"OpenPBX Rules!");
#endif	
		transmit_response(s, req);
		break;
...
int load_module()
{
	...
	/* Announce our presence to OpenPBX */	
	if (!res) {
