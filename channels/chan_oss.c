...
OPENPBX_FILE_VERSION(__FILE__, "$Revision: 1.56 $")
 ...
 *	}
 *
 * XXX NOTE these macros should NOT be replicated in other parts of openpbx. 
	...
	if (setformat(o, O_RDWR) < 0) {	/* open device */
		if (option_verbose > 0) {
			...
			ast_verbose(VERBOSE_PREFIX_2 "Turn off OSS support by adding "
			    "'noload=chan_oss.so' in /etc/openpbx/modules.conf\n");
		}
		goto error;
	}
	if (o->duplex != M_FULL)
