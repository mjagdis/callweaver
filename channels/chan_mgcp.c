...
static int mgcp_reload(cw_dynstr_t *ds_p, int argc, char *argv[]);
...
static int unalloc_sub(struct mgcp_subchannel *sub)
{
	...
	if (p->sub == sub) {
		cw_log(CW_LOG_WARNING, "Trying to unalloc the real channel %s@%s?!?\n", p->name, p->parent->name);
		return -1;
	}
	cw_log(CW_LOG_DEBUG, "Released sub %d of channel %s@%s\n", sub->id, p->name, p->parent->name);
...
static int __mgcp_xmit(struct mgcp_gateway *gw, char *data, int len)
{
	...
	if (res != len) {
		cw_log(CW_LOG_WARNING, "mgcp_xmit returned %d: %s\n", res, strerror(errno));
	}
	return res;
}
...
static void dump_queue(struct mgcp_gateway *gw, struct mgcp_endpoint *p)
{
	...
	while (cur) {
		if (!p || cur->owner_ep == p) {
			...
			cw_log(CW_LOG_NOTICE, "Removing message from %s transaction %u\n", 
				gw->name, cur->seqno);
...
static int retrans_pkt(void *data)
{
	...
	while (cur) {
		if (cur->retrans < MAX_RETRANS) {
			...
			cw_log(CW_LOG_WARNING, "Maximum retries exceeded for transaction %u on [%s]\n",
				cur->seqno, gw->name);
...
/* SC
	...
	if (gw->messagepending && (gw->lastouttime + 20 < t)) {
		cw_log(CW_LOG_NOTICE, "Timeout waiting for response to message:%d,  lastouttime: %ld, now: %ld.  Dumping pending queue\n",
...
*/
	...
	if (gettimeofday(&tv, NULL) < 0) {
		...
		cw_log(CW_LOG_NOTICE, "gettimeofday() failed!\n");
	} else {
...
/* SC
		...
		cw_log(CW_LOG_DEBUG, "Deferring transmission of transaction %d\n", seqno);
*/
	return 0;
}
                        ...
                        struct mgcp_request *req, unsigned int seqno)
{
	...
	cw_log(CW_LOG_DEBUG, "Slow sequence is %d\n", p->slowsequence);
	if (p->slowsequence) {
		queue = &p->cmd_queue;
		l = &p->cmd_queue_lock;
		cw_mutex_lock(l);
	...
	if (!r) {
		cw_log(CW_LOG_WARNING, "Cannot post MGCP request: insufficient memory\n");
		cw_mutex_unlock(l);
		return -1;
	}
...
static int mgcp_call(struct cw_channel *ast, const char *dest)
{
	...
	if ((ast->_state != CW_STATE_DOWN) && (ast->_state != CW_STATE_RESERVED)) {
		cw_log(CW_LOG_WARNING, "mgcp_call called on %s, neither down nor reserved\n", ast->name);
		cw_mutex_unlock(&sub->lock);
		return -1;
	}
	...
	if (p->type == TYPE_LINE) {
		...
		cw_log(CW_LOG_NOTICE, "Don't know how to dial on trunks yet\n");
		res = -1;
	}
	cw_mutex_unlock(&sub->lock);
...
static int mgcp_hangup(struct cw_channel *ast)
{
	...
	if (strcmp(sub->magic, MGCP_SUBCHANNEL_MAGIC)) {
		cw_log(CW_LOG_DEBUG, "Invalid magic. MGCP subchannel freed up already.\n");
		return 0;
	}
	cw_mutex_lock(&sub->lock);
	if (mgcpdebug) {
...
static int mgcp_show_endpoints(cw_dynstr_t *ds_p, int argc, char *argv[])
{
	char iabuf[INET_ADDRSTRLEN];
...
static struct cw_clicmd  cli_show_endpoints = {
...
static int mgcp_audit_endpoint(cw_dynstr_t *ds_p, int argc, char *argv[])
{
	struct mgcp_gateway  *g;
	struct mgcp_endpoint *e;
	int found = 0;
...
static struct cw_clicmd  cli_audit_endpoint = {
...
static int mgcp_answer(struct cw_channel *ast)
{
	...
	if (ast->_state != CW_STATE_UP) {
		...
		if (option_debug)
			cw_log(CW_LOG_DEBUG, "mgcp_answer(%s)\n", ast->name);
		transmit_notify_request(sub, "");
		transmit_modify_request(sub);
	}
	cw_mutex_unlock(&sub->lock);
...
#if 0
				cw_log(CW_LOG_NOTICE, "MGCP cw_dsp_process\n");
#endif
...
static int mgcp_fixup(struct cw_channel *oldchan, struct cw_channel *newchan)
{
	...
	cw_mutex_lock(&sub->lock);
	cw_log(CW_LOG_NOTICE, "mgcp_fixup(%s, %s)\n", oldchan->name, newchan->name);
...
static struct mgcp_subchannel *find_subchannel_and_lock(char *name, int msgid, struct sockaddr_in *sin)
{
	...
	if (name) {
		...
		at = strchr(tmp, '@');
		if (!at) {
			cw_log(CW_LOG_NOTICE, "Endpoint '%s' has no at sign!\n", name);
			return NULL;
		}
		*at = '\0';
		at++;
	...
	while(g) {
		if ((!name || !strcasecmp(g->name, at)) && 
			...
			while(p) {
				if (option_debug)
					cw_log(CW_LOG_DEBUG, "Searching on %s@%s for subchannel\n",
						p->name, g->name);
				if (msgid) {
					...
					do {
						if (option_debug)
							cw_log(CW_LOG_DEBUG, "Searching on %s@%s-%d for subchannel with lastout: %d\n",
								p->name, g->name, sub->id, msgid);
						if (sub->lastout == msgid) {
							if (option_debug)
								cw_log(CW_LOG_DEBUG, "Found subchannel sub%d to handle request %d sub->lastout: %d\n",
					...
					cw_log(CW_LOG_DEBUG, "Coundn't determine subchannel, assuming current master %s@%s-%d\n", 
	...
	if (!sub) {
		if (name) {
			if (g)
				cw_log(CW_LOG_NOTICE, "Endpoint '%s' not found on gateway '%s'\n", tmp, at);
			else
				cw_log(CW_LOG_NOTICE, "Gateway '%s' (and thus its endpoint '%s') does not exist\n", at, tmp);
		} 
	}
	return sub;
}
...
static void parse(struct mgcp_request *req)
{
		...
		if (*c == '\n') {
			...
			*c = 0;
			...
			if (f >= MGCP_MAX_HEADERS - 1) {
				cw_log(CW_LOG_WARNING, "Too many MGCP headers...\n");
			} else
				f++;
			req->header[f] = c + 1;
		} else if (*c == '\r') {
		...
		if (*c == '\n') {
			...
			*c = 0;
			...
			if (f >= MGCP_MAX_LINES - 1) {
				cw_log(CW_LOG_WARNING, "Too many SDP lines...\n");
			} else
				f++;
			req->line[f] = c + 1;
		} else if (*c == '\r') {
	...
	if (*c) 
		cw_log(CW_LOG_WARNING, "Odd content, extra stuff left over ('%s')\n", c);
}
...
static int process_sdp(struct mgcp_subchannel *sub, struct mgcp_request *req)
{
	...
	if (sscanf(m, "audio %d RTP/AVP %n", &portno, &len) != 1) {
		cw_log(CW_LOG_WARNING, "Unable to determine port number for RTP in '%s'\n", m); 
		return -1;
	}
	...
	while (!cw_strlen_zero(codecs)) {
		if (sscanf(codecs, "%d%n", &codec, &len) != 1) {
			if (codec_count)
				break;
			cw_log(CW_LOG_WARNING, "Error in codec string '%s' at '%s'\n", m, codecs);
			return -1;
		}
	...
	if (!p->capability) {
		cw_log(CW_LOG_WARNING, "No compatible codecs!\n");
		return -1;
	}
	return 0;
}
...
static int add_header(struct mgcp_request *req, const char *var, const char *value)
{
	...
	if (req->headers < MGCP_MAX_HEADERS)
		...
		cw_log(CW_LOG_WARNING, "Out of header space\n");
		return -1;
	}
	return 0;	
}
...
static int add_line(struct mgcp_request *req, char *line)
{
	...
	if (req->lines < MGCP_MAX_LINES)
		...
		cw_log(CW_LOG_WARNING, "Out of line space\n");
		return -1;
	}
	return 0;	
}
                            ...
                            int result, unsigned int ident, struct mgcp_request *resp)
{
	...
	if (p && (result >= 400) && (result <= 599)) {
		...
		if (sub) {
			if (sub->owner) {
				cw_log(CW_LOG_NOTICE, "Terminating on result %d from %s@%s-%d\n", 
					...
					if (sub->owner) {
						if (!cw_strlen_zero(sub->cxident)) {
							if (strcasecmp(c, sub->cxident)) {
								cw_log(CW_LOG_WARNING, "Subchannel already has a cxident. sub->cxident: %s requested %s\n", sub->cxident, c);
							}
						}
	...
	if (sub->outgoing) {
		...
		if (!sub->owner) {
			...
			if (p->immediate) {
				...
				if (!c) {
					cw_log(CW_LOG_WARNING, "Unable to start PBX on channel %s@%s\n", p->name, p->parent->name);
					transmit_notify_request(sub, "G/cg");
					cw_hangup(c);
				}
			} else {
				...
				if (c) {
					if (cw_pthread_create(&t, &global_attr_detached, mgcp_ss, c)) {
						cw_log(CW_LOG_WARNING, "Unable to create switch thread: %s\n", strerror(errno));
						cw_hangup(c);
					}
				} else {
					cw_log(CW_LOG_WARNING, "Unable to create channel for %s@%s\n", p->name, p->parent->name);
				}
			}
		} else {
			if (p->hookstate == MGCP_OFFHOOK) {
				cw_log(CW_LOG_WARNING, "Off hook, but already have owner on %s@%s\n", p->name, p->parent->name);
			} else {
...
static int handle_request(struct mgcp_subchannel *sub, struct mgcp_request *req, struct sockaddr_in *sin)
{
	...
	if (!strcasecmp(req->verb, "RSIP")) {
		...
		s = strchr(ev, '/');
		if (s) ev = s + 1;
		cw_log(CW_LOG_DEBUG, "Endpoint '%s@%s-%d' observed '%s'\n", p->name, p->parent->name, sub->id, ev);
		...
		if (!strcasecmp(ev, "hd")) {
			...
			if (p->callwaiting || p->transfer || p->threewaycalling) {
				...
				cw_log(CW_LOG_WARNING, "Callwaiting, call transfer or threeway calling not enabled on endpoint %s@%s\n", 
					p->name, p->parent->name);
			}
			...
			p->hookstate = MGCP_ONHOOK;
			sub->cxmode = MGCP_CX_RECVONLY;
			cw_log(CW_LOG_DEBUG, "MGCP %s@%s Went on hook\n", p->name, p->parent->name);
			...
			*/
			if (p->transfer && (sub->owner && sub->next->owner) && ((!sub->outgoing) || (!sub->next->outgoing))) {
				...
				if (res < 0) {
					...
					cw_log(CW_LOG_WARNING, "Transfer attempt failed\n");
				  ...
				  (ev[0] == '*') || (ev[0] == '#')))
        {
			...
			cw_log(CW_LOG_NOTICE, "Received unknown event '%s' from %s@%s\n", ev, p->name, p->parent->name);
		}
	} else {
		cw_log(CW_LOG_WARNING, "Unknown verb '%s' received from %s\n", req->verb, cw_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr));
...
static int mgcpsock_read(struct cw_io_rec *ior, int fd, short events, void *ignore)
{
	...
	if (cw_strlen_zero(req.identifier)) {
		cw_log(CW_LOG_NOTICE, "Message from %s missing identifier\n", cw_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr));
		return 1;
	}
	...
	if (sscanf(req.verb, "%d", &result) && sscanf(req.identifier, "%d", &ident)) {
		...
		if (sub) {
			...
			for (prev = NULL, cur = gw->msgs; cur; prev = cur, cur = cur->next) {
				if (cur->seqno == ident) {
					cw_log(CW_LOG_DEBUG, "Got response back on transaction %d\n", ident);
					if (prev)
						prev->next = cur->next;
					else
						gw->msgs = cur->next;
			...
			cw_log(CW_LOG_NOTICE, "Got response back on [%s] for transaction %d we aren't sending?\n", 
				gw->name, ident);
		}
	} else {
		if (cw_strlen_zero(req.endpoint) || 
			...
			cw_log(CW_LOG_NOTICE, "Message must have a verb, an idenitifier, version, and endpoint\n");
			return 1;
		}
...
static struct cw_channel *mgcp_request(const char *drvtype, int format, void *data, int *cause)
{
	...
	if (cw_strlen_zero(tmp)) {
		cw_log(CW_LOG_NOTICE, "MGCP Channels require an endpoint\n");
		return NULL;
	}
	sub = find_subchannel_and_lock(tmp, 0, NULL);
	if (!sub) {
		cw_log(CW_LOG_WARNING, "Unable to find MGCP endpoint '%s'\n", tmp);
	...
	cw_mutex_unlock(&sub->lock);
	if (!tmpc)
		cw_log(CW_LOG_WARNING, "Unable to make channel for '%s'\n", tmp);
	restart_monitor();
	return tmpc;
}
		...
		while(v) {
			if (!strcasecmp(v->name, "host")) {
				...
				if (!strcasecmp(v->value, "inband"))
					...
					cw_log(CW_LOG_WARNING, "'%s' is not a valid DTMF mode at line %d\n", v->value, v->lineno);
					...
					if (!ep_reload) {
						...
						for (i = 0; i < MAX_SUBS; i++) {
							...
							if (sub) {
								...
								cw_log(CW_LOG_WARNING, "Out of memory allocating subchannel");
								return NULL;
							}
	 					}
				...
				cw_log(CW_LOG_WARNING, "Don't know keyword '%s' at line %d\n", v->name, v->lineno);
			v = v->next;
		}
	}
	if (!ntohl(gw->addr.sin_addr.s_addr) && !gw->dynamic) {
		cw_log(CW_LOG_WARNING, "Gateway '%s' lacks IP address and isn't dynamic\n", gw->name);
		if (!gw_reload) {
			cw_mutex_destroy(&gw->msgs_lock);
			free(gw);
		}
...
static int mgcp_do_debug(cw_dynstr_t *ds_p, int argc, char *argv[])
{
	CW_UNUSED(argv);
...
static int mgcp_no_debug(cw_dynstr_t *ds_p, int argc, char *argv[])
{
	CW_UNUSED(argv);
...
static struct cw_clicmd  cli_mgcp_reload = {
...
static int reload_config(void)
{
	...
	if (gethostname(ourhost, sizeof(ourhost)-1)) {
		cw_log(CW_LOG_WARNING, "Unable to get hostname, MGCP disabled\n");
		return 0;
	}
	...
	if (!cfg) {
		cw_log(CW_LOG_NOTICE, "Unable to load config %s, MGCP disabled\n", config);
		return 0;
	}
	...
	if (ntohl(bindaddr.sin_addr.s_addr)) {
		...
		if (!hp) {
			cw_log(CW_LOG_WARNING, "Unable to get our IP address, MGCP disabled\n");
...
static int unload_module(void)
{
	...
	if (cw_mutex_trylock(&mgcp_reload_lock)) {
		cw_log(CW_LOG_WARNING, "MGCP is currently reloading.  Unable to remove module.\n");
		return -1;
	} else {
		mgcp_reloading = 1;
		cw_mutex_unlock(&mgcp_reload_lock);
	...
	if (!cw_mutex_lock(&gatelock)) {
		...
		cw_log(CW_LOG_WARNING, "Unable to lock the gateways list.\n");
		/* We always want to leave this in a consistent state */
		cw_channel_register(&mgcp_tech);
