...
#define CHECK_FORMATS(ast, p) do { \
	if (p->chan) {\
		if (ast->nativeformats != p->chan->nativeformats) { \
			cw_log(CW_LOG_DEBUG, "Native formats changing from %d to %d\n", ast->nativeformats, p->chan->nativeformats); \
			/* Native formats changed, reset things */ \
			ast->nativeformats = p->chan->nativeformats; \
			cw_log(CW_LOG_DEBUG, "Resetting read to %d and write to %d\n", ast->readformat, ast->writeformat);\
...
static struct agent_pvt *add_agent(char *agent, int pending)
{
	...
	if (!cw_separate_app_args(&args, argstr, ",", '\0', NULL)) {
		if (args.used) {
			agt = args.data[0];
			if (args.used > 1)
				password = cw_skip_blanks(args.data[1]);
			if (args.used > 2)
				name = cw_skip_blanks(args.data[2]);
		} else
		...
		if (!p && (p = malloc(sizeof(struct agent_pvt)))) {
			// Build the agent.
...
static int __agent_start_monitoring(struct cw_channel *ast, struct agent_pvt *p, int needlock)
{
	...
	if (!ast->monitor) {
		...
		cw_log(CW_LOG_ERROR, "Recording already started on that call.\n");
	return res;
}
...
static struct cw_frame *agent_read(struct cw_channel *ast)
{
	...
	if (!f) {
		...
		if (p->chan) {
			...
			if (!cw_strlen_zero(p->loginchan)) {
				if (p->chan)
					cw_log(CW_LOG_DEBUG, "Bridge on '%s' being cleared (2)\n", p->chan->name);
				cw_hangup(p->chan);
...
static int agent_call(struct cw_channel *ast, const char *dest)
{
	...
	if (!p->chan) {
		if (p->pending) {
			cw_log(CW_LOG_DEBUG, "Pretending to dial on pending agent\n");
			newstate = CW_STATE_DIALING;
			res = 0;
		} else {
			cw_log(CW_LOG_NOTICE, "Whoa, they hung up between alloc and call...  what are the odds of that?\n");
			res = -1;
		}
		cw_mutex_unlock(&p->lock);
		if (newstate)
	...
	cw_log(CW_LOG_DEBUG, "Playing beep, lang '%s'\n", p->chan->language);
	res = cw_streamfile(p->chan, beep, p->chan->language);
	cw_log(CW_LOG_DEBUG, "Played beep, result '%d'\n", res);
	if (!res) {
		res = cw_waitstream(p->chan, "");
		cw_log(CW_LOG_DEBUG, "Waited for stream, result '%d'\n", res);
	}
	if (!res) {
		res = cw_set_read_format(p->chan, cw_best_codec(p->chan->nativeformats));
		cw_log(CW_LOG_DEBUG, "Set read format, result '%d'\n", res);
		if (res)
			cw_log(CW_LOG_WARNING, "Unable to set read format to %s\n", cw_getformatname(cw_best_codec(p->chan->nativeformats)));
	} else {
		/* Agent hung-up */
		p->chan = NULL;
	}
	...
	if (!res) {
		...
		cw_log(CW_LOG_DEBUG, "Set write format, result '%d'\n", res);
		if (res)
			cw_log(CW_LOG_WARNING, "Unable to set write format to %s\n", cw_getformatname(cw_best_codec(p->chan->nativeformats)));
	}
	if( !res )
	{
...
static int agent_hangup(struct cw_channel *ast)
{
	...
	if (p->chan) {
		...
		if (!cw_strlen_zero(p->loginchan)) {
			...
			cw_log(CW_LOG_DEBUG, "Hungup, howlong is %d, autologoff is %d\n", howlong, p->autologoff);
...
#if 0
	if( !res )
		cw_log(CW_LOG_DEBUG, "agent_cont_sleep() returning %d\n", res );
#endif		
	return res;
}
...
static struct cw_channel *agent_bridgedchannel(struct cw_channel *chan, struct cw_channel *bridge)
{
	...
	if (option_debug)
		cw_log(CW_LOG_DEBUG, "Asked for bridged channel on '%s'/'%s', returning '%s'\n", chan->name, bridge->name, ret ? ret->name : "<none>");
	return ret;
}
	...
	if (tmp) {
		...
		if (p->chan) {
			if (cw_test_flag(p->chan, CW_FLAG_BLOCKING)) {
				cw_log(CW_LOG_ERROR, "A blocker exists after agent channel ownership acquired\n" );
				CRASH;
			}
			cw_moh_stop(p->chan);
		}
	} else
		cw_log(CW_LOG_WARNING, "Unable to allocate agent channel structure\n");
	return tmp;
}
...
static int read_agent_config(void)
{
	...
	if (!cfg) {
		cw_log(CW_LOG_NOTICE, "No agent configuration found -- agent support disabled\n");
		return 0;
	}
	cw_mutex_lock(&agentlock);
	p = agents;
...
static int check_availability(struct agent_pvt *newlyavailable, int needlock)
{
	...
	if (option_debug)
		cw_log(CW_LOG_DEBUG, "Checking availability of '%s'\n", newlyavailable->agent);
	if (needlock)
		cw_mutex_lock(&agentlock);
	p = agents;
	while(p) {
		...
		if (!p->abouttograb && p->pending && ((p->group && (newlyavailable->group & p->group)) || !strcmp(p->agent, newlyavailable->agent))) {
			if (option_debug)
				cw_log(CW_LOG_DEBUG, "Call '%s' looks like a winner for agent '%s'\n", p->owner->name, newlyavailable->agent);
	...
	if (parent && chan)  {
		...
		if (!res) {
			...
			if (p->abouttograb) {
				...
				if (option_debug)
					cw_log(CW_LOG_DEBUG, "Sneaky, parent disappeared in the mean time...\n");
				agent_cleanup(newlyavailable);
			}
		} else {
			if (option_debug)
				cw_log(CW_LOG_DEBUG, "Ugh...  Agent hung up at exactly the wrong time\n");
			agent_cleanup(newlyavailable);
		}
	}
	return 0;
...
static int check_beep(struct agent_pvt *newlyavailable, int needlock)
{
	...
	cw_log(CW_LOG_DEBUG, "Checking beep availability of '%s'\n", newlyavailable->agent);
	if (needlock)
		cw_mutex_lock(&agentlock);
	p = agents;
	while(p) {
		...
		if (!p->abouttograb && p->pending && ((p->group && (newlyavailable->group & p->group)) || !strcmp(p->agent, newlyavailable->agent))) {
			if (option_debug)
				cw_log(CW_LOG_DEBUG, "Call '%s' looks like a would-be winner for agent '%s'\n", p->owner->name, newlyavailable->agent);
			cw_mutex_unlock(&p->lock);
			break;
		}
		cw_mutex_unlock(&p->lock);
...
static struct cw_channel *agent_request(const char *type, int format, void *data, int *cause)
{
	...
	if (!p) {
		...
		while(p) {
			...
			if (!p->pending && ((groupmatch && (p->group & groupmatch)) || !strcmp(data, p->agent))) {
				...
				tv = cw_tvnow();
#if 0
				cw_log(CW_LOG_NOTICE, "Time now: %ld, Time of lastdisc: %ld\n", tv.tv_sec, p->lastdisc.tv_sec);
#endif
	...
	if (!chan && waitforagent) {
		...
		if (hasagent) {
			if (option_debug)
				cw_log(CW_LOG_DEBUG, "Creating place holder for '%s'\n", s);
			p = add_agent(data, 1);
			p->group = groupmatch;
			chan = agent_new(p, CW_STATE_DOWN);
			if (!chan) {
				cw_log(CW_LOG_WARNING, "Weird...  Fix this to drop the unused pending agent\n");
			}
		} else
			cw_log(CW_LOG_DEBUG, "Not creating place holder for '%s' since nobody logged in\n", s);
	}
	if (hasagent)
		*cause = CW_CAUSE_BUSY;
	else
...
static int agent_logoff_cmd(cw_dynstr_t *ds_p, int argc, char **argv)
{
	int ret;
	char *agent;
...
static void complete_agent_logoff_cmd(cw_dynstr_t *ds_p, char *argv[], int lastarg, int lastarg_len)
{
	char name[CW_MAX_AGENT];
	struct agent_pvt *p;
 ...
 * Show agents in cli.
 */
static int agents_show(cw_dynstr_t *ds_p, int argc, char **argv)
{
...
static struct cw_clicmd cli_show_agents = {
...
static struct cw_clicmd cli_agent_logoff = {
 ...
 * @returns 
 */
static int __login_exec(struct cw_channel *chan, int argc, char **argv, cw_dynstr_t *result, int callbackmode)
{
...
#if 0
		cw_log(CW_LOG_NOTICE, "user: %s, pass: %s\n", user, pass);
#endif		
			...
			if (!strcmp(p->agent, user) &&
				...
				if (!p->chan) {
					...
					if (callbackmode) {
						...
						while (!extension || extension[0] != '#') {
							...
							if (extension) {
								...
								cw_log(CW_LOG_WARNING, "Extension '%s@%s' is not valid for automatic login of agent '%s'\n", tmpchan, !cw_strlen_zero(context) ? context : "default", p->agent);
					...
					if (!res) {
						...
						if (res)
							cw_log(CW_LOG_WARNING, "Unable to set read format to %d\n", cw_best_codec(chan->nativeformats));
					}
					if (!res) {
						res = cw_set_write_format(chan, cw_best_codec(chan->nativeformats));
						if (res)
							cw_log(CW_LOG_WARNING, "Unable to set write format to %d\n", cw_best_codec(chan->nativeformats));
					}
					...
					if (callbackmode && !res) {
						...
						while (res >= 0) {
							...
							if (p->lastdisc.tv_sec) {
								if (cw_tvdiff_ms(cw_tvnow(), p->lastdisc) > p->wrapuptime) {
									if (option_debug)
										cw_log(CW_LOG_DEBUG, "Wrapup time for %s expired!\n", p->agent);
									p->lastdisc = cw_tv(0, 0);
									if (p->ackcall > 1)
										check_beep(p, 0);
									else
						...
						cw_mutex_lock(&p->lock);
						if (res && p->owner) 
							cw_log(CW_LOG_WARNING, "Huh?  We broke out when there was still an owner?\n");
 ...
 */
static int login_exec(struct cw_channel *chan, int argc, char **argv, cw_dynstr_t *result)
{
 ...
 */
static int callback_exec(struct cw_channel *chan, int argc, char **argv, cw_dynstr_t *result)
{
 ...
 */
static int agentmonitoroutgoing_exec(struct cw_channel *chan, int argc, char **argv, cw_dynstr_t *result)
{
	struct cw_var_t *var;
	...
	if (chan->cid.cid_num) {
		...
		if ((var = pbx_builtin_getvar_helper(NULL, cw_hash_var_name(agentvar), agentvar))) {
			...
			res = -1;
			if (!nowarnings)
				cw_log(CW_LOG_WARNING, "Couldn't find the global variable %s, so I can't figure out which agent (if it's an agent) is placing outgoing call.\n", agentvar);
		}
	} else {
		res = -1;
		if (!nowarnings)
			cw_log(CW_LOG_WARNING, "There is no callerid on that call, so I can't figure out which agent (if it's an agent) is placing outgoing call.\n");
	}
	if (res) {
		pbx_builtin_setvar_helper(chan, "AGENTSTATUS", "FAIL");
	} else {
	...
	for (cur_agent = agents; cur_agent; cur_agent = cur_agent->next) {
		...
		if (!cw_strlen_zero(cur_agent->loginchan)) {
			...
			if (cw_db_put(pa_family, cur_agent->agent, buf))
				cw_log(CW_LOG_WARNING, "failed to create persistent entry!\n");
			else if (option_debug)
				cw_log(CW_LOG_DEBUG, "Saved Agent: %s on %s\n", cur_agent->agent, cur_agent->loginchan);
		} else {
...
{
	cw_dynstr_t agent_data = CW_DYNSTR_INIT;
	char *agent_num;
...
static int load_module(void)
{
	...
	agentlogin_app = cw_register_function(app, login_exec, synopsis, syntax, descrip);
...
static int unload_module(void)
{
	...
	res |= cw_unregister_function(agentlogin_app);
