...
OPENPBX_FILE_VERSION(__FILE__, "$Revision: 1.860 $")
...
static int reqprep(struct sip_request *req, struct sip_pvt *p, int sipmethod, int seqno, int newbranch)
{
	...
	if (ast_test_flag(p, SIP_OUTGOING)) {
		if (!ast_test_flag(p, SIP_GENRPID)) {
			build_rpid(p);
...
static int __transmit_response(struct sip_pvt *p, char *msg, struct sip_request *req, int reliable)
{
	...
	if (p->owner && p->owner->hangupcause) {
		add_header(&resp, "X-OpenPBX-HangupCause", ast_cause2str(p->owner->hangupcause));
	}
	...
	if(!p->rpid && !p->rpid_from) {		
		...
		switch (p->callingpres)
			...
			case AST_PRES_NUMBER_NOT_AVAILABLE:
				...
				break;
			default:
				ast_log(LOG_WARNING, "Badly encoded callingpres (%d)\n", p->callingpres);
		...
		if (allowtags) {
			snprintf(rpid, sizeof(rpid), "\"%s\" <sip:%s@%s>;privacy=%s;screen=%s", clin, clid, ast_strlen_zero(p->fromdomain) ? ast_inet_ntoa(iabuf, sizeof(iabuf), p->ourip) : p->fromdomain, privacy, screen);
		} else {
			snprintf(rpid, sizeof(rpid), "\"%s\" <sip:%s@%s>", clin, clid, ast_strlen_zero(p->fromdomain) ? ast_inet_ntoa(iabuf, sizeof(iabuf), p->ourip) : p->fromdomain);
		}
		snprintf(rpid_from, sizeof(rpid_from), "\"%s\" <sip:%s@%s>;tag=as%08x", clin, ast_strlen_zero(p->fromuser) ? clid : p->fromuser, ast_strlen_zero(p->fromdomain) ? ast_inet_ntoa(iabuf, sizeof(iabuf), p->ourip) : p->fromdomain, p->tag);
		p->rpid = strdup(rpid);
...
static int get_refer_info(struct sip_pvt *sip_pvt, struct sip_request *outgoing_req)
{
	...
	if (!ast_strlen_zero(replace_callid)) {	
		...
		if ((sip_pvt_ptr = get_sip_pvt_byid_locked(replace_callid))) {
			...
			ast_log(LOG_NOTICE, "Supervised transfer requested, but unable to find callid '%s'.  Both legs must reside on OpenPBX box to transfer at this time.\n", replace_callid);
...
static int reload_config(void)
{
	...
	while(v) {
		...
		if (!strcasecmp(v->name, "context")) {
			...
			ast_set2_flag((&global_flags), ast_true(v->value), SIP_GENRPID);
		}
		/* else if (strcasecmp(v->name,"type"))
		 *	ast_log(LOG_WARNING, "Ignoring %s\n", v->name);
		 */
