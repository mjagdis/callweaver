...
static char default_useragent[OPBX_MAX_EXTENSION] = DEFAULT_USERAGENT;
...
static char default_notifymime[OPBX_MAX_EXTENSION] = DEFAULT_NOTIFYMIME;
...
static struct opbx_flags global_flags_page2 = {0};    /*!< more global SIP_ flags */
...
OPBX_MUTEX_DEFINE_STATIC(rand_lock);
...
OPBX_MUTEX_DEFINE_STATIC(netlock);
...
struct domain {
    ...
    char context[OPBX_MAX_EXTENSION];    /*!< Incoming context for this domain */
    enum domain_mode mode;            /*!< How did we find this domain? */
    OPBX_LIST_ENTRY(domain) list;        /*!< List mechanics */
};

static OPBX_LIST_HEAD_STATIC(domain_list, domain);    /*!< The SIP domain list */
...
struct sip_auth {
    char realm[OPBX_MAX_EXTENSION];  /*!< Realm in which these credentials are valid */
...
static struct sip_pvt {
    ...
    struct opbx_codec_pref prefs;        /*!< codec prefs */
    ...
    struct opbx_channel *owner;        /*!< Who owns us */
    char exten[OPBX_MAX_EXTENSION];        /*!< Extension where to start */
    char refer_to[OPBX_MAX_EXTENSION];    /*!< Place to store REFER-TO extension */
    char referred_by[OPBX_MAX_EXTENSION];    /*!< Place to store REFERRED-BY extension */
    char refer_contact[OPBX_MAX_EXTENSION];    /*!< Place to store Contact info from a REFER extension */
    struct sip_pvt *refer_call;        /*!< Call we are referring */
    struct sip_route *route;        /*!< Head of linked list of routing steps (fm Record-Route) */
    int route_persistant;            /*!< Is this the "real" route? */
    char from[256];                /*!< The From: header */
    char useragent[256];            /*!< User agent in SIP request */
    char context[OPBX_MAX_CONTEXT];        /*!< Context for this call */
    char subscribecontext[OPBX_MAX_CONTEXT];    /*!< Subscribecontext */
    char fromdomain[MAXHOSTNAMELEN];    /*!< Domain to show in the from field */
    char fromuser[OPBX_MAX_EXTENSION];    /*!< User to show in the user field */
    char fromname[OPBX_MAX_EXTENSION];    /*!< Name to show in the user field */
    ...
    struct opbx_dsp *vad;            /*!< Voice Activation Detection dsp */
    
    struct sip_peer *peerpoke;        /*!< If this calls is to poke a peer, which one */
    struct sip_registry *registry;        /*!< If this is a REGISTER call, to which registry */
    struct opbx_rtp *rtp;            /*!< RTP Session */
    struct opbx_rtp *vrtp;            /*!< Video RTP session */
...
struct sip_user {
    ...
    char context[OPBX_MAX_CONTEXT];    /*!< Default context for incoming calls */
    char subscribecontext[OPBX_MAX_CONTEXT];    /* Default context for subscriptions */
    ...
    struct opbx_flags flags_page2;    /*!< SIP_PAGE2 flags */
...
struct sip_peer {
    ...
    char mailbox[OPBX_MAX_EXTENSION]; /*!< Mailbox setting for MWI checks */
...
static struct opbx_register_list {
...
static struct opbx_ha *localaddr;
...
static struct sip_user *build_user(const char *name, struct opbx_variable *v, int realtime);
static int sip_do_reload(void);
static int expire_register(void *data);
static int callevents = 0;

static struct opbx_channel *sip_request_call(const char *type, int format, void *data, int *cause);
static int sip_devicestate(void *data);
static int sip_sendtext(struct opbx_channel *ast, const char *text);
...
static int sip_transfer(struct opbx_channel *ast, const char *dest);
static int sip_fixup(struct opbx_channel *oldchan, struct opbx_channel *newchan);
static int sip_senddigit(struct opbx_channel *ast, char digit);
...
static unsigned int parse_sip_options(struct sip_pvt *pvt, char *supported)
{
    char *next = NULL;
    char *sep = NULL;
    char *temp = opbx_strdupa(supported);
    int i;
    ...
    if (option_debug > 2 && sipdebug)
        opbx_log(OPBX_LOG_DEBUG, "Begin: parsing SIP \"Supported: %s\"\n", supported);
    ...
    while (next)
    {
        ...
        while (*next == ' ')    /* Skip spaces */
            next++;
        if (option_debug > 2 && sipdebug)
            opbx_log(OPBX_LOG_DEBUG, "Found SIP option: -%s-\n", next);
        for (i = 0;  (i < (sizeof(sip_options) / sizeof(sip_options[0])))  &&  !res;  i++)
        {
            if (!strcasecmp(next, sip_options[i].text))
            {
                ...
                res = 1;
                if (option_debug > 2  &&  sipdebug)
                    opbx_log(OPBX_LOG_DEBUG, "Matched SIP option: %s\n", next);
            }
        }
        if (!res) 
            if (option_debug > 2 && sipdebug)
                opbx_log(OPBX_LOG_DEBUG, "Found no match for SIP option: %s (Please file bug report!)\n", next);
        next = sep;
    }
    if (pvt)
    {
        ...
        if (option_debug)
            opbx_log(OPBX_LOG_DEBUG, "* SIP extension value: %d for call %s\n", profile, pvt->callid);
    }
    return profile;
}
...
static void build_via(struct sip_pvt *p, char *buf, int len)
{
    ...
    if (opbx_test_flag(p, SIP_NAT) & SIP_NAT_RFC3581)
        snprintf(buf, len, "SIP/2.0/UDP %s:%d;branch=z9hG4bK%08x;rport", opbx_inet_ntoa(iabuf, sizeof(iabuf), p->ourip), p->ourport, p->branch);
...
static int opbx_sip_ouraddrfor(struct in_addr *them, struct in_addr *us, struct sip_pvt *p)
{
        ...
        (stun_active || externip.sin_addr.s_addr))
    {
        ...
        if (externexpire  &&  (time(NULL) >= externexpire))
        {
            ...
            if ((hp = opbx_gethostbyname(externhost, &ahp)))
...
static int append_history(struct sip_pvt *p, const char *event, const char *data)
{
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "Can't allocate memory for history\n");
        return 0;
    }
...
static int retrans_pkt(void *data)
{
    ...
    {
        if (opbx_test_flag(pkt, FLAG_FATAL) || sipdebug)    /* Tell us if it's critical or if we're debugging */
            opbx_log(OPBX_LOG_WARNING, "Maximum retries exceeded on transmission %s for seqno %d (%s %s)\n", pkt->owner->callid, pkt->seqno, (opbx_test_flag(pkt, FLAG_FATAL)) ? "Critical" : "Non-critical", (opbx_test_flag(pkt, FLAG_RESPONSE)) ? "Response" : "Request");
    }
    else
    {
        if (pkt->method == SIP_OPTIONS && sipdebug)
            opbx_log(OPBX_LOG_WARNING, "Cancelling retransmit of OPTIONs (call id %s) \n", pkt->owner->callid);
    }
    ...
    if (opbx_test_flag(pkt, FLAG_FATAL))
    {
        ...
        if (pkt->owner->owner)
        {
            opbx_set_flag(pkt->owner, SIP_ALREADYGONE);
            opbx_log(OPBX_LOG_WARNING, "Hanging up call %s - no reply to our critical packet.\n", pkt->owner->callid);
            opbx_queue_hangup(pkt->owner->owner);
            opbx_mutex_unlock(&pkt->owner->owner->lock);
        }
        else
        {
	    ...
	    if (pkt->method != SIP_OPTIONS)
        	opbx_set_flag(pkt->owner, SIP_NEEDDESTROY);    
        }
    }
    ...
    if (cur)
    {
        ...
        opbx_log(OPBX_LOG_WARNING, "Weird, couldn't find packet owner!\n");
    if (pkt  &&  pkt->owner)
        opbx_mutex_unlock(&pkt->owner->lock);
    return 0;
}
...
static int __sip_autodestruct(void *data)
{
    ...
    if (p->subscribed)
    {
        transmit_state_notify(p, OPBX_EXTENSION_DEACTIVATED, 1, 1, 1);    /* Send first notification */
        p->subscribed = NONE;
    ...
    opbx_log(OPBX_LOG_DEBUG, "Auto destroying call '%s'\n", p->callid);
    append_history(p, "AutoDestroy", "");
    if (p->owner)
    {
        opbx_log(OPBX_LOG_WARNING, "Autodestruct on call '%s' with owner in place\n", p->callid);
...
static int sip_scheddestroy(struct sip_pvt *p, int ms)
{
    ...
    if (sip_debug_test_pvt(p))
        opbx_verbose("Scheduling destruction of call '%s' in %d ms\n", p->callid, ms);
    if (recordhistory)
    {
    ...
    if (p->autokillid > -1)
        opbx_sched_del(sched, p->autokillid);
    p->autokillid = opbx_sched_add(sched, ms, __sip_autodestruct, p);
    return 0;
}
...
static int __sip_ack(struct sip_pvt *p, int seqno, int resp, int sipmethod)
{
    ...
    while (cur)
    {
        if ((cur->seqno == seqno) && ((opbx_test_flag(cur, FLAG_RESPONSE)) == resp)
            &&
            ((opbx_test_flag(cur, FLAG_RESPONSE)) || 
             (!strncasecmp(msg, cur->data, strlen(msg)) && (cur->data[strlen(msg)] < 33))))
        {
            if (!resp  &&  (seqno == p->pendinginvite))
            {
                opbx_log(OPBX_LOG_DEBUG, "Acked pending invite %d\n", p->pendinginvite);
                p->pendinginvite = 0;
                resetinvite = 1;
            }
            /* this is our baby */
            ...
            {
                if (sipdebug && option_debug > 3)
                    opbx_log(OPBX_LOG_DEBUG, "** SIP TIMER: Cancelling retransmit of packet (reply received) Retransid #%d\n", cur->retransid);
    ...
    opbx_log(OPBX_LOG_DEBUG, "Stopping retransmission on '%s' of %s %d: Match %s\n", p->callid, resp ? "Response" : "Request", seqno, res ? "Not Found" : "Found");
    return res;
}
...
static int __sip_pretend_ack(struct sip_pvt *p)
{
    ...
    while (p->packets)
    {
        if (cur == p->packets)
        {
            opbx_log(OPBX_LOG_WARNING, "Have a packet that doesn't want to give up! %s\n", sip_methods[cur->method].text);
            return -1;
        }
        cur = p->packets;
        if (cur->method)
            __sip_ack(p, p->packets->seqno, (opbx_test_flag(p->packets, FLAG_RESPONSE)), cur->method);
        else
        {
            /* Unknown packet type */
            char *c;
            ...
            opbx_copy_string(method, p->packets->data, sizeof(method));
            c = opbx_skip_blanks(method); /* XXX what ? */
            *c = '\0';
            __sip_ack(p, p->packets->seqno, (opbx_test_flag(p->packets, FLAG_RESPONSE)), find_sip_method(method));
        }
    }
    return 0;
}
...
static int __sip_semi_ack(struct sip_pvt *p, int seqno, int resp, int sipmethod)
{
    ...
    while (cur)
    {
             ...
             (!strncasecmp(msg, cur->data, strlen(msg)) && (cur->data[strlen(msg)] < 33))))
        {
            ...
            {
                if (option_debug > 3 && sipdebug)
                    opbx_log(OPBX_LOG_DEBUG, "*** SIP TIMER: Cancelling retransmission #%d - %s (got response)\n", cur->retransid, msg);
                opbx_sched_del(sched, cur->retransid);
        	cur->retransid = -1;
            }
            res = 0;
    ...
    opbx_log(OPBX_LOG_DEBUG, "(Provisional) Stopping retransmission (but retaining packet) on '%s' %s %d: %s\n", p->callid, resp ? "Response" : "Request", seqno, res ? "Not Found" : "Found");
    return res;
}
...
static void sip_dealloc_headsdp_lines(struct sip_request *req) 
{
    ...
    while (tmpsdl != NULL)
    {
        tmprem=tmpsdl;
        tmpsdl=tmpsdl->next;
        //opbx_verbose("(-) removing header[%d]: %s\n",tmprem->type, tmprem->content);
        free(tmprem);
    }
    req->head_lines = NULL;
    ...
    while (tmpsdl != NULL)
    {
        tmprem = tmpsdl;
        tmpsdl = tmpsdl->next;
        //opbx_verbose("(-) removing line[%d]: %s\n",tmprem->type, tmprem->content);
        free(tmprem);
    }
    req->sdp_lines = NULL;
    return;
...
static void sip_rebuild_payload(struct sip_pvt *p, struct sip_request *req,int have_stun)
{
    ...
    while (tmpsdl != NULL)
    {
        ...
        if (p->stun_needed && have_stun)
        {
            ...
            {
                snprintf(p->our_contact, sizeof(p->our_contact), "<sip:%s%s%s:%d>", p->exten, opbx_strlen_zero(p->exten) ? "" : "@", 
                         opbx_inet_ntoa(iabuf, sizeof(iabuf), p->ourip), p->ourport);
            ...
            { 
                ...
                opbx_inet_ntoa(iabuf, sizeof(iabuf), p->ourip), p->ourport, p->branch);
...
#if STUN_DEV_DEBUG
        if (stundebug)
            opbx_log(OPBX_LOG_DEBUG,"(*) joining header[%d]: %s\n",tmpsdl->type, tmpsdl->content);
#endif
...
#if STUN_DEV_DEBUG
    if (stundebug)
        opbx_log(OPBX_LOG_DEBUG,"STUN_NEEDED: %d - HAVE_STUN: %d\n",p->stun_needed, have_stun);
#endif
    while (tmpsdl)
    {
        if (p->stun_needed  &&  have_stun)
        {
            ...
            else if (tmpsdl->type==SIP_DL_SDP_M_AUDIO)
            {
                ...
                if (stundebug)
                {
                    opbx_log(OPBX_LOG_DEBUG,"M_AUDIO was: %s\n",tmpsdl->content);
                    opbx_log(OPBX_LOG_DEBUG,"M_AUDIO is : %s\n",buf);
                }
#endif
...
#if STUN_DEV_DEBUG
        if (stundebug)
            opbx_log(OPBX_LOG_DEBUG,"(*) joining sdp[%d]: %s\n",tmpsdl->type, tmpsdl->content);
#endif
        strcat(req->data, tmpsdl->content);
        strcat(req->data, "\r\n");
        tmpsdl = tmpsdl->next;
...
static int send_response(struct sip_pvt *p, struct sip_request *req, int reliable, int seqno)
{
	...
	if (rr->streq) 
        ...
        {
            opbx_log(OPBX_LOG_DEBUG, "Setting NAT on UDPTL to %d\n", sip_is_nat_needed(p) );
            opbx_udptl_setnat(p->udptl, sip_is_nat_needed(p) );
        }
        if (rr->streq)
        {
            if (stundebug)
                opbx_log(OPBX_LOG_DEBUG,"** Sent STUN packet for response %d\n",rr->streq->req_head.id.id[0]);
            rr->p->stun_resreq_id = opbx_sched_add(sched, STUN_WAIT_RETRY_TIME, sip_resend_reqresp, rr);     
            return 0;   
        }
        else
        {
            opbx_log(OPBX_LOG_WARNING, "STUN: couldn't send packet. Trying to revert to old externip mode.\n");
        }
    }
    else if (p->stun_needed == 2)
    {
        if (stundebug)
            opbx_log(OPBX_LOG_DEBUG,"This transmit response has already a stun %d\n",p->stun_needed);
        sip_rebuild_payload(p, req, 1);
    }
    else
    {
        if (stundebug)
            opbx_log(OPBX_LOG_DEBUG,"This transmit response has stun at %d\n",p->stun_needed);
    ...
    {
        if ( sip_is_nat_needed(p) )
            opbx_verbose("%sTransmitting (NAT) to %s:%d:\n%s\n---\n", reliable ? "Reliably " : "", opbx_inet_ntoa(iabuf, sizeof(iabuf), p->recv.sin_addr), ntohs(p->recv.sin_port), req->data);
        else
...
static int send_request(struct sip_pvt *p, struct sip_request *req, int reliable, int seqno)
{
	...
	if (rr->streq) 
        ...
        if (p->vrtp  &&  opbx_rtp_get_stunstate(p->vrtp) == 0)
        {
            opbx_log(OPBX_LOG_DEBUG, "Setting NAT on VRTP to %d\n", sip_is_nat_needed(p) );
            opbx_rtp_setnat(p->vrtp, sip_is_nat_needed(p) );
        }
        ...
        {
            if (stundebug)
                opbx_log(OPBX_LOG_DEBUG,"** Sent STUN packet for request %d\n",rr->streq->req_head.id.id[0]);
                        rr->p->stun_resreq_id=opbx_sched_add(sched, STUN_WAIT_RETRY_TIME, sip_resend_reqresp, rr);     
            return 0;   
        }
        else
    ...
    {
        if (stundebug)
        opbx_log(OPBX_LOG_DEBUG,"This transmit request has already a stun %d\n",p->stun_needed);
        sip_rebuild_payload(p,req,1);
    }
    else
    {
        if (stundebug) opbx_log(OPBX_LOG_DEBUG,"This transmit request has stun at %d\n",p->stun_needed);
        sip_rebuild_payload(p,req,0);
    }
...
static int sip_resend_reqresp(void *data)
{
    ...
    {
        opbx_log(OPBX_LOG_DEBUG, "REQRESP: callid %s has been destroyed in the meanwhile. \n",rr->callid);    
    ...
    {
        if (stundebug)
            opbx_log(OPBX_LOG_DEBUG,"** expected stun reqid %d\n",rr->streq->req_head.id.id[0]);
    }
    else
    {
        if (stundebug)
            opbx_log(OPBX_LOG_DEBUG,"** deleting transmission retrial 'cause stun request is not set\n");
        rr->p->stun_needed=0;
        sip_dealloc_headsdp_lines(&rr->req);
        free(data);
        return 0;
    ...
    {
        if (stundebug)
            opbx_log(OPBX_LOG_DEBUG,"Deleting this request of reqresp (too many time has passed to wait for stun responses (sip %d)\n",rr->p->stun_resreq_id);
        p->stun_needed = 0;
        sip_dealloc_headsdp_lines(&rr->req);
        free(data);
        return 0;
    ...
    if (stundebug)
        opbx_log(OPBX_LOG_DEBUG,"** Trying to resend a packet after stun request. "
    ...
    if (stundebug)
    {
        opbx_log(OPBX_LOG_DEBUG,"** STUN: state - sip: %d - sdp ports: rtp:%d vrtp:%d udptl:%d \n",
                 (!map) ? 1 : 2,
    ...
    if (map
        &&
        opbx_rtp_get_stunstate(p->rtp) != 1
        &&
        opbx_rtp_get_stunstate(p->vrtp) != 1
        &&
    ...
    {
        if (stundebug)
            opbx_log(OPBX_LOG_DEBUG,"** SIP port stun mapped address not found. STUN state on sdp ports is: rtp:%d vrtp:%d udptl:%d \n",
                     opbx_rtp_get_stunstate(p->rtp),
                     opbx_rtp_get_stunstate(p->vrtp),
                     opbx_udptl_get_stunstate(p->udptl));
        rr->p->stun_resreq_id=opbx_sched_add(sched, STUN_WAIT_RETRY_TIME, sip_resend_reqresp, rr);
...
#if STUN_DEV_DEBUG
    if (stundebug)
        opbx_log(OPBX_LOG_DEBUG,"** STUN rebuilding payload before sending reqresp\n");
#endif
    ...
    {
        if (stundebug)
            opbx_log(OPBX_LOG_DEBUG,"** STUN Sending response after acquiring STUN\n");
    ...
    {
        if (stundebug)
            opbx_log(OPBX_LOG_DEBUG,"** STUN Sending request after acquiring STUN\n");
    ...
    {
        if (stundebug)
            opbx_log(OPBX_LOG_DEBUG, "deleting this request of reqresp %d\n",rr->p->stun_resreq_id);
    }
...
static char *get_in_brackets(char *tmp)
{
    ...
    while (1)
    {
        ...
        if (first_quote && first_bracket && (first_quote < first_bracket))
        {
            ...
            if (!*parse)
            {
                opbx_log(OPBX_LOG_WARNING, "No closing quote found in '%s'\n", tmp);
                return tmp;
            }
            parse++;
            continue;
        ...
        if (first_bracket)
        {
            ...
            {
                opbx_log(OPBX_LOG_WARNING, "No closing bracket found in '%s'\n", tmp);
                return tmp;
            }
        }
        return tmp;
...
static void sip_destroy_peer(struct sip_peer *peer)
{
    ...
    if (opbx_test_flag(peer, SIP_SELFDESTRUCT))
        apeerobjs--;
    else if (opbx_test_flag(peer, SIP_REALTIME))
        rpeerobjs--;
    else
        speerobjs--;
...
static struct sip_peer *realtime_peer(const char *peername, struct sockaddr_in *sin)
{
    ...
    if (newpeername) 
        var = opbx_load_realtime("sippeers", "name", peername, NULL);
    else if (sin)
    {
    ...
    if (!newpeername)
    {
        ...
        opbx_log(OPBX_LOG_WARNING, "Cannot Determine peer name ip=%s\n", iabuf);
        opbx_variables_destroy(var);
    ...
    if (opbx_test_flag((&global_flags_page2), SIP_PAGE2_RTCACHEFRIENDS))
    {
        ...
        if (opbx_test_flag((&global_flags_page2), SIP_PAGE2_RTAUTOCLEAR))
        {
            if (peer->expire > -1)
                opbx_sched_del(sched, peer->expire);
            peer->expire = opbx_sched_add(sched, (global_rtautoclear) * 1000, expire_register, (void *)peer);
        }
...
static int sip_addrcmp(char *name, struct sockaddr_in *sin)
{
    ...
    return     !(!inaddrcmp(&p->addr, sin) || 
                    (opbx_test_flag(p, SIP_INSECURE_PORT) &&
...
static void sip_destroy_user(struct sip_user *user)
{
    ...
    if (user->chanvars)
    {
        opbx_variables_destroy(user->chanvars);
        user->chanvars = NULL;
    }
    if (opbx_test_flag(user, SIP_REALTIME))
        ruserobjs--;
    else
        suserobjs--;
    free(user);
...
static struct sip_user *realtime_user(const char *username)
{
    ...
    var = opbx_load_realtime("sipusers", "name", username, NULL);
    ...
    user = build_user(username, var, !opbx_test_flag((&global_flags_page2), SIP_PAGE2_RTCACHEFRIENDS));
    ...
    if (opbx_test_flag((&global_flags_page2), SIP_PAGE2_RTCACHEFRIENDS))
    {
        opbx_set_flag((&user->flags_page2), SIP_PAGE2_RTCACHEFRIENDS);
        suserobjs++;
    ...
    else
    {
        ...
        suserobjs--;
        ruserobjs++;
        opbx_set_flag(user, SIP_REALTIME);
    }
    opbx_variables_destroy(var);
    return user;
}
...
static int create_addr_from_peer(struct sip_pvt *r, struct sip_peer *peer)
{
    ...
    {
        opbx_log(OPBX_LOG_DEBUG, "Setting NAT on RTP to %d\n", sip_is_nat_needed(r)  );
        opbx_rtp_setnat(r->rtp, sip_is_nat_needed(r)  );
    }
    if (r->vrtp)
    {
        opbx_log(OPBX_LOG_DEBUG, "Setting NAT on VRTP to %d\n", sip_is_nat_needed(r)  );
    ...
    opbx_copy_string(r->peersecret, peer->secret, sizeof(r->peersecret));
    opbx_copy_string(r->peermd5secret, peer->md5secret, sizeof(r->peermd5secret));
    opbx_copy_string(r->tohost, peer->tohost, sizeof(r->tohost));
    opbx_copy_string(r->fullcontact, peer->fullcontact, sizeof(r->fullcontact));
    if (!r->initreq.headers && !opbx_strlen_zero(peer->fromdomain))
    {
        if ((callhost = strchr(r->callid, '@')))
            strncpy(callhost + 1, peer->fromdomain, sizeof(r->callid) - (callhost - r->callid) - 2);
    }
    if (opbx_strlen_zero(r->tohost))
    {
        if (peer->addr.sin_addr.s_addr)
            opbx_inet_ntoa(r->tohost, sizeof(r->tohost), peer->addr.sin_addr);
        else
            opbx_inet_ntoa(r->tohost, sizeof(r->tohost), peer->defaddr.sin_addr);
    }
...
static int create_addr(struct sip_pvt *dialog, char *opeer)
{
    ...
    opbx_copy_string(peer, opeer, sizeof(peer));
    port = strchr(peer, ':');
    if (port)
    {
        *port = '\0';
    ...
    if (!p)
    {
        ...
        if (srvlookup)
        {
            ...
            ret = opbx_get_srv(NULL, host, sizeof(host), &tportno, service);
            if (ret > 0)
            {
                hostn = host;
                portno = tportno;
        ...
        if ((hp = opbx_gethostbyname(hostn, &ahp)))
        {
            opbx_copy_string(dialog->tohost, peer, sizeof(dialog->tohost));
...
static int auto_congest(void *nothing)
{
    ...
    if (p->owner)
    {
        ...
        {
            opbx_log(OPBX_LOG_NOTICE, "Auto-congesting %s\n", p->owner->name);
...
static int sip_call(struct opbx_channel *ast, char *dest, int timeout)
{
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "sip_call called on %s, neither down nor reserved\n", ast->name);
        return -1;
    }
    ...
    OPBX_LIST_TRAVERSE(headp,current,entries)
    {
        ...
        if (!p->options->vxml_url && !strcasecmp(opbx_var_name(current), "VXML_URL"))
        {
            p->options->vxml_url = opbx_var_value(current);
               }
        else if (!p->options->uri_options && !strcasecmp(opbx_var_name(current), "SIP_URI_OPTIONS"))
        {
        ...
        else if (!strncasecmp(opbx_var_name(current), "T38CALL", strlen("T38CALL")))
        {
            ...
            opbx_log(OPBX_LOG_DEBUG,"T38State change to %d on channel %s\n",p->t38state, ast->name);
        }
        
#ifdef OSP_SUPPORT
        else if (!p->options->osptoken && !strcasecmp(opbx_var_name(current), "OSPTOKEN"))
        {
            p->options->osptoken = opbx_var_value(current);
        }
        else if (!osphandle && !strcasecmp(opbx_var_name(current), "OSPHANDLE"))
        {
            osphandle = opbx_var_value(current);
        }
#endif
    }
    ...
    if (!p->options->osptoken || !osphandle || (sscanf(osphandle, "%d", &p->osphandle) != 1))
    {
        ...
        opbx_log(OPBX_LOG_DEBUG, "Disabling OSP support for this call. osptoken = %s, osphandle = %s\n", p->options->osptoken, osphandle);
        p->options->osptoken = NULL;
        osphandle = NULL;
        p->osphandle = -1;
    }
#endif
    opbx_log(OPBX_LOG_DEBUG, "Outgoing Call for %s\n", p->username);
    ...
    if (res != -1)
    {
        ...
        opbx_log(OPBX_LOG_DEBUG,"Our T38 capability (%d), joint T38 capability (%d)\n", p->t38capability, p->t38jointcapability);
        transmit_invite(p, SIP_INVITE, 1, 2);
        if (p->maxtime)
        {
            /* Initialize auto-congest time */
            p->initid = opbx_sched_add(sched, p->maxtime * 4, auto_congest, p);
        }
    }
    return res;
}
...
static void sip_registry_destroy(struct sip_registry *reg)
{
    ...
    if (reg->timeout > -1)
        opbx_sched_del(sched, reg->timeout);
    regobjs--;
    free(reg);
...
static void __sip_destroy(struct sip_pvt *p, int lockowner)
{
    ...
    if (sip_debug_test_pvt(p))
        opbx_verbose("Destroying call '%s'\n", p->callid);
    ...
    if (p->stateid > -1)
        opbx_extension_state_del(p->stateid, NULL);
    if (p->initid > -1)
    ...
    {
        if (lockowner)
            opbx_mutex_lock(&p->owner->lock);
        opbx_log(OPBX_LOG_DEBUG, "Detaching from %s\n", p->owner->name);
    ...
    if (!cur)
    {
        opbx_log(OPBX_LOG_WARNING, "Trying to destroy \"%s\", not found in dialog list?!?! \n", p->callid);
        return;
    } 
    while ((cp = p->packets))
    {
...
static int update_call_counter(struct sip_pvt *fup, int event)
{
    ...
    int outgoing = opbx_test_flag(fup, SIP_OUTGOING);
    struct sip_user *u = NULL;
    struct sip_peer *p = NULL;

    if (option_debug > 2)
        opbx_log(OPBX_LOG_DEBUG, "Updating call counter for %s call\n", outgoing ? "outgoing" : "incoming");
    /* Test if we need to check call limits, in order to avoid 
       realtime lookups if we do not need it */
    if (!opbx_test_flag(fup, SIP_CALL_LIMIT))
        return 0;

    opbx_copy_string(name, fup->username, sizeof(name));
    ...
    else
    {
        ...
        if (p)
        {
            ...
            opbx_copy_string(name, fup->peername, sizeof(name));
        }
        else
        {
            if (option_debug > 1)
                opbx_log(OPBX_LOG_DEBUG, "%s is not a local user, no call limit\n", name);
            return 0;
        }
    }
    switch (event)
    ...
    case DEC_CALL_LIMIT:
        ...
        if (option_debug > 1  ||  sipdebug)
            opbx_log(OPBX_LOG_DEBUG, "Call %s %s '%s' removed from call limit %d\n", outgoing ? "to" : "from", u ? "user":"peer", name, *call_limit);
        break;
    case INC_CALL_LIMIT:
        if (*call_limit > 0 )
        {
            if (*inuse >= *call_limit)
            {
                opbx_log(OPBX_LOG_ERROR, "Call %s %s '%s' rejected due to usage limit of %d\n", outgoing ? "to" : "from", u ? "user":"peer", name, *call_limit);
                if (u)
        ...
        (*inuse)++;
	opbx_set_flag(fup,SIP_INC_COUNT);
        if (option_debug > 1  ||  sipdebug)
            opbx_log(OPBX_LOG_DEBUG, "Call %s %s '%s' is %d out of %d\n", outgoing ? "to" : "from", u ? "user":"peer", name, *inuse, *call_limit);
        break;
    default:
        opbx_log(OPBX_LOG_ERROR, "update_call_counter(%s, %d) called with no event!\n", name, event);
    }
    if (u)
        ASTOBJ_UNREF(u,sip_destroy_user);
    else
...
static int hangup_sip2cause(int cause)
{
    ...
    case 487:	 /* Call cancelled */
        return OPBX_CAUSE_CALL_REJECTED;
    case 404:    /* Not found */
    ...
    case 488:    /* No codecs approved */
        return OPBX_CAUSE_BEARERCAPABILITY_NOTAVAIL;
    case 500:    /* Server internal failure */
        return OPBX_CAUSE_FAILURE;
    case 501:    /* Call rejected */
        return OPBX_CAUSE_FACILITY_REJECTED;
    case 502:    
        return OPBX_CAUSE_DESTINATION_OUT_OF_ORDER;
...
\verbatim
 Possible values from causes.h
        ...
        OPBX_CAUSE_FAILURE       OPBX_CAUSE_CONGESTION    OPBX_CAUSE_UNALLOCATED
...
static char *hangup_cause2sip(int cause)
{
    switch(cause)
    {
        case OPBX_CAUSE_UNALLOCATED:        /* 1 */
        case OPBX_CAUSE_NO_ROUTE_DESTINATION:    /* 3 IAX2: Can't find extension in context */
        case OPBX_CAUSE_NO_ROUTE_TRANSIT_NET:    /* 2 */
            return "404 Not Found";
        case OPBX_CAUSE_CONGESTION:        /* 34 */
        case OPBX_CAUSE_SWITCH_CONGESTION:    /* 42 */
            return "503 Service Unavailable";
        case OPBX_CAUSE_NO_USER_RESPONSE:    /* 18 */
            return "408 Request Timeout";
        case OPBX_CAUSE_NO_ANSWER:        /* 19 */
            return "480 Temporarily unavailable";
        case OPBX_CAUSE_CALL_REJECTED:        /* 21 */
            return "403 Forbidden";
        case OPBX_CAUSE_NUMBER_CHANGED:        /* 22 */
            return "410 Gone";
        case OPBX_CAUSE_NORMAL_UNSPECIFIED:    /* 31 */
            return "480 Temporarily unavailable";
        case OPBX_CAUSE_INVALID_NUMBER_FORMAT:
            return "484 Address incomplete";
        case OPBX_CAUSE_USER_BUSY:
            return "486 Busy here";
        case OPBX_CAUSE_FAILURE:
                    return "500 Server internal failure";
        case OPBX_CAUSE_FACILITY_REJECTED:    /* 29 */
            return "501 Not Implemented";
        case OPBX_CAUSE_CHAN_NOT_IMPLEMENTED:
            return "503 Service Unavailable";
        /* Used in chan_iax2 */
        case OPBX_CAUSE_DESTINATION_OUT_OF_ORDER:
            return "502 Bad Gateway";
        case OPBX_CAUSE_BEARERCAPABILITY_NOTAVAIL:    /* Can't find codec to connect to host */
            return "488 Not Acceptable Here";
            
        case OPBX_CAUSE_NOTDEFINED:
        default:
            opbx_log(OPBX_LOG_DEBUG, "AST hangup cause %d (no match found in SIP)\n", cause);
            return NULL;
    }
 ...
 * Part of PBX interface, called from opbx_hangup */
static int sip_hangup(struct opbx_channel *ast)
{
    struct sip_pvt *p = ast->tech_pvt;
    int needcancel = 0;
    ...
    if (option_debug)
        opbx_log(OPBX_LOG_DEBUG, "Hangup call %s, SIP callid %s)\n", ast->name, p->callid);

    opbx_mutex_lock(&p->lock);
#ifdef OSP_SUPPORT
    if ((p->osphandle > -1) && (ast->_state == OPBX_STATE_UP))
    {
        opbx_osp_terminate(p->osphandle, OPBX_CAUSE_NORMAL, p->ospstart, time(NULL) - p->ospstart);
    }
#endif    
    opbx_log(OPBX_LOG_DEBUG, "update_call_counter(%s) - decrement call limit counter\n", p->username);
    update_call_counter(p, DEC_CALL_LIMIT);
    /* Determine how to disconnect */
    if (p->owner != ast)
    {
        opbx_log(OPBX_LOG_WARNING, "Huh?  We aren't the owner? Can't hangup call.\n");
        opbx_mutex_unlock(&p->lock);
        return 0;
    }
    ...
    */
    ...
    if (opbx_test_flag(p, SIP_ALREADYGONE)) {
    ...
    if (!opbx_test_flag(p, SIP_ALREADYGONE) && !opbx_strlen_zero(p->initreq.data))
    {
        if (needcancel)
        {
		...
		/* are we allowed to send CANCEL yet? if not, mark
		   it pending */
		if (!opbx_test_flag(p, SIP_CAN_BYE)) {
			opbx_set_flag(p, SIP_PENDINGBYE);
			...
			/* Actually don't destroy us yet, wait for the 487 on our original 
			   INVITE, but do set an autodestruct just in case we never get it. */
                ...
                //opbx_clear_flag(&locflags, SIP_NEEDDESTROY);
        ...
        else
        {
            ...
            else
            {
                ...
                opbx_set_flag(p, SIP_PENDINGBYE);    
                opbx_clear_flag(p, SIP_NEEDREINVITE);    
            }
        }
    }
    if (needdestroy)
...
static void try_suggested_sip_codec(struct sip_pvt *p)
{
	...
	fmt = opbx_getformatbyname(codec);
	if (fmt) {
		opbx_log(OPBX_LOG_NOTICE, "Changing codec to '%s' for this call because of ${SIP_CODEC) variable\n",codec);
		if (p->jointcapability & fmt) {
			p->jointcapability &= fmt;
			p->capability &= fmt;
		} else
			opbx_log(OPBX_LOG_NOTICE, "Ignoring ${SIP_CODEC} variable because it is not shared by both ends.\n");
	} else
		opbx_log(OPBX_LOG_NOTICE, "Ignoring ${SIP_CODEC} variable because of unrecognized/not configured codec (check allow/disallow in sip.conf): %s\n",codec);
	return;	
}
...
static int sip_answer(struct opbx_channel *ast)
{
	...
	try_suggested_sip_codec(p);	
        ...
        if (option_debug)
            opbx_log(OPBX_LOG_DEBUG, "sip_answer(%s)\n", ast->name);
...
static int sip_rtp_write(struct opbx_channel *ast, struct opbx_frame *frame, int *faxdetect)
{
    struct sip_pvt *p = ast->tech_pvt;
    int res = 0;
    ...
    case OPBX_FRAME_VIDEO:
        if (p)
        {
            ...
            if (p->vrtp)
            {
                ...
                time(&p->lastrtptx);
                res =  opbx_rtp_write(p->vrtp, frame);
            }
            opbx_mutex_unlock(&p->lock);
        }
        break;
    ...
    case OPBX_FRAME_MODEM:
        if (p)
        {
            ...
            if (p->udptl)
            {
                ...
                res = opbx_udptl_write(p->udptl, frame);
            }
            opbx_mutex_unlock(&p->lock);
        }
        break;
    default: 
        opbx_log(OPBX_LOG_WARNING, "Can't send %d type frames with SIP write\n", frame->frametype);
        return 0;
    }
...
static int sip_fixup(struct opbx_channel *oldchan, struct opbx_channel *newchan)
{
    ...
    if (!p) {
	opbx_log(OPBX_LOG_WARNING, "No pvt after masquerade. Strange things may happen\n");
	return -1;
    }
...
static int sip_senddigit(struct opbx_channel *ast, char digit)
{
    ...
    int res = 0;
    opbx_mutex_lock(&p->lock);
    switch (opbx_test_flag(p, SIP_DTMF))
    {
    case SIP_DTMF_INFO:
...
static int sip_indicate(struct opbx_channel *ast, int condition)
{
    ...
    case OPBX_CONTROL_RINGING:
        if (ast->_state == OPBX_STATE_RING)
        {
            if (!opbx_test_flag(p, SIP_PROGRESS_SENT)
                ||
                (opbx_test_flag(p, SIP_PROG_INBAND) == SIP_PROG_INBAND_NEVER))
            {
                /* Send 180 ringing if out-of-band seems reasonable */
                transmit_response(p, "180 Ringing", &p->initreq);
                opbx_set_flag(p, SIP_RINGING);
                if (opbx_test_flag(p, SIP_PROG_INBAND) != SIP_PROG_INBAND_YES)
                    break;
            }
            else
            {
    ...
    case OPBX_CONTROL_BUSY:
        if (ast->_state != OPBX_STATE_UP)
        {
            ...
            opbx_softhangup_nolock(ast, OPBX_SOFTHANGUP_DEV);
            break;
        }
        res = -1;
        break;
    ...
    case OPBX_CONTROL_PROGRESS:
        ...
        res = -1;
        break;
    case OPBX_CONTROL_HOLD:    /* The other part of the bridge are put on hold */
        if (sipdebug)
            opbx_log(OPBX_LOG_DEBUG, "Bridged channel now on hold%s\n", p->callid);
        res = -1;
        break;
    case OPBX_CONTROL_UNHOLD:    /* The other part of the bridge are back from hold */
        if (sipdebug)
            opbx_log(OPBX_LOG_DEBUG, "Bridged channel is back from hold, let's talk! : %s\n", p->callid);
        res = -1;
        break;
    case OPBX_CONTROL_VIDUPDATE:    /* Request a video frame update */
        if (p->vrtp && !opbx_test_flag(p, SIP_NOVIDEO))
        {
...
static enum opbx_bridge_result sip_bridge(struct opbx_channel *c0, struct opbx_channel *c1, int flag, struct opbx_frame **fo,struct opbx_channel **rc, int timeoutms)
{
    ...
    if (c0->tech->bridge == sip_bridge)
    {
        ...
        opbx_log(OPBX_LOG_DEBUG, "T38 on channel %s is: %s", c0->name, ( res1 == T38_NEGOTIATED )  ?  "enabled\n"  :  "not enabled\n");
    }
    ...
    if (c1->tech->bridge == sip_bridge)
    {
        ...
        opbx_log(OPBX_LOG_DEBUG, "T38 on channel %s is: %s", c1->name, ( res2 == T38_NEGOTIATED ) ?  "enabled\n"  :  "not enabled\n");
    }
...
static struct opbx_channel *sip_new(struct sip_pvt *i, int state, char *title)
{
    ...
    if (tmp == NULL)
    {
        opbx_log(OPBX_LOG_WARNING, "Unable to allocate SIP channel structure\n");
        return NULL;
    }
    tmp->tech = &sip_tech;
...
//    opbx_mutex_lock(&i->lock);
    if (i->jointcapability)
        tmp->nativeformats = opbx_codec_choose(&i->prefs, i->jointcapability, 1);
    else if (i->capability)
        tmp->nativeformats = opbx_codec_choose(&i->prefs, i->capability, 1);
    else
        tmp->nativeformats = opbx_codec_choose(&i->prefs, global_capability, 1);
//    opbx_mutex_unlock(&i->lock);
    ...
    if (i->rtp)
    {
        tmp->fds[0] = opbx_rtp_fd(i->rtp);
        tmp->fds[1] = opbx_rtcp_fd(i->rtp);
    }
    if (i->vrtp)
    {
    ...
    if (!opbx_strlen_zero(i->musicclass))
        opbx_copy_string(tmp->musicclass, i->musicclass, sizeof(tmp->musicclass));
    i->owner = tmp;
//    opbx_mutex_lock(&usecnt_lock);
    ...
    if (!opbx_strlen_zero(i->exten) && strcmp(i->exten, "s"))
...
static char* get_sdp_by_line(char* line, char *name, int nameLen)
{
    if (strncasecmp(line, name, nameLen) == 0 && line[nameLen] == '=')
        return opbx_skip_blanks(line + nameLen + 1);
    return "";
}
        ...
        for (x = *start;  x < req->headers;  x++)
        {
            if (!strncasecmp(req->header[x], name, len))
            {
                ...
                if (pedanticsipchecking)
                    r = opbx_skip_blanks(r);

                if (*r == ':')
                {
                    *start = x+1;
                    return opbx_skip_blanks(r+1);
                }
            }
        }
...
static struct opbx_frame *sip_rtp_read(struct opbx_channel *ast, struct sip_pvt *p, int *faxdetect)
{
    ...
    if (f && (f->frametype == OPBX_FRAME_DTMF) && (opbx_test_flag(p, SIP_DTMF) != SIP_DTMF_RFC2833))
        return &null_frame;
    if (p->owner)
    {
		...
		if (!(f->subclass & p->jointcapability)) {
		    opbx_log(OPBX_LOG_DEBUG, "Bogus frame of format '%s' received from '%s'!\n",
...
static void build_callid(char *callid, int len, struct in_addr ourip, char *fromdomain)
{
    ...
    if (!opbx_strlen_zero(fromdomain))
...
static struct sip_pvt *sip_alloc(char *callid, struct sockaddr_in *sin, int useglobal_nat, const int intended_method)
{
    ...
    if (sip_methods[intended_method].need_rtp)
    {
        ...
        {
            opbx_log(OPBX_LOG_WARNING, "Unable to create RTP audio %s session: %s\n", videosupport ? "and video" : "", strerror(errno));
            opbx_mutex_destroy(&p->lock);
            if (p->chanvars)
            {
                opbx_variables_destroy(p->chanvars);
    ...
    if (p->method != SIP_REGISTER)
        opbx_copy_string(p->fromdomain, default_fromdomain, sizeof(p->fromdomain));
    ...
    opbx_mutex_unlock(&iflock);
    if (option_debug)
        opbx_log(OPBX_LOG_DEBUG, "Allocating new SIP dialog for %s - %s (%s)\n", callid ? callid : "(No Call-ID)", sip_methods[intended_method].text, p->rtp ? "With RTP" : "No RTP");
    return p;
}
...
static struct sip_pvt *find_call(struct sip_request *req, struct sockaddr_in *sin, struct sockaddr_in *sout, const int intended_method)
{
    ...
    if (pedanticsipchecking)
    {
        ...
        if (gettag(req, "To", totag, sizeof(totag)))
            opbx_set_flag(req, SIP_PKT_WITH_TOTAG);    /* Used in handle_request/response */
        ...
        if (option_debug > 4)
            opbx_log(OPBX_LOG_DEBUG, "= Looking for  Call ID: %s (Checking %s) --From tag %s --To-tag %s  \n", callid, req->method==SIP_RESPONSE ? "To" : "From", fromtag, totag);
    }
    ...
    while (p)
    {
        ...
        if (req->method == SIP_REGISTER)
            ...
            found = (!strcmp(p->callid, callid)
                    && 
                    (!pedanticsipchecking  ||  !tag  ||  opbx_strlen_zero(p->theirtag)  ||  !strcmp(p->theirtag, tag)));

        if (option_debug > 4)
            opbx_log(OPBX_LOG_DEBUG, "= %s Their Call ID: %s Their Tag %s Our tag: %s\n", found ? "Found" : "No match", p->callid, p->theirtag, p->tag);
        ...
        if (pedanticsipchecking  &&  found  &&  req->method != SIP_RESPONSE)
        {
            ...
            if (!found && option_debug > 4)
                opbx_log(OPBX_LOG_DEBUG, "= Being pedantic: This is not our match on request: Call ID: %s Ourtag <null> Totag %s Method %s\n", p->callid, totag, sip_methods[req->method].text);
        }
	...
	if (p)
	    ...
	    */
	    ...
	    if (option_debug > 3)
		opbx_log(OPBX_LOG_DEBUG, "Failed allocating SIP dialog, sending 500 Server internal error and giving up\n");
	}
    }
...
static int sip_register(char *value, int lineno)
{
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "Format for registration is user[:secret[:authuser]]@host[:port][/contact] at line %d\n", lineno);
        return -1;
    }
    stringp=username;
    ...
    if (!reg)
    {
        opbx_log(OPBX_LOG_ERROR, "Out of memory. Can't allocate SIP registry entry\n");
        return -1;
    }
    memset(reg, 0, sizeof(struct sip_registry));
    regobjs++;
    ASTOBJ_INIT(reg);
    opbx_copy_string(reg->contact, contact, sizeof(reg->contact));
    if (username)
        opbx_copy_string(reg->username, username, sizeof(reg->username));
    if (hostname)
        opbx_copy_string(reg->hostname, hostname, sizeof(reg->hostname));
    if (authuser)
        opbx_copy_string(reg->authuser, authuser, sizeof(reg->authuser));
    if (secret)
...
static void parse_request(struct sip_request *req)
{
    ...
    while (*c)
    {
        if (*c == '\n')
        {
            ...
            *c = 0;
            ...
            if (sipdebug && option_debug > 3)
                opbx_log(OPBX_LOG_DEBUG, "Header %d: %s (%d)\n", f, req->header[f], (int) strlen(req->header[f]));
            if (opbx_strlen_zero(req->header[f]))
            {
            ...
            {
                opbx_log(OPBX_LOG_WARNING, "Too many SIP headers. Ignoring.\n");
            }
            else
            {
                f++;
    ...
    while (*c)
    {
        if (*c == '\n')
        {
            ...
            *c = 0;
            if (sipdebug && option_debug > 3)
                opbx_log(OPBX_LOG_DEBUG, "Line: %s (%d)\n", req->line[f], (int) strlen(req->line[f]));
            if (f >= SIP_MAX_LINES - 1)
            {
                opbx_log(OPBX_LOG_WARNING, "Too many SDP lines. Ignoring.\n");
            }
            else
            {
                f++;
    ...
    /* Check for last line */
    if (!opbx_strlen_zero(req->line[f])) 
        f++;
    req->lines = f;
    if (*c) 
...
static int find_sdp(struct sip_request *req)
{
	...
	if (opbx_strlen_zero(search))
		return 0;

	/* make a duplicate of the string, with two extra characters
	   at the beginning */
	boundary = opbx_strdupa(search - 2);
...
static int process_sdp(struct sip_pvt *p, struct sip_request *req)
{
    ...
    if (!p->rtp)
    {
        opbx_log(OPBX_LOG_ERROR, "Got SDP but have no RTP session allocated.\n");
        return -1;
    }
    ...
    while ((m = get_sdp_iterate(&iterator, req, "m"))[0] != '\0')
    {
            ...
            (sscanf(m, "audio %d RTP/AVP %n", &x, &len) == 1))
        {
            ...
            opbx_rtp_pt_clear(p->rtp);
            codecs = m + len;
            while (!opbx_strlen_zero(codecs))
            {
                if (sscanf(codecs, "%d%n", &codec, &len) != 1)
                {
                    opbx_log(OPBX_LOG_WARNING, "Error in codec string '%s'\n", codecs);
                    return -1;
                }
                if (debug)
                    opbx_verbose("Found RTP audio format %d\n", codec);
                opbx_rtp_set_m_type(p->rtp, codec);
	...
	char *t38_disabled = pbx_builtin_getvar_helper(p->owner, "T38_DISABLE");
                ...
                (sscanf(m, "image %d UDPTL t38 %n", &x, &len) == 1)))
        {
            ...
            opbx_log(OPBX_LOG_DEBUG, "Activating UDPTL on response %s (1)\n", p->callid);
        ...
        {
            opbx_log(OPBX_LOG_DEBUG, "Activating RTP on response %s (1)\n", p->callid);
            opbx_rtp_set_active(p->rtp, 1);
    	    if (t38udptlsupport &&p->udptl )
        	opbx_udptl_set_active(p->udptl, 0);
            p->udptl_active = 0;
        ...
        if (p->vrtp && (sscanf(m, "video %d RTP/AVP %n", &x, &len) == 1))
        {
            ...
            while (!opbx_strlen_zero(codecs))
            {
                ...
                if (debug)
                    opbx_verbose("Found RTP video format %d\n", codec);
                opbx_rtp_set_m_type(p->vrtp, codec);
                codecs = opbx_skip_blanks(codecs + len);
            }
        }
        if (!found )
            opbx_log(OPBX_LOG_WARNING, "Unknown or ignored SDP media type in offer: %s\n", m);
    }
    ...
    if (pedanticsipchecking)
    {
        ...
        if (!opbx_strlen_zero(c))
        {
            ...
            else
            {
                /* XXX This could block for a long time, and block the main thread! XXX */
                if ((hp = opbx_gethostbyname(host, &ahp)) == NULL){
                    opbx_log(OPBX_LOG_WARNING, "Unable to lookup host in secondary c= line, '%s'\n", c);
		    return -1;
		}
            }
        }
    ...
    if (portno != -1)
    {
        ...
        if (p->rtp && sin.sin_port)
        {
            ...
            if (debug)
            {
                ...
                opbx_log(OPBX_LOG_DEBUG,"Peer audio RTP is at port %s:%d\n",opbx_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), ntohs(sin.sin_port));
            }
        }
    }
    ...
    if (vportno != -1)
    {
        ...
        if (p->vrtp && sin.sin_port)
        {
            ...
            if (debug)
            {
                ...
                opbx_log(OPBX_LOG_DEBUG,"Peer video RTP is at port %s:%d\n",opbx_inet_ntoa(iabuf, sizeof(iabuf), sin.sin_addr), ntohs(sin.sin_port));
            }
        }
    }
    ...
    {
        char *mimeSubtype = opbx_strdupa(a); /* ensures we have enough space */
	...
	if (!strcasecmp(a, "sendonly") || !strcasecmp(a, "inactive"))
        ...
        if (sscanf(a, "rtpmap: %u %[^/]/", &codec, mimeSubtype) != 2)
            continue;
        if (debug)
            opbx_verbose("Found description format %s\n", mimeSubtype);
        /* Note: should really look at the 'freq' and '#chans' params too */
        opbx_rtp_set_rtpmap_type(p->rtp, codec, "audio", mimeSubtype);
        if (p->vrtp)
    ...
    if (udptlportno != -1)
    {
        ...
        while ((a = get_sdp_iterate(&iterator, req, "a"))[0] != '\0')
        {
            ...
            {
                found = 1;
                opbx_log(OPBX_LOG_DEBUG,"MaxBufferSize:%d\n",x);
            }
            else if ((sscanf(a, "T38MaxBitRate:%d", &x) == 1))
            {
                found = 1;
                opbx_log(OPBX_LOG_DEBUG,"T38MaxBitRate: %d\n",x);
                switch (x)
                {
                case 33600:
            ...
            {
                found = 1;
                opbx_log(OPBX_LOG_DEBUG,"FaxVersion: %d\n",x);
                if (x == 0)
            ...
            {
                found = 1;
                opbx_log(OPBX_LOG_DEBUG,"FaxMaxDatagram: %d\n",x);
                opbx_udptl_set_far_max_datagram(p->udptl, x);
                opbx_udptl_set_local_max_datagram(p->udptl, x);
            }
            else if ((sscanf(a, "T38FaxFillBitRemoval:%d", &x) == 1))
            {
                found = 1;
                opbx_log(OPBX_LOG_DEBUG,"FillBitRemoval: %d\n",x);
                if (x == 1)
            ...
            {
                found = 1;
                opbx_log(OPBX_LOG_DEBUG,"Transcoding MMR: %d\n",x);
                if (x == 1)
            ...
            {
                found = 1;
                opbx_log(OPBX_LOG_DEBUG,"Transcoding JBIG: %d\n",x);
                if (x == 1)
    ...
    if (p->vrtp)
        opbx_rtp_get_current_formats(p->vrtp,
    ...
    if (opbx_test_flag(p, SIP_DTMF) == SIP_DTMF_AUTO)
    {
        opbx_clear_flag(p, SIP_DTMF);
        if (p->noncodeccapability & OPBX_RTP_DTMF)
        {
    ...
    if (debug)
    {
        ...
        opbx_verbose("Capabilities: us - %s, peer - audio=%s/video=%s, combined - %s\n",
            opbx_getformatname_multiple(s1, slen, p->capability),
            opbx_getformatname_multiple(s2, slen, peercapability),
            opbx_getformatname_multiple(s3, slen, vpeercapability),
            opbx_getformatname_multiple(s4, slen, p->jointcapability));

        opbx_verbose("Non-codec capabilities: us - %s, peer - %s, combined - %s\n",
            opbx_rtp_lookup_mime_multiple(s1, slen, noncodeccapability, 0),
            opbx_rtp_lookup_mime_multiple(s2, slen, peernoncodeccapability, 0),
            opbx_rtp_lookup_mime_multiple(s3, slen, p->noncodeccapability, 0));
    }
    if (!p->jointcapability)
    {
        opbx_log(OPBX_LOG_NOTICE, "No compatible codecs!\n");
        return -1;
    }
    ...
    if (!(p->owner->nativeformats & p->jointcapability))
    {
        ...
        opbx_log(OPBX_LOG_DEBUG, "Oooh, we need to change our formats since our peer supports only %s and not %s\n", 
    ...
    if ((bridgepeer=opbx_bridged_channel(p->owner)))
    {
        ...
        {
            opbx_moh_stop(bridgepeer);
        
            /* Activate a re-invite */
            opbx_queue_frame(p->owner, &af);
        }
        else
        {
            /* No address for RTP, we're on hold */
            
            opbx_moh_start(bridgepeer, NULL);
            if (sendonly)
                opbx_rtp_stop(p->rtp);
            /* Activate a re-invite */
            opbx_queue_frame(p->owner, &af);
    ...
    if (sin.sin_addr.s_addr && !sendonly)
    {
        ...
        if (callevents && opbx_test_flag(p, SIP_CALL_ONHOLD))
        {
        ...
        opbx_clear_flag(p, SIP_CALL_ONHOLD);
    }
    else
    {            
        /* No address for RTP, we're on hold */
        ...
        opbx_set_flag(p, SIP_CALL_ONHOLD);
    }
...
static int add_header(struct sip_request *req, const char *var, const char *value, short int type)
{
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "Out of SIP header space\n");
        return -1;
    }
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "Out of space, can't add anymore (%s:%s)\n", var, value);
        return -1;
    }
...
static int add_line(struct sip_request *req, const char *dline, short int type)
{
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "Out of SIP line space\n");
        return -1;
    }
    if (!req->lines)
    {
...
static int copy_header(struct sip_request *req, struct sip_request *orig, char *field)
{
    ...
    opbx_log(OPBX_LOG_NOTICE, "No field '%s' present to copy\n", field);
    return -1;
}
...
static int copy_via_headers(struct sip_pvt *p, struct sip_request *req, struct sip_request *orig, char *field)
{
    ...
    for (;;)
    {
        ...
        if (!opbx_strlen_zero(oh))
        {
            if (!copied)
            {
                ...
                if (rport  &&  (opbx_test_flag(p, SIP_NAT) == SIP_NAT_ALWAYS))
                {
                    /* We need to add received port - rport */
                    opbx_copy_string(tmp, oh, sizeof(tmp));
                    ...
                    /* Whoo hoo!  Now we can indicate port address translation too!  Just
                          ...
                          posterity.  */
                    snprintf(new, sizeof(new), "%s;received=%s;rport=%d", tmp, opbx_inet_ntoa(iabuf, sizeof(iabuf), p->recv.sin_addr), ntohs(p->recv.sin_port));
                }
                else
                {
                    /* We should *always* add a received to the topmost via */
                    snprintf(new, sizeof(new), "%s;received=%s", oh, opbx_inet_ntoa(iabuf, sizeof(iabuf), p->recv.sin_addr));
                }
    ...
    if (!copied)
    {
        opbx_log(OPBX_LOG_NOTICE, "No header field '%s' present to copy\n", field);
        return -1;
    }
    return 0;
}
...
static void add_route(struct sip_request *req, struct sip_route *route)
{
    ...
    while (route)
    {
        ...
        *p++ = '<';
        opbx_copy_string(p, route->hop, rem);  p += n;
        *p++ = '>';
        rem -= (n+2);
        route = route->next;
    }
...
static void set_destination(struct sip_pvt *p, char *uri)
{
    ...
    if (debug)
        opbx_verbose("set_destination: Parsing <%s> for address/port to send to\n", uri);
    ...
    if (hn > sizeof(hostname)) 
        hn = sizeof(hostname);
    opbx_copy_string(hostname, h, hn);
    h += hn - 1;
    ...
    if (maddr)
    {
        ...
        opbx_copy_string(hostname, maddr, hn);
    }
    
    if ((hp = opbx_gethostbyname(hostname, &ahp)) == NULL)
    {
        opbx_log(OPBX_LOG_WARNING, "Can't find address for host '%s'\n", hostname);
        return;
    }
    ...
    if (debug)
        opbx_verbose("set_destination: set destination to %s, port %d\n", opbx_inet_ntoa(iabuf, sizeof(iabuf), p->sa.sin_addr), port);

    opbx_sip_ouraddrfor(&p->sa.sin_addr,&p->ourip,p);
}
...
static int reqprep(struct sip_request *req, struct sip_pvt *p, int sipmethod, int seqno, int newbranch)
{
    ...
    if (p->route && !opbx_strlen_zero(p->route->hop) && strstr(p->route->hop,";lr") == NULL)
        is_strict = 1;
    ...
    else if (sipmethod == SIP_ACK)
    {
        ...
        if (!opbx_strlen_zero(p->okcontacturi))
            c = is_strict ? p->route->hop : p->okcontacturi;
         else
             c = p->initreq.rlPart2;
    }
    else if (!opbx_strlen_zero(p->okcontacturi))
    {
            c = is_strict ? p->route->hop : p->okcontacturi; /* Use for BYE or REINVITE */
    }
    else if (!opbx_strlen_zero(p->uri))
    {
        c = p->uri;
    }
    else
    {
        ...
        c = get_header(orig, (opbx_test_flag(p, SIP_OUTGOING)) ? "To" : "From");
        opbx_copy_string(stripped, c, sizeof(stripped));
...
static int __transmit_response(struct sip_pvt *p, char *msg, struct sip_request *req, int reliable)
{
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "Unable to determine sequence number from '%s'\n", get_header(req, "CSeq"));
        return -1;
    }
...
static int t38_get_rate(int t38cap)
{
    ...
    {
        opbx_log(OPBX_LOG_DEBUG, "T38MaxFaxRate 14400 found\n");
        return 14400;
    }
    else if (maxrate & T38FAX_RATE_12000)
    {
        opbx_log(OPBX_LOG_DEBUG, "T38MaxFaxRate 12000 found\n");
        return 12000;
    }
    else if (maxrate & T38FAX_RATE_9600)
    {
        opbx_log(OPBX_LOG_DEBUG, "T38MaxFaxRate 9600 found\n");
        return 9600;
    }
    else if (maxrate & T38FAX_RATE_7200)
    {
        opbx_log(OPBX_LOG_DEBUG, "T38MaxFaxRate 7200 found\n");
        return 7200;
    }
    else if (maxrate & T38FAX_RATE_4800)
    {
        opbx_log(OPBX_LOG_DEBUG, "T38MaxFaxRate 4800 found\n");
        return 4800;
    }
    else if (maxrate & T38FAX_RATE_2400)
    {
        opbx_log(OPBX_LOG_DEBUG, "T38MaxFaxRate 2400 found\n");
        return 2400;
    }
    else
    {
        opbx_log(OPBX_LOG_DEBUG, "Strange, T38MaxFaxRate NOT found in peers T38 SDP.\n");
        return 0;
    }
}
...
static int add_t38_sdp(struct sip_request *resp, struct sip_pvt *p)
{
    ...
    if (!p->udptl)
    {
        opbx_log(OPBX_LOG_WARNING, "No way to add SDP without an UDPTL structure\n");
        return -1;
    }
    ...
    if (debug)
    {
        opbx_log(OPBX_LOG_DEBUG, "Our T38 capability (%d), peer T38 capability (%d), joint capability (%d)\n",
            p->t38capability,
            p->t38peercapability,
            p->t38jointcapability);    
    }
    ...
    snprintf(o, sizeof(o), "o=root %d %d IN IP4 %s", p->sessionid, p->sessionversion, opbx_inet_ntoa(iabuf, sizeof(iabuf), udptldest.sin_addr));
    add_line(resp, o, SIP_DL_SDP_O);
    snprintf(s, sizeof(s), "s=session");
    add_line(resp, s, SIP_DL_DONTCARE);
    snprintf(c, sizeof(c), "c=IN IP4 %s", opbx_inet_ntoa(iabuf, sizeof(iabuf), udptldest.sin_addr));
...
static int add_sdp(struct sip_request *resp, struct sip_pvt *p)
{
    ...
    snprintf(v, sizeof(v), "v=0");
    add_line(resp, v, SIP_DL_DONTCARE);
    snprintf(o, sizeof(o), "o=root %d %d IN IP4 %s", p->sessionid, p->sessionversion, opbx_inet_ntoa(iabuf, sizeof(iabuf), dest.sin_addr));
    add_line(resp, o, SIP_DL_SDP_O);
    snprintf(s, sizeof(s), "s=session");
    add_line(resp, s, SIP_DL_DONTCARE);
    snprintf(c, sizeof(c), "c=IN IP4 %s", opbx_inet_ntoa(iabuf, sizeof(iabuf), dest.sin_addr));
    add_line(resp, c, SIP_DL_SDP_C);
    snprintf(t, sizeof(t), "t=0 0");
    add_line(resp, t, SIP_DL_DONTCARE);
    ...
    {
        if ((rtp_code = opbx_rtp_lookup_code(p->rtp, 1, p->prefcodec)) != -1)
        {
    ...
    for (x = 0;  x < 32;  x++)
    {
        ...
        if (alreadysent & pref_codec)
            continue;
        if ((rtp_code = opbx_rtp_lookup_code(p->rtp, 1, pref_codec)) != -1)
        {
            sprintf(tmpstr," %d",rtp_code);
    ...
    for (x = 1; x <= ((videosupport && p->vrtp) ? OPBX_FORMAT_MAX_VIDEO : OPBX_FORMAT_MAX_AUDIO); x <<= 1)
    {
        ...
        if (alreadysent & x)
            continue;
        if ((rtp_code = opbx_rtp_lookup_code(p->rtp, 1, x)) != -1)
        {
            sprintf(tmpstr," %d",rtp_code);
    ...
    for (x = 1;  x <= OPBX_RTP_MAX;  x <<= 1)
    {
        if (!(p->noncodeccapability & x))
            continue;
        if ((rtp_code = opbx_rtp_lookup_code(p->rtp, 0, x)) != -1)
        {
            sprintf(tmpstr," %d",rtp_code);
                strcat(m_audio,tmpstr);
        }
...
static int transmit_response_with_sdp(struct sip_pvt *p, char *msg, struct sip_request *req, int retrans)
{
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "Unable to get seqno from '%s'\n", get_header(req, "CSeq"));
        return -1;
    }
    respprep(&resp, p, msg, req);
    if (p->rtp)
    {
        opbx_rtp_offered_from_local(p->rtp, 0);
	try_suggested_sip_codec(p);	
        add_sdp(&resp, p);
    }
    else
    {
        opbx_log(OPBX_LOG_ERROR, "Can't add SDP to response, since we have no RTP session allocated. Call-ID %s\n", p->callid);
    }
...
static int transmit_response_with_t38_sdp(struct sip_pvt *p, char *msg, struct sip_request *req, int retrans)
{
    ...
    if (p->udptl)
    {
        opbx_udptl_offered_from_local(p->udptl, 0);
        add_t38_sdp(&resp, p);
    }
    else
    {
        opbx_log(OPBX_LOG_ERROR, "Can't add SDP to response, since we have no UDPTL session allocated. Call-ID %s\n", p->callid);
    }
    return send_response(p, &resp, retrans, seqno);
}
...
static int determine_firstline_parts( struct sip_request *req ) 
{
    ...
    cmd = opbx_skip_blanks(req->header[0]);
    if (!*cmd)
        return -1;
    req->rlPart1 = cmd;
    e = opbx_skip_nonblanks(cmd);
    /* Get the command */
    if (*e)
        *e++ = '\0';
...
{
    struct sip_request req;
    if (opbx_test_flag(p, SIP_REINVITE_UPDATE))
    ...
    opbx_log(OPBX_LOG_DEBUG, "Activating UDPTL on reinvite %s (b)\n", p->callid);
    if (t38udptlsupport  &&  p->udptl )
    {
        opbx_rtp_set_active(p->rtp, 0);
    	opbx_udptl_set_active(p->udptl, 1);
...
static void extract_uri(struct sip_pvt *p, struct sip_request *req)
{
    char stripped[256];
    char *c, *n;
    opbx_copy_string(stripped, get_header(req, "Contact"), sizeof(stripped));
    c = get_in_brackets(stripped);
    n = strchr(c, ';');
    if (n)
        *n = '\0';
...
static void build_contact(struct sip_pvt *p)
{
    ...
    if (p->stun_needed)
        ...
        snprintf(p->our_contact, sizeof(p->our_contact), "<sip:%s%s%s:%d>", p->exten, opbx_strlen_zero(p->exten) ? "" : "@", opbx_inet_ntoa(iabuf, sizeof(iabuf), p->ourip), p->ourport);

    else {
        if (p->ourport != 5060)    // Needs to be 5060, according to the RFC
        snprintf(p->our_contact, sizeof(p->our_contact), "<sip:%s%s%s:%d>", p->exten, opbx_strlen_zero(p->exten) ? "" : "@", opbx_inet_ntoa(iabuf, sizeof(iabuf), p->ourip), p->ourport);
        else
        snprintf(p->our_contact, sizeof(p->our_contact), "<sip:%s%s%s>", p->exten, opbx_strlen_zero(p->exten) ? "" : "@", opbx_inet_ntoa(iabuf, sizeof(iabuf), p->ourip));
    }
}
...
static void build_rpid(struct sip_pvt *p)
{
    ...
    case OPBX_PRES_NUMBER_NOT_AVAILABLE:
        ...
        break;
    default:
        opbx_log(OPBX_LOG_WARNING, "Unsupported callingpres (%d)\n", p->callingpres);
...
static void initreqprep(struct sip_request *req, struct sip_pvt *p, int sipmethod)
{
    ...
    if (opbx_test_flag(p, SIP_USEREQPHONE))
    {
        char onlydigits = 1;

        x = 0;

        /* Test p->username against allowed characters in OPBX_DIGIT_ANY
        ...
        {
            if (!strchr(OPBX_DIGIT_ANYNUM, p->username[x]))
            {
                onlydigits = 0;
                break;
            }
    ...
    if (!opbx_strlen_zero(p->fromuser))
        ...
        opbx_copy_string(p->fromuser, l, sizeof(p->fromuser));

    /* Allow user to be overridden */
    if (!opbx_strlen_zero(p->fromname))
        n = p->fromname;
    else /* Save for any further attempts */
        opbx_copy_string(p->fromname, n, sizeof(p->fromname));

    if (pedanticsipchecking)
    {
        opbx_uri_encode(n, tmp, sizeof(tmp), 0);
        n = tmp;
        opbx_uri_encode(l, tmp2, sizeof(tmp2), 0);
        l = tmp2;
    }

    if ((p->ourport != 5060) && opbx_strlen_zero(p->fromdomain))    /* Needs to be 5060 */
        snprintf(from, sizeof(from), "\"%s\" <sip:%s@%s:%d>;tag=%s", n, l, opbx_strlen_zero(p->fromdomain) ? opbx_inet_ntoa(iabuf, sizeof(iabuf), p->ourip) : p->fromdomain, p->ourport, p->tag);
    else
    ...
    if (!opbx_strlen_zero(p->fullcontact))
    {
        ...
        opbx_build_string(&invite, &invite_max, "%s", p->fullcontact);
    }
    else
    {
        /* Otherwise, use the username while waiting for registration */
        opbx_build_string(&invite, &invite_max, "sip:");
        if (!opbx_strlen_zero(p->username))
        {
            n = p->username;
            if (pedanticsipchecking)
            {
            ...
            opbx_build_string(&invite, &invite_max, "%s@", n);
        }
        opbx_build_string(&invite, &invite_max, "%s", p->tohost);
        if (ntohs(p->sa.sin_port) != 5060)        /* Needs to be 5060 */
            opbx_build_string(&invite, &invite_max, ":%d", ntohs(p->sa.sin_port));
        opbx_build_string(&invite, &invite_max, "%s", urioptions);
    }

    /* If custom URI options have been provided, append them */
    if (p->options && p->options->uri_options)
        opbx_build_string(&invite, &invite_max, ";%s", p->options->uri_options);

    opbx_copy_string(p->uri, invite_buf, sizeof(p->uri));
    ...
    if (sipmethod == SIP_NOTIFY && !opbx_strlen_zero(p->theirtag)) { 
...
static int transmit_invite(struct sip_pvt *p, int sipmethod, int sdp, int init)
{
    ...
    {
        /* Call transfer */
        if (!opbx_strlen_zero(p->refer_to))
            add_header(&req, "Refer-To", p->refer_to, SIP_DL_DONTCARE);
        if (!opbx_strlen_zero(p->referred_by))
            add_header(&req, "Referred-By", p->referred_by, SIP_DL_DONTCARE);
    }
#ifdef OSP_SUPPORT
    if ((req.method != SIP_OPTIONS) && p->options && !opbx_strlen_zero(p->options->osptoken))
    {
        opbx_log(OPBX_LOG_DEBUG,"Adding OSP Token: %s\n", p->options->osptoken);
        add_header(&req, "P-OSP-Auth-Token", p->options->osptoken, SIP_DL_DONTCARE);
    }
#endif
    if (p->options && !opbx_strlen_zero(p->options->distinctive_ring))
    {
    ...
    if (p->options && p->options->addsipheaders )
    {
        ...
        if (ast)
        {
            ...
            headp = &ast->varshead;
            if (!headp)
                opbx_log(OPBX_LOG_WARNING,"No Headp for the channel...ooops!\n");
            else
            {
                OPBX_LIST_TRAVERSE(headp, current, entries)
                { 
                    /* SIPADDHEADER: Add SIP header to outgoing call        */
                    if (!strncasecmp(opbx_var_name(current), "SIPADDHEADER", strlen("SIPADDHEADER")))
                    {
                        header = opbx_var_value(current);
                        headdup = opbx_strdupa(header);
                        ...
                        if ((content = strchr(headdup, ':')))
                        {
                            *content = '\0';
                            ...
                            if (sipdebug)
                                opbx_log(OPBX_LOG_DEBUG, "Adding SIP Header \"%s\" with content :%s: \n", headdup, content);
                        }
                    }
                }
            }
    ...
    {
        opbx_udptl_offered_from_local(p->udptl, 1);
        opbx_log(OPBX_LOG_DEBUG, "T38 is in state %d on channel %s\n",p->t38state, p->owner ? p->owner->name : "<none>");
        add_t38_sdp(&req, p);
    }
    else if (sdp  &&  p->rtp)
    {
...
static int transmit_state_notify(struct sip_pvt *p, int state, int full, int substate, int timeout)
{
    ...
    if (opbx_get_hint(hint, sizeof(hint), NULL, 0, NULL, p->context, p->exten))
    {
        /* If they are not registered, we will override notification and show no availability */
        if (opbx_device_state(hint) == OPBX_DEVICE_UNAVAILABLE)
        {
            local_state = NOTIFY_CLOSED;
            pidfstate = "away";
            pidfnote = "Not online";
    ...
    if (strncmp(c, "sip:", 4))
    {
        opbx_log(OPBX_LOG_WARNING, "Huh?  Not a SIP header (%s)?\n", c);
        return -1;
    }
    if ((a = strchr(c, ';')))
        *a = '\0';
    ...
    switch(state)
    {
    case OPBX_EXTENSION_DEACTIVATED:
    ...
    case XPIDF_XML:
    case CPIM_PIDF_XML:
        opbx_build_string(&t, &maxbytes, "<?xml version=\"1.0\"?>\n");
        opbx_build_string(&t, &maxbytes, "<!DOCTYPE presence PUBLIC \"-//IETF//DTD RFCxxxx XPIDF 1.0//EN\" \"xpidf.dtd\">\n");
        opbx_build_string(&t, &maxbytes, "<presence>\n");
        opbx_build_string(&t, &maxbytes, "<presentity uri=\"%s;method=SUBSCRIBE\" />\n", mfrom);
        opbx_build_string(&t, &maxbytes, "<atom id=\"%s\">\n", p->exten);
        opbx_build_string(&t, &maxbytes, "<address uri=\"%s;user=ip\" priority=\"0.800000\">\n", mto);
        opbx_build_string(&t, &maxbytes, "<status status=\"%s\" />\n", (local_state ==  NOTIFY_OPEN) ? "open" : (local_state == NOTIFY_INUSE) ? "inuse" : "closed");
        opbx_build_string(&t, &maxbytes, "<msnsubstatus substatus=\"%s\" />\n", (local_state == NOTIFY_OPEN) ? "online" : (local_state == NOTIFY_INUSE) ? "onthephone" : "offline");
        opbx_build_string(&t, &maxbytes, "</address>\n</atom>\n</presence>\n");
        break;
    case PIDF_XML: /* Eyebeam supports this format */
        ...
        opbx_build_string(&t, &maxbytes, "<contact priority=\"1\">%s</contact>\n", mto);
        if (pidfstate[0] == 'b') /* Busy? Still open ... */
            opbx_build_string(&t, &maxbytes, "<status><basic>open</basic></status>\n");
        else
            opbx_build_string(&t, &maxbytes, "<status><basic>%s</basic></status>\n", (local_state != NOTIFY_CLOSED) ? "open" : "closed");
        opbx_build_string(&t, &maxbytes, "</tuple>\n</presence>\n");
        break;
    case DIALOG_INFO_XML: /* SNOM subscribes in this format */
        opbx_build_string(&t, &maxbytes, "<?xml version=\"1.0\"?>\n");
        opbx_build_string(&t, &maxbytes, "<dialog-info xmlns=\"urn:ietf:params:xml:ns:dialog-info\" version=\"%d\" state=\"%s\" entity=\"%s\">\n", p->dialogver++, full ? "full":"partial", mto);
        if ((state & OPBX_EXTENSION_RINGING) && global_notifyringing)
            opbx_build_string(&t, &maxbytes, "<dialog id=\"%s\" direction=\"recipient\">\n", p->exten);
        else
            opbx_build_string(&t, &maxbytes, "<dialog id=\"%s\">\n", p->exten);
        opbx_build_string(&t, &maxbytes, "<state>%s</state>\n", statestring);
        opbx_build_string(&t, &maxbytes, "</dialog>\n</dialog-info>\n");
        break;
    case NONE:
    default:
        break;
    ...
    if (t > tmp + sizeof(tmp))
        opbx_log(OPBX_LOG_WARNING, "Buffer overflow detected!!  (Please file a bug report)\n");
...
static int sip_reg_timeout(void *data)
{
    ...
    if (global_regattempts_max && (r->regattempts > global_regattempts_max))
    {
        ...
        opbx_log(OPBX_LOG_NOTICE, "   -- Giving up forever trying to register '%s@%s'\n", r->username, r->hostname);
...
static int transmit_register(struct sip_registry *r, int sipmethod, char *auth, char *authheader)
{
    ...
    {
        opbx_log(OPBX_LOG_NOTICE, "Strange, trying to register %s@%s when registration already pending\n", r->username, r->hostname);
        return 0;
    }
    ...
    if (r->call)
    {
        ...
        if (!auth)
        {
            opbx_log(OPBX_LOG_WARNING, "Already have a REGISTER going on to %s@%s?? \n", r->username, r->hostname);
            return 0;
        }
        p = r->call;
    ...
    else
    {
        ...
        if (!p)
        {
            opbx_log(OPBX_LOG_WARNING, "Unable to allocate registration call\n");
            return 0;
        }
        if (recordhistory)
        {
        ...
        if (create_addr(p, r->hostname))
        {
            ...
            if (r->timeout > -1)
            {
                ...
                opbx_log(OPBX_LOG_WARNING, "Still have a registration timeout for %s@%s (create_addr() error), %d\n", r->username, r->hostname, r->timeout);
            }
            else
            {
                r->timeout = opbx_sched_add(sched, global_reg_timeout*1000, sip_reg_timeout, r);
                opbx_log(OPBX_LOG_WARNING, "Probably a DNS error for registration to %s@%s, trying REGISTER again (after %d seconds)\n", r->username, r->hostname, global_reg_timeout);
            }
            r->regattempts++;
            return 0;
        }
        ...
        if (!opbx_strlen_zero(r->secret))    /* Secret (password) */
            opbx_copy_string(p->peersecret, r->secret, sizeof(p->peersecret));
        if (!opbx_strlen_zero(r->md5secret))
            opbx_copy_string(p->peermd5secret, r->md5secret, sizeof(p->peermd5secret));
        /* User name in this realm  
        - if authuser is set, use that, otherwise use username */
        if (!opbx_strlen_zero(r->authuser))
        {    
            opbx_copy_string(p->peername, r->authuser, sizeof(p->peername));
            opbx_copy_string(p->authname, r->authuser, sizeof(p->authname));
        }
        else
        {
            if (!opbx_strlen_zero(r->username))
            {
                opbx_copy_string(p->peername, r->username, sizeof(p->peername));
                opbx_copy_string(p->authname, r->username, sizeof(p->authname));
                opbx_copy_string(p->fromuser, r->username, sizeof(p->fromuser));
            }
        }
        if (!opbx_strlen_zero(r->username))
            opbx_copy_string(p->username, r->username, sizeof(p->username));
        /* Save extension in packet */
        opbx_copy_string(p->exten, r->contact, sizeof(p->exten));
    ...
    if (auth == NULL)
    {
        if (r->timeout > -1)
        {
            opbx_log(OPBX_LOG_WARNING, "Still have a registration timeout, #%d - deleting it\n", r->timeout);
            opbx_sched_del(sched, r->timeout);
        }
        r->timeout = opbx_sched_add(sched, global_reg_timeout * 1000, sip_reg_timeout, r);
        opbx_log(OPBX_LOG_DEBUG, "Scheduled a registration timeout for %s id  #%d \n", r->hostname, r->timeout);
    }
    ...
    else if (!opbx_strlen_zero(r->nonce))
    {
        ...
        if (sipdebug)
            opbx_log(OPBX_LOG_DEBUG, "   >>> Re-using Auth data for %s@%s\n", r->username, r->hostname);
        opbx_copy_string(p->realm, r->realm, sizeof(p->realm));
        opbx_copy_string(p->nonce, r->nonce, sizeof(p->nonce));
        opbx_copy_string(p->domain, r->domain, sizeof(p->domain));
        opbx_copy_string(p->opaque, r->opaque, sizeof(p->opaque));
        opbx_copy_string(p->qop, r->qop, sizeof(p->qop));
        ...
        if(!build_reply_digest(p, sipmethod, digest, sizeof(digest)))
            ...
            opbx_log(OPBX_LOG_NOTICE, "No authorization available for authentication of registration to %s@%s\n", r->username, r->hostname);
    ...
    if (sip_debug_test_pvt(p))
    {
        opbx_verbose("REGISTER %d headers, %d lines\n", p->initreq.headers, p->initreq.lines);
    }
    determine_firstline_parts(&p->initreq);
    r->regstate=auth?REG_STATE_AUTHSENT:REG_STATE_REGSENT;
    r->regattempts++;    /* Another attempt */
    if (option_debug > 3)
        opbx_verbose("REGISTER attempt %d to %s@%s\n", r->regattempts, r->username, r->hostname);
...
static int transmit_refer(struct sip_pvt *p, const char *dest)
{
    ...
    opbx_copy_string(from, of, sizeof(from));
    of = get_in_brackets(from);
    opbx_copy_string(p->from,of,sizeof(p->from));
    if (strncmp(of, "sip:", 4))
    {
        opbx_log(OPBX_LOG_NOTICE, "From address missing 'sip:', using it anyway\n");
    }
    else
        of += 4;
    ...
    opbx_copy_string(p->refer_to, referto, sizeof(p->refer_to));
    opbx_copy_string(p->referred_by, p->our_contact, sizeof(p->referred_by));

    reqprep(&req, p, SIP_REFER, 0, 1);
    add_header(&req, "Refer-To", referto, SIP_DL_DONTCARE);
    if (!opbx_strlen_zero(p->our_contact))
...
static int transmit_request_with_auth(struct sip_pvt *p, int sipmethod, int seqno, int reliable, int newbranch)
{
    ...
    if (*p->realm)
    {
        ...
        if (!build_reply_digest(p, sipmethod, digest, sizeof(digest)))
        {
            ...
            opbx_log(OPBX_LOG_WARNING, "No authentication available for call %s\n", p->callid);
    }
...
static int expire_register(void *data)
{
    ...
    if (opbx_test_flag(peer, SIP_SELFDESTRUCT) || opbx_test_flag((&peer->flags_page2), SIP_PAGE2_RTAUTOCLEAR))
    {
...
static void reg_source_db(struct sip_peer *peer)
{
    ...
    if (username)
        opbx_copy_string(peer->username, username, sizeof(peer->username));
    if (contact)
...
static int parse_ok_contact(struct sip_pvt *pvt, struct sip_request *req)
{
    ...
    /* Look for brackets */
    opbx_copy_string(contact, get_header(req, "Contact"), sizeof(contact));
    c = get_in_brackets(contact);

    /* Save full contact to call pvt for later bye or re-invite */
    opbx_copy_string(pvt->fullcontact, c, sizeof(pvt->fullcontact));    

    /* Save URI for later ACKs, BYE or RE-invites */
    opbx_copy_string(pvt->okcontacturi, c, sizeof(pvt->okcontacturi));
    
    /* Make sure it's a SIP URL */
    if (strncasecmp(c, "sip:", 4))
        opbx_log(OPBX_LOG_NOTICE, "'%s' is not a valid SIP contact (missing sip:) trying to use anyway\n", c);
    else
        c += 4;
    ...
    if (!sip_is_nat_needed(pvt) )
    {
        ...
        {
            opbx_log(OPBX_LOG_WARNING, "Invalid host '%s'\n", n);
            return -1;
        }
...
static enum parse_register_result parse_register_contact(struct sip_pvt *pvt, struct sip_peer *p, struct sip_request *req)
{
    ...
    */
    ...
    opbx_copy_string(p->fullcontact, c, sizeof(p->fullcontact));
    ...
    if (c)    /* Overwrite the default username from config at registration */
        opbx_copy_string(p->username, c, sizeof(p->username));
    else
        p->username[0] = '\0';
    ...
    if ((expiry < 1) || (expiry > max_expiry))
        expiry = max_expiry;
    if (!opbx_test_flag(p, SIP_REALTIME))
        p->expire = opbx_sched_add(sched, (expiry + 10) * 1000, expire_register, p);
    else
        p->expire = -1;
    pvt->expiry = expiry;
...
static void list_route(struct sip_route *route)
{
    if (!route)
    {
        opbx_verbose("list_route: no route\n");
        return;
    }
    while (route)
    {
        opbx_verbose("list_route: hop: <%s>\n", route->hop);
        route = route->next;
    }
}
...
static void build_route(struct sip_pvt *p, struct sip_request *req, int backwards)
{
    ...
    {
        opbx_log(OPBX_LOG_DEBUG, "build_route: Retaining previous route: <%s>\n", p->route->hop);
        return;
    }
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            if (thishop)
            {
                ...
                opbx_log(OPBX_LOG_DEBUG, "build_route: Record-Route hop: <%s>\n", thishop->hop);
                /* Link in */
                if (backwards)
                {
    ...
    if (!head  ||  (!opbx_strlen_zero(head->hop) && strstr(head->hop,";lr") == NULL))
    {
        ...
        if (!opbx_strlen_zero(contact))
        {
            opbx_log(OPBX_LOG_DEBUG, "build_route: Contact hop: %s\n", contact);
...
static int check_auth(struct sip_pvt *p, struct sip_request *req, char *randdata, int randlen, char *username, char *secret, char *md5secret, int sipmethod, char *uri, int reliable, int ignore)
{
    ...
    if (sipmethod == SIP_REGISTER || sipmethod == SIP_SUBSCRIBE || sipmethod == SIP_INVITE)
    {
        ...
        opbx_log(OPBX_LOG_DEBUG, "Checking OSP Authentication!\n");
        osptoken = get_header (req, "P-OSP-Auth-Token");
        switch (opbx_test_flag (p, SIP_OSPAUTH))
        {
            case SIP_OSPAUTH_NO:
                break;
            case SIP_OSPAUTH_GATEWAY:
                if (opbx_strlen_zero (osptoken))
                {
                    if (opbx_strlen_zero (secret) && opbx_strlen_zero (md5secret))
                    {
                        return (0);
    ...
    if (ignore && !opbx_strlen_zero(randdata) && opbx_strlen_zero(authtoken))
    {
        /* This is a retransmitted invite/register/etc, don't reconstruct authentication
           information */
        if (!opbx_strlen_zero(randdata))
        {
            if (!reliable)
            {
    ...
    else
    {
        ...
        opbx_copy_string(tmp, authtoken, sizeof(tmp));
        c = tmp;

        while (c)
        {
            c = opbx_skip_blanks(c);
            if (!*c)
                break;
        ...
        if (wrongnonce)
        {
            ...
            {
                if (sipdebug)
                    opbx_log(OPBX_LOG_NOTICE, "stale nonce received from '%s'\n", get_header(req, "To"));
            ...
            else
            {
                ...
                if (sipdebug)
                    opbx_log(OPBX_LOG_NOTICE, "Bad authentication received from '%s'\n", get_header(req, "To"));
                transmit_response_with_auth(p, response, req, randdata, reliable, respheader, 0);
            }
...
static int cb_extensionstate(char *context, char* exten, int state, void *data)
{
    ...
    case OPBX_EXTENSION_REMOVED:    /* Extension is gone */
        ...
        p->stateid = -1;
        p->subscribed = NONE;
        append_history(p, "Subscribestatus", state == OPBX_EXTENSION_REMOVED ? "HintRemoved" : "Deactivated");
        break;
    default:    /* Tell user */
        p->laststate = state;
        break;
...
static int register_verify(struct sip_pvt *p, struct sockaddr_in *sin, struct sip_request *req, char *uri, int ignore)
{
    ...
    if (uri == NULL) {
        opbx_log(OPBX_LOG_WARNING, "register_verify: URI is NULL!\n");
    ...
    *t = '\0';
    ...
    opbx_copy_string(tmp, get_header(req, "To"), sizeof(tmp));
    if (pedanticsipchecking)
        opbx_uri_decode(tmp);
    ...
    {
        name = c;
        opbx_log(OPBX_LOG_NOTICE, "Invalid to address: '%s' from %s (missing sip:) trying to use anyway...\n", c, opbx_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr));
    }
    ...
    if ((c = strchr(name, '@')))
    {
        *c++ = '\0';
        ...
        if ((c = strchr(domain, ':')))    /* Remove :port */
            *c = '\0';
        if (!OPBX_LIST_EMPTY(&domain_list))
        {
    ...
    opbx_copy_string(p->exten, name, sizeof(p->exten));
    build_contact(p);
    peer = find_peer(name, NULL, 1);

    if (!(peer && opbx_apply_ha(peer->ha, sin)))
    {
        if (peer)
            ASTOBJ_UNREF(peer,sip_destroy_peer);
	peer = NULL;
    ...
    if (peer)
    {
        ...
        {
            opbx_log(OPBX_LOG_ERROR, "Peer '%s' is trying to register, but not configured as host=dynamic\n", peer->name);
        }
        else
        {
            opbx_copy_flags(p, peer, SIP_NAT);
            ...
            if (!(res = check_auth(p, req, p->randdata, sizeof(p->randdata), peer->name, peer->secret, peer->md5secret, SIP_REGISTER, uri, 0, ignore)))
            {
                ...
                case PARSE_REGISTER_FAILED:
                    opbx_log(OPBX_LOG_WARNING, "Failed to parse contact info\n");
    ...
    if (res < 0)
    {
        ...
        if (option_debug > 1)
        {
            opbx_log(OPBX_LOG_DEBUG, "SIP REGISTER attempt failed for %s : %s\n",
                peer->name,
...
static int get_rdnis(struct sip_pvt *p, struct sip_request *oreq)
{
    ...
    if (!req)
        req = &p->initreq;
    opbx_copy_string(tmp, get_header(req, "Diversion"), sizeof(tmp));
    if (opbx_strlen_zero(tmp))
        return 0;
    c = get_in_brackets(tmp);
    if (strncmp(c, "sip:", 4))
    {
        opbx_log(OPBX_LOG_WARNING, "Huh?  Not an RDNIS SIP header (%s)?\n", c);
        return -1;
    }
    c += 4;
    ...
    if (sip_debug_test_pvt(p))
        opbx_verbose("RDNIS is %s\n", c);
    opbx_copy_string(p->rdnis, c, sizeof(p->rdnis));
...
static int get_destination(struct sip_pvt *p, struct sip_request *oreq)
{
    ...
    if (!req)
        req = &p->initreq;
    if (req->rlPart2)
        opbx_copy_string(tmp, req->rlPart2, sizeof(tmp));
    uri = get_in_brackets(tmp);
    
    opbx_copy_string(tmpf, get_header(req, "From"), sizeof(tmpf));
	...
	*opts = '\0';
    ...
    opbx_copy_string(p->domain, domain, sizeof(p->domain));

    if (!OPBX_LIST_EMPTY(&domain_list))
    {
        char domain_context[OPBX_MAX_EXTENSION];
        ...
        if (!check_sip_domain(p->domain, domain_context, sizeof(domain_context)))
        {
            ...
            {
                opbx_log(OPBX_LOG_DEBUG, "Got SIP %s to non-local domain '%s'; refusing request.\n", sip_methods[req->method].text, p->domain);
                return -2;
            }
        }
        /* If we have a context defined, overwrite the original context */
        if (!opbx_strlen_zero(domain_context))
            opbx_copy_string(p->context, domain_context, sizeof(p->context));
    }
    ...
    if (from)
    {
        ...
        {
            *a = '\0';
            opbx_copy_string(p->fromdomain, a + 1, sizeof(p->fromdomain));
        }
        else
            opbx_copy_string(p->fromdomain, from, sizeof(p->fromdomain));
    }
    if (sip_debug_test_pvt(p))
        opbx_verbose("Looking for %s in %s (domain %s)\n", uri, p->context, p->domain);

    /* Return 0 if we have a matching extension */
    if (opbx_exists_extension(NULL, p->context, uri, 1, from)
        ||
        !strcmp(uri, opbx_pickup_ext()))
    {
        if (!oreq)
            opbx_copy_string(p->exten, uri, sizeof(p->exten));
        return 0;
    }

    /* Return 1 for overlap dialling support */
    if (opbx_canmatch_extension(NULL, p->context, uri, 1, from)
        ||
        !strncmp(uri, opbx_pickup_ext(),strlen(uri)))
    {
        return 1;
    }
...
static struct sip_pvt *get_sip_pvt_byid_locked(char *callid) 
{
    ...
    while (sip_pvt_ptr)
    {
        if (!strcmp(sip_pvt_ptr->callid, callid))
        {
            ...
            opbx_mutex_lock(&sip_pvt_ptr->lock);
            if (sip_pvt_ptr->owner)
            {
                while (opbx_mutex_trylock(&sip_pvt_ptr->owner->lock))
                {
...
static int get_refer_info(struct sip_pvt *sip_pvt, struct sip_request *outgoing_req, char **transfercontext)
{
    ...
    struct sip_pvt *sip_pvt_ptr = NULL;
    struct opbx_channel *chan = NULL, *peer = NULL;
    ...
    if (!((p_refer_to = get_header(req, "Refer-To")) && (h_refer_to = opbx_strdupa(p_refer_to))))
    {
        opbx_log(OPBX_LOG_WARNING, "No Refer-To Header That's illegal\n");
        return -1;
    }

    refer_to = get_in_brackets(h_refer_to);

    if (!((p_referred_by = get_header(req, "Referred-By")) && (h_referred_by = opbx_strdupa(p_referred_by))))
    {
        opbx_log(OPBX_LOG_WARNING, "No Referrred-By Header That's not illegal\n");
        return -1;
    }
    else
    {
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "Refer-to: Huh?  Not a SIP header (%s)?\n", refer_to);
        return -1;
    }

    if (strncmp(referred_by, "sip:", 4))
    {
        opbx_log(OPBX_LOG_WARNING, "Referred-by: Huh?  Not a SIP header (%s) Ignoring?\n", referred_by);
        referred_by = NULL;
    }
    ...
    if ((ptr = strchr(refer_to, '?')))
    {
        ...
        *ptr = '\0';
        ...
        {
            char *p;
            replace_callid = opbx_strdupa(ptr + 9);
            /* someday soon to support invite/replaces properly!
               replaces_header = opbx_strdupa(replace_callid); 
               -anthm
            */
            opbx_uri_decode(replace_callid);
            if ((ptr = strchr(replace_callid, '%'))) 
                *ptr = '\0';
            if ((ptr = strchr(replace_callid, ';'))) 
                *ptr = '\0';
            /* Skip leading whitespace XXX memmove behaviour with overlaps ? */
            p = opbx_skip_blanks(replace_callid);
            if (p != replace_callid)
    ...
    *transfercontext = pbx_builtin_getvar_helper(sip_pvt->owner, "TRANSFER_CONTEXT");
    ...
    {
        opbx_verbose("Transfer to %s in %s\n", refer_to, *transfercontext);
        if (referred_by)
            opbx_verbose("Transfer from %s in %s\n", referred_by, sip_pvt->context);
    }
    if (!opbx_strlen_zero(replace_callid))
    {    
        /* This is a supervised transfer */
        opbx_log(OPBX_LOG_DEBUG,"Assigning Replace-Call-ID Info %s to REPLACE_CALL_ID\n",replace_callid);
        
        opbx_copy_string(sip_pvt->refer_to, "", sizeof(sip_pvt->refer_to));
        opbx_copy_string(sip_pvt->referred_by, "", sizeof(sip_pvt->referred_by));
        opbx_copy_string(sip_pvt->refer_contact, "", sizeof(sip_pvt->refer_contact));
        ...
        if ((sip_pvt_ptr = get_sip_pvt_byid_locked(replace_callid)))
        {
            ...
            {
                opbx_log(OPBX_LOG_NOTICE, "Supervised transfer attempted to transfer into same call id (%s == %s)!\n", replace_callid, sip_pvt->callid);
                sip_pvt->refer_call = NULL;
            }
            else
            {
        ...
        {
            opbx_log(OPBX_LOG_NOTICE, "Supervised transfer requested, but unable to find callid '%s'.  Both legs must reside on CallWeaver box to transfer at this time.\n", replace_callid);
    ...
    else if (opbx_exists_extension(NULL, *transfercontext, refer_to, 1, NULL) || !strcmp(refer_to, opbx_parking_ext()))
    {
        /* This is an unsupervised transfer (blind transfer) */
        
        opbx_log(OPBX_LOG_DEBUG,"Unsupervised transfer to (Refer-To): %s\n", refer_to);
        if (referred_by)
            opbx_log(OPBX_LOG_DEBUG,"Transferred by  (Referred-by: ) %s \n", referred_by);
        opbx_log(OPBX_LOG_DEBUG,"Transfer Contact Info %s (REFER_CONTACT)\n", h_contact);
        opbx_copy_string(sip_pvt->refer_to, refer_to, sizeof(sip_pvt->refer_to));
        if (referred_by)
            opbx_copy_string(sip_pvt->referred_by, referred_by, sizeof(sip_pvt->referred_by));
        if (h_contact)
        {
    ...
    else if (opbx_canmatch_extension(NULL, *transfercontext, refer_to, 1, NULL))
    {
        return 1;
    }
...
static int get_also_info(struct sip_pvt *p, struct sip_request *oreq)
{
    ...
    if (!req)
        req = &p->initreq;
    opbx_copy_string(tmp, get_header(req, "Also"), sizeof(tmp));
    ...
    if (sip_debug_test_pvt(p))
    {
        opbx_verbose("Looking for %s in %s\n", c, p->context);
    }
    if (opbx_exists_extension(NULL, p->context, c, 1, NULL))
    {
        /* This is an unsupervised transfer */
        opbx_log(OPBX_LOG_DEBUG,"Assigning Extension %s to REFER-TO\n", c);
        opbx_copy_string(p->refer_to, c, sizeof(p->refer_to));
        opbx_copy_string(p->referred_by, "", sizeof(p->referred_by));
        opbx_copy_string(p->refer_contact, "", sizeof(p->refer_contact));
        p->refer_call = NULL;
        return 0;
    }
    else if (opbx_canmatch_extension(NULL, p->context, c, 1, NULL))
    {
        return 1;
    }
...
static int check_via(struct sip_pvt *p, struct sip_request *req)
{
    ...
    opbx_copy_string(via, get_header(req, "Via"), sizeof(via));
    ...
    {
        *c = '\0';
        c = opbx_skip_blanks(c+1);
        if (strcasecmp(via, "SIP/2.0/UDP"))
        {
            opbx_log(OPBX_LOG_WARNING, "Don't know how to respond via '%s'\n", via);
            return -1;
        }
        pt = strchr(c, ':');
        if (pt)
            *pt++ = '\0';    /* remember port pointer */
        if ((hp = opbx_gethostbyname(c, &ahp)) == NULL)
        {
            opbx_log(OPBX_LOG_WARNING, "'%s' is not a valid host\n", c);
            return -1;
        }
...
static char *get_calleridname(char *input, char *output, size_t outputsize)
{
    ...
    if (tmp && tmp < end)
    {
        ...
        if (bytes > maxbytes)
            bytes = maxbytes;
        opbx_copy_string(output, tmp + 1, bytes);
    }
    else
    {
        /* we didn't find "name" */
        /* clear the empty characters in the begining*/
        input = opbx_skip_blanks(input);
        ...
        if (end >= input)
        {
            ...
            opbx_copy_string(output, input, bytes);
        }
        else
            return NULL;
    }
...
static int get_rpid_num(char *input,char *output, int maxlen)
{
    ...
    /* we found "number" */
    opbx_copy_string(output,start,maxlen);
    output[maxlen-1] = '\0';
    ...
    if (strstr(input,"privacy=full") || strstr(input,"privacy=uri"))
        return OPBX_PRES_PROHIB_USER_NUMBER_NOT_SCREENED;
...
static int check_user_full(struct sip_pvt *p, struct sip_request *req, int sipmethod, char *uri, int reliable, struct sockaddr_in *sin, int ignore, char *mailbox, int mailboxlen)
{
    ...
    struct opbx_variable *tmpvar = NULL, *v = NULL;
    char *uri2 = opbx_strdupa(uri);
    ...
    *t = '\0';
    ...
    if (opbx_strlen_zero(p->exten))
    {
        ...
        if (!strncmp(t, "sip:", 4))
            t+= 4;
        opbx_copy_string(p->exten, t, sizeof(p->exten));
        t = strchr(p->exten, '@');
        if (t)
            *t = '\0';
        if (opbx_strlen_zero(p->our_contact))
    ...
    if (user && opbx_apply_ha(user->ha, sin))
    {
        ...
        if (!opbx_strlen_zero(rpid_num) && opbx_test_flag(p, SIP_TRUSTRPID))
        {
            if (*calleridname)
                opbx_copy_string(p->cid_name, calleridname, sizeof(p->cid_name));
            opbx_copy_string(p->cid_num, rpid_num, sizeof(p->cid_num));
            opbx_shrink_phone_number(p->cid_num);
        }
        ...
        if (!(res = check_auth(p, req, p->randdata, sizeof(p->randdata), user->name, user->secret, user->md5secret, sipmethod, uri2, reliable, ignore)))
        {
            ...
            if (!opbx_strlen_zero(user->context))
                opbx_copy_string(p->context, user->context, sizeof(p->context));
            if (!opbx_strlen_zero(user->cid_num) && !opbx_strlen_zero(p->cid_num))
            {
                opbx_copy_string(p->cid_num, user->cid_num, sizeof(p->cid_num));
                opbx_shrink_phone_number(p->cid_num);
            }
            if (!opbx_strlen_zero(user->cid_name) && !opbx_strlen_zero(p->cid_num)) 
                opbx_copy_string(p->cid_name, user->cid_name, sizeof(p->cid_name));
            opbx_copy_string(p->username, user->name, sizeof(p->username));
            opbx_copy_string(p->peersecret, user->secret, sizeof(p->peersecret));
            opbx_copy_string(p->subscribecontext, user->subscribecontext, sizeof(p->subscribecontext));
            opbx_copy_string(p->peermd5secret, user->md5secret, sizeof(p->peermd5secret));
            opbx_copy_string(p->accountcode, user->accountcode, sizeof(p->accountcode));
            opbx_copy_string(p->language, user->language, sizeof(p->language));
            opbx_copy_string(p->musicclass, user->musicclass, sizeof(p->musicclass));
    ...
    else
    {
        ...
        {
            if (!mailbox && debug)
                opbx_verbose("Found user '%s', but fails host access\n", user->name);
    ...
    if (!user)
    {
        ...
        if (peer)
        {
            ...
            opbx_copy_string(p->peermd5secret, peer->md5secret, sizeof(p->peermd5secret));
            p->peermd5secret[sizeof(p->peermd5secret)-1] = '\0';
            p->callingpres = peer->callingpres;
            if (peer->maxms && peer->lastms)
                p->timer_t1 = peer->lastms;
            if (opbx_test_flag(peer, SIP_INSECURE_INVITE))
            {
            ...
            if (!(res = check_auth(p, req, p->randdata, sizeof(p->randdata), peer->name, p->peersecret, p->peermd5secret, sipmethod, uri, reliable, ignore)))
            {
                ...
                if (peer->call_limit)
                    opbx_set_flag(p, SIP_CALL_LIMIT);
                opbx_copy_string(p->peername, peer->name, sizeof(p->peername));
                opbx_copy_string(p->authname, peer->name, sizeof(p->authname));
                /* copy channel vars */
                ...
                {
                    opbx_copy_string(p->username, peer->username, sizeof(p->username));
                    /* Use the default username for authentication on outbound calls */
                    opbx_copy_string(p->authname, peer->username, sizeof(p->authname));
                }
                if (!opbx_strlen_zero(peer->cid_num) && !opbx_strlen_zero(p->cid_num))
                {
                    opbx_copy_string(p->cid_num, peer->cid_num, sizeof(p->cid_num));
                    opbx_shrink_phone_number(p->cid_num);
                }
                if (!opbx_strlen_zero(peer->cid_name) && !opbx_strlen_zero(p->cid_name)) 
                    opbx_copy_string(p->cid_name, peer->cid_name, sizeof(p->cid_name));
                opbx_copy_string(p->fullcontact, peer->fullcontact, sizeof(p->fullcontact));
                if (!opbx_strlen_zero(peer->context))
                    opbx_copy_string(p->context, peer->context, sizeof(p->context));
                opbx_copy_string(p->peersecret, peer->secret, sizeof(p->peersecret));
                opbx_copy_string(p->peermd5secret, peer->md5secret, sizeof(p->peermd5secret));
                opbx_copy_string(p->language, peer->language, sizeof(p->language));
                opbx_copy_string(p->accountcode, peer->accountcode, sizeof(p->accountcode));
        ...
        { 
            if (debug)
                opbx_verbose("Found no matching peer or user for '%s:%d'\n", opbx_inet_ntoa(iabuf, sizeof(iabuf), p->recv.sin_addr), ntohs(p->recv.sin_port));
		...
		if (global_alwaysauthreject)
            ...
            {
                opbx_copy_flags(p, &global_flags, SIP_OSPAUTH);
...
static void receive_message(struct sip_pvt *p, struct sip_request *req)
{
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "Unable to retrieve text from %s\n", p->callid);
    ...
    {
        if (sip_debug_test_pvt(p))
            opbx_verbose("Message received: '%s'\n", buf);
        memset(&f, 0, sizeof(f));
    ...
    { /* Message outside of a call, we do not support that */
        opbx_log(OPBX_LOG_WARNING,"Received message to %s from %s, dropped it...\n  Content-Type:%s\n  Message: %s\n", get_header(req,"To"), get_header(req,"From"), content_type, buf);
...
#define FORMAT2 "%-25.25s %-15.15s %-15.15s \n"
    ...
    opbx_cli(fd, FORMAT, "* User name", "In use", "Limit");
    ASTOBJ_CONTAINER_TRAVERSE(&userl, 1, do {
        ASTOBJ_RDLOCK(iterator);
        if (iterator->call_limit)
            snprintf(ilimits, sizeof(ilimits), "%d", iterator->call_limit);
        else 
            opbx_copy_string(ilimits, "N/A", sizeof(ilimits));
        snprintf(iused, sizeof(iused), "%d", iterator->inUse);
        if (showall || iterator->call_limit)
            opbx_cli(fd, FORMAT2, iterator->name, iused, ilimits);
        ASTOBJ_UNLOCK(iterator);
    } while (0) );

    opbx_cli(fd, FORMAT, "* Peer name", "In use", "Limit");
...
static int peer_status(struct sip_peer *peer, char *status, int statuslen)
{
    ...
    if (peer->maxms)
    {
        if (peer->lastms < 0)
        {
            opbx_copy_string(status, "UNREACHABLE", statuslen);
        }
        ...
        {
            opbx_copy_string(status, "UNKNOWN", statuslen);
        }
    }
    else
    { 
...
#define FORMAT  "%-25.25s  %-15.15s  %-15.15s  %-15.15s  %-5.5s%-10.10s\n"
    ...
    opbx_cli(fd, FORMAT, "Username", "Secret", "Accountcode", "Def.Context", "ACL", "NAT");
    ASTOBJ_CONTAINER_TRAVERSE(&userl, 1, do {
        ASTOBJ_RDLOCK(iterator);
        ...
        opbx_cli(fd, FORMAT, iterator->name, 
            iterator->secret, 
            iterator->accountcode,
            iterator->context,
            iterator->ha ? "Yes" : "No",
            nat2str(opbx_test_flag(iterator, SIP_NAT)));
        ASTOBJ_UNLOCK(iterator);
    } while (0)
    );
...
#define FORMAT  "%-25.25s  %-15.15s %-3.3s %-3.3s %-3.3s %-8d %-10s\n"
    ...
    ASTOBJ_CONTAINER_TRAVERSE(&peerl, 1, do {
		...
		opbx_test_flag(&iterator->flags_page2, SIP_PAGE2_DYNAMIC) ? " D " : "   ", 	/* Dynamic or not? */
        ...
        {    /* Manager format */
            ...
            idtext,
            iterator->name, 
            iterator->addr.sin_addr.s_addr ? opbx_inet_ntoa(iabuf, sizeof(iabuf), iterator->addr.sin_addr) : "-none-",
            ntohs(iterator->addr.sin_port), 
	    opbx_test_flag(&iterator->flags_page2, SIP_PAGE2_DYNAMIC) ? "yes" : "no",  /* Dynamic or not? */
    ...
    if (!s)
    {
        opbx_cli(fd,"%d sip peers [%d online , %d offline]\n",total_peers,peers_online,peers_offline);
    }
...
static int sip_show_objects(int fd, int argc, char *argv[])
{
    ...
    if (argc != 3)
        return RESULT_SHOWUSAGE;
    opbx_cli(fd, "-= User objects: %d static, %d realtime =-\n\n", suserobjs, ruserobjs);
    ASTOBJ_CONTAINER_DUMP(fd, tmp, sizeof(tmp), &userl);
    opbx_cli(fd, "-= Peer objects: %d static, %d realtime, %d autocreate =-\n\n", speerobjs, rpeerobjs, apeerobjs);
    ASTOBJ_CONTAINER_DUMP(fd, tmp, sizeof(tmp), &peerl);
    opbx_cli(fd, "-= Registry objects: %d =-\n\n", regobjs);
...
static int sip_prune_realtime(int fd, int argc, char *argv[])
{
    ...
    if (multi)
    {
        if (prunepeer)
        {
            ...
            ASTOBJ_CONTAINER_TRAVERSE(&peerl, 1, do {
                ...
                if (opbx_test_flag((&iterator->flags_page2), SIP_PAGE2_RTCACHEFRIENDS))
                {
            ...
            if (pruned)
            {
                ...
                opbx_cli(fd, "%d peers pruned.\n", pruned);
            }
            else
                opbx_cli(fd, "No peers found to prune.\n");
        ...
        if (pruneuser)
        {
            ...
            if (pruned)
            {
                ...
                opbx_cli(fd, "%d users pruned.\n", pruned);
            }
            else
                opbx_cli(fd, "No users found to prune.\n");
    ...
    else
    {
        if (prunepeer)
        {
            ...
            {
                if (!opbx_test_flag((&peer->flags_page2), SIP_PAGE2_RTCACHEFRIENDS))
                {
                    opbx_cli(fd, "Peer '%s' is not a Realtime peer, cannot be pruned.\n", name);
                    ASTOBJ_CONTAINER_LINK(&peerl, peer);
                }
                else
                    opbx_cli(fd, "Peer '%s' pruned.\n", name);
                ASTOBJ_UNREF(peer, sip_destroy_peer);
            }
            else
        ...
        if (pruneuser)
        {
            ...
            {
                if (!opbx_test_flag((&user->flags_page2), SIP_PAGE2_RTCACHEFRIENDS))
                {
                    opbx_cli(fd, "User '%s' is not a Realtime user, cannot be pruned.\n", name);
                    ASTOBJ_CONTAINER_LINK(&userl, user);
                }
                else
                    opbx_cli(fd, "User '%s' pruned.\n", name);
                ASTOBJ_UNREF(user, sip_destroy_user);
            }
            else
                opbx_cli(fd, "User '%s' not found.\n", name);
        }
    }
...
static void print_codec_to_cli(int fd, struct opbx_codec_pref *pref) 
{
    int x, codec;

    for (x = 0;  x < 32;  x++)
    {
        codec = opbx_codec_pref_index(pref, x);
        if (!codec)
            break;
...
static int sip_show_domains(int fd, int argc, char *argv[])
{
    ...
    if (OPBX_LIST_EMPTY(&domain_list))
    {
        opbx_cli(fd, "SIP Domain support not enabled.\n\n");
        return RESULT_SUCCESS;
    }
    else
    {
        opbx_cli(fd, FORMAT, "Our local SIP domains:", "Context", "Set by");
        OPBX_LIST_LOCK(&domain_list);
        OPBX_LIST_TRAVERSE(&domain_list, d, list)
            opbx_cli(fd, FORMAT, d->domain, opbx_strlen_zero(d->context) ? "(default)": d->context,
                domain_mode_to_text(d->mode));
        OPBX_LIST_UNLOCK(&domain_list);
        opbx_cli(fd, "\n");
        return RESULT_SUCCESS;
    }
}
...
static int _sip_show_peer(int type, int fd, struct mansession *s, struct message *m, int argc, char *argv[])
{
    ...
    if (peer  &&  type == 0)
    {
        ...
        opbx_cli(fd, "  MD5Secret    : %s\n", opbx_strlen_zero(peer->md5secret)?"<Not set>":"<Set>");
        auth = peer->auth;
        while (auth)
        {
            opbx_cli(fd, "  Realm-auth   : Realm %-15.15s User %-10.20s ", auth->realm, auth->username);
            opbx_cli(fd, "%s\n", !opbx_strlen_zero(auth->secret)?"<Secret set>":(!opbx_strlen_zero(auth->md5secret)?"<MD5secret set>" : "<Not set>"));
            auth = auth->next;
        }
        opbx_cli(fd, "  Context      : %s\n", peer->context);
        opbx_cli(fd, "  Subscr.Cont. : %s\n", opbx_strlen_zero(peer->subscribecontext)?"<Not set>":peer->subscribecontext);
        opbx_cli(fd, "  Language     : %s\n", peer->language);
        if (!opbx_strlen_zero(peer->accountcode))
            opbx_cli(fd, "  Accountcode  : %s\n", peer->accountcode);
        opbx_cli(fd, "  AMA flags    : %s\n", opbx_cdr_flags2str(peer->amaflags));
        opbx_cli(fd, "  CallingPres  : %s\n", opbx_describe_caller_presentation(peer->callingpres));
        if (!opbx_strlen_zero(peer->fromuser))
            opbx_cli(fd, "  FromUser     : %s\n", peer->fromuser);
        if (!opbx_strlen_zero(peer->fromdomain))
        ...
        opbx_cli(fd, "  Dynamic      : %s\n", (opbx_test_flag(&peer->flags_page2, SIP_PAGE2_DYNAMIC)?"Yes":"No"));
        opbx_cli(fd, "  Callerid     : %s\n", opbx_callerid_merge(cbuf, sizeof(cbuf), peer->cid_name, peer->cid_num, "<unspecified>"));
        opbx_cli(fd, "  Expire       : %ld\n", opbx_sched_when(sched,peer->expire));
        opbx_cli(fd, "  Insecure     : %s\n", insecure2str(opbx_test_flag(peer, SIP_INSECURE_PORT), opbx_test_flag(peer, SIP_INSECURE_INVITE)));
        opbx_cli(fd, "  Nat          : %s\n", nat2str(opbx_test_flag(peer, SIP_NAT)));
        opbx_cli(fd, "  ACL          : %s\n", (peer->ha?"Yes":"No"));
        ...
        opbx_cli(fd, "  LastMsg      : %d\n", peer->lastmsg);
        opbx_cli(fd, "  ToHost       : %s\n", peer->tohost);
        ...
        if (peer->sipoptions)
        {
            for (x = 0;  (x < (sizeof(sip_options)/sizeof(sip_options[0])));  x++)
            {
                if (peer->sipoptions & sip_options[x].id)
                    opbx_cli(fd, "%s ", sip_options[x].text);
            }
        }
        else
            opbx_cli(fd, "(none)");
    ...
    else if (peer && type == 1)
    {
        ...
        opbx_cli(fd, "Channeltype: SIP\r\n");
        if (actionid)
            opbx_cli(fd, "ActionID: %s\r\n", actionid);
        opbx_cli(fd, "ObjectName: %s\r\n", peer->name);
        opbx_cli(fd, "ChanObjectType: peer\r\n");
        opbx_cli(fd, "SecretExist: %s\r\n", opbx_strlen_zero(peer->secret)?"N":"Y");
        opbx_cli(fd, "MD5SecretExist: %s\r\n", opbx_strlen_zero(peer->md5secret)?"N":"Y");
        opbx_cli(fd, "Context: %s\r\n", peer->context);
        opbx_cli(fd, "Language: %s\r\n", peer->language);
        if (!opbx_strlen_zero(peer->accountcode))
            opbx_cli(fd, "Accountcode: %s\r\n", peer->accountcode);
        opbx_cli(fd, "AMAflags: %s\r\n", opbx_cdr_flags2str(peer->amaflags));
        opbx_cli(fd, "CID-CallingPres: %s\r\n", opbx_describe_caller_presentation(peer->callingpres));
        if (!opbx_strlen_zero(peer->fromuser))
            opbx_cli(fd, "SIP-FromUser: %s\r\n", peer->fromuser);
        if (!opbx_strlen_zero(peer->fromdomain))
        ...
        opbx_cli(fd, "LastMsgsSent: %d\r\n", peer->lastmsgssent);
        opbx_cli(fd, "Call limit: %d\r\n", peer->call_limit);
        opbx_cli(fd, "Dynamic: %s\r\n", (opbx_test_flag(&peer->flags_page2, SIP_PAGE2_DYNAMIC)?"Y":"N"));
        opbx_cli(fd, "Callerid: %s\r\n", opbx_callerid_merge(cbuf, sizeof(cbuf), peer->cid_name, peer->cid_num, ""));
        opbx_cli(fd, "RegExpire: %ld seconds\r\n", opbx_sched_when(sched,peer->expire));
        opbx_cli(fd, "SIP-AuthInsecure: %s\r\n", insecure2str(opbx_test_flag(peer, SIP_INSECURE_PORT), opbx_test_flag(peer, SIP_INSECURE_INVITE)));
        opbx_cli(fd, "SIP-NatSupport: %s\r\n", nat2str(opbx_test_flag(peer, SIP_NAT)));
        opbx_cli(fd, "ACL: %s\r\n", (peer->ha?"Y":"N"));
        opbx_cli(fd, "SIP-CanReinvite: %s\r\n", (opbx_test_flag(peer, SIP_CAN_REINVITE)?"Y":"N"));
        opbx_cli(fd, "SIP-PromiscRedir: %s\r\n", (opbx_test_flag(peer, SIP_PROMISCREDIR)?"Y":"N"));
        opbx_cli(fd, "SIP-UserPhone: %s\r\n", (opbx_test_flag(peer, SIP_USEREQPHONE)?"Y":"N"));

        /* - is enumerated */
        opbx_cli(fd, "SIP-DTMFmode %s\r\n", dtmfmode2str(opbx_test_flag(peer, SIP_DTMF)));
        opbx_cli(fd, "SIPLastMsg: %d\r\n", peer->lastmsg);
        opbx_cli(fd, "ToHost: %s\r\n", peer->tohost);
        opbx_cli(fd, "Address-IP: %s\r\nAddress-Port: %d\r\n",  peer->addr.sin_addr.s_addr ? opbx_inet_ntoa(iabuf, sizeof(iabuf), peer->addr.sin_addr) : "", ntohs(peer->addr.sin_port));
        opbx_cli(fd, "Default-addr-IP: %s\r\nDefault-addr-port: %d\r\n", opbx_inet_ntoa(iabuf, sizeof(iabuf), peer->defaddr.sin_addr), ntohs(peer->defaddr.sin_port));
        opbx_cli(fd, "Default-Username: %s\r\n", peer->username);
        opbx_cli(fd, "Codecs: ");
        opbx_getformatname_multiple(codec_buf, sizeof(codec_buf) -1, peer->capability);
        opbx_cli(fd, "%s\r\n", codec_buf);
        opbx_cli(fd, "CodecOrder: ");
        pref = &peer->prefs;
        for (x = 0;  x < 32;  x++)
        {
            codec = opbx_codec_pref_index(pref,x);
        ...
        peer_status(peer, status, sizeof(status));
        opbx_cli(fd, "%s\r\n", status);
         opbx_cli(fd, "SIP-Useragent: %s\r\n", peer->useragent);
         opbx_cli(fd, "Reg-Contact : %s\r\n", peer->fullcontact);
        if (peer->chanvars)
        {
            for (v = peer->chanvars;  v;  v = v->next)
            {
                opbx_cli(fd, "ChanVariable:\n");
                opbx_cli(fd, " %s,%s\r\n", v->name, v->value);
            }
        }
...
static int sip_show_user(int fd, int argc, char *argv[])
{
    ...
    if (user)
    {
        opbx_cli(fd,"\n\n");
        opbx_cli(fd, "  * Name       : %s\n", user->name);
        opbx_cli(fd, "  Secret       : %s\n", opbx_strlen_zero(user->secret)?"<Not set>":"<Set>");
        opbx_cli(fd, "  MD5Secret    : %s\n", opbx_strlen_zero(user->md5secret)?"<Not set>":"<Set>");
        opbx_cli(fd, "  Context      : %s\n", user->context);
        opbx_cli(fd, "  Language     : %s\n", user->language);
        if (!opbx_strlen_zero(user->accountcode))
            opbx_cli(fd, "  Accountcode  : %s\n", user->accountcode);
        opbx_cli(fd, "  AMA flags    : %s\n", opbx_cdr_flags2str(user->amaflags));
...
#define FORMAT  "%-30.30s  %-12.12s  %8d %-20.20s\n"
    ...
    if (argc != 3)
        return RESULT_SHOWUSAGE;
    opbx_cli(fd, FORMAT2, "Host", "Username", "Refresh", "State");
    ASTOBJ_CONTAINER_TRAVERSE(&regl, 1, do {
        ASTOBJ_RDLOCK(iterator);
        snprintf(host, sizeof(host), "%s:%d", iterator->hostname, iterator->portno ? iterator->portno : DEFAULT_SIP_PORT);
        opbx_cli(fd, FORMAT, host, iterator->username, iterator->refresh, regstate2str(iterator->regstate));
        ASTOBJ_UNLOCK(iterator);
    } while(0));
    return RESULT_SUCCESS;
#undef FORMAT
...
static int sip_show_settings(int fd, int argc, char *argv[])
{
    ...
    realtimepeers = opbx_check_realtime("sippeers");
    realtimeusers = opbx_check_realtime("sipusers");
    ...
    opbx_cli(fd, "  SIP domain support:     %s\n", OPBX_LIST_EMPTY(&domain_list) ? "No" : "Yes");
    opbx_cli(fd, "  Call to non-local dom.: %s\n", allow_external_domains ? "Yes" : "No");
    opbx_cli(fd, "  URI user is phone no:   %s\n", opbx_test_flag(&global_flags, SIP_USEREQPHONE) ? "Yes" : "No");
    opbx_cli(fd, "  Our auth realm          %s\n", global_realm);
    opbx_cli(fd, "  Realm. auth:            %s\n", authl ? "Yes": "No");
    opbx_cli(fd, "  User Agent:             %s\n", default_useragent);
    opbx_cli(fd, "  MWI checking interval:  %d secs\n", global_mwitime);
    opbx_cli(fd, "  Reg. context:           %s\n", opbx_strlen_zero(regcontext) ? "(not set)" : regcontext);
    opbx_cli(fd, "  Caller ID:              %s\n", default_callerid);
    opbx_cli(fd, "  From: Domain:           %s\n", default_fromdomain);
    opbx_cli(fd, "  Record SIP history:     %s\n", recordhistory ? "On" : "Off");
    opbx_cli(fd, "  Call Events:            %s\n", callevents ? "On" : "Off");
    opbx_cli(fd, "  IP ToS:                 0x%x\n", tos);
#ifdef OSP_SUPPORT
    opbx_cli(fd, "  OSP Support:            Yes\n");
#else
    opbx_cli(fd, "  OSP Support:            No\n");
#endif
    if (!realtimepeers && !realtimeusers)
        opbx_cli(fd, "  SIP realtime:           Disabled\n" );
    else
        opbx_cli(fd, "  SIP realtime:           Enabled\n" );
    ...
    opbx_cli(fd, "\n");
    opbx_cli(fd, "  Relax DTMF:             %s\n", relaxdtmf ? "Yes" : "No");
    opbx_cli(fd, "  Compact SIP headers:    %s\n", compactheaders ? "Yes" : "No");
    opbx_cli(fd, "  RTP Timeout:            %d %s\n", global_rtptimeout, global_rtptimeout ? "" : "(Disabled)" );
    opbx_cli(fd, "  RTP Hold Timeout:       %d %s\n", global_rtpholdtimeout, global_rtpholdtimeout ? "" : "(Disabled)");
    opbx_cli(fd, "  MWI NOTIFY mime type:   %s\n", default_notifymime);
    opbx_cli(fd, "  DNS SRV lookup:         %s\n", srvlookup ? "Yes" : "No");
    opbx_cli(fd, "  Pedantic SIP support:   %s\n", pedanticsipchecking ? "Yes" : "No");
    opbx_cli(fd, "  Reg. max duration:      %d secs\n", max_expiry);
    opbx_cli(fd, "  Reg. default duration:  %d secs\n", default_expiry);
    opbx_cli(fd, "  Outbound reg. timeout:  %d secs\n", global_reg_timeout);
    opbx_cli(fd, "  Outbound reg. attempts: %d\n", global_regattempts_max);
    opbx_cli(fd, "  Notify ringing state:   %s\n", global_notifyringing ? "Yes" : "No");
    opbx_cli(fd, "\nDefault Settings:\n");
    opbx_cli(fd, "-----------------\n");
    opbx_cli(fd, "  Context:                %s\n", default_context);
    opbx_cli(fd, "  Nat:                    %s\n", nat2str(opbx_test_flag(&global_flags, SIP_NAT)));
    opbx_cli(fd, "  DTMF:                   %s\n", dtmfmode2str(opbx_test_flag(&global_flags, SIP_DTMF)));
    opbx_cli(fd, "  Qualify:                %d\n", default_qualify);
    opbx_cli(fd, "  Use ClientCode:         %s\n", opbx_test_flag(&global_flags, SIP_USECLIENTCODE) ? "Yes" : "No");
    opbx_cli(fd, "  Progress inband:        %s\n", (opbx_test_flag(&global_flags, SIP_PROG_INBAND) == SIP_PROG_INBAND_NEVER) ? "Never" : (opbx_test_flag(&global_flags, SIP_PROG_INBAND) == SIP_PROG_INBAND_NO) ? "No" : "Yes" );
    opbx_cli(fd, "  Language:               %s\n", opbx_strlen_zero(default_language) ? "(Defaults to English)" : default_language);
    opbx_cli(fd, "  Musicclass:             %s\n", global_musicclass);
    opbx_cli(fd, "  Voice Mail Extension:   %s\n", global_vmexten);
...
#define FORMAT  "%-15.15s  %-10.10s  %-11.11s  %5.5d/%5.5d  %-4.4s  %-3.3s %-3.3s  %-15.15s\n"
    ...
    cur = iflist;
    if (!subscriptions)
        opbx_cli(fd, FORMAT2, "Peer", "User/ANR", "Call ID", "Seq (Tx/Rx)", "Format", "Hold", "Last Message");
    else
        opbx_cli(fd, FORMAT3, "Peer", "User", "Call ID", "Extension", "Last state", "Type");
    while (cur)
    {
        if (cur->subscribed == NONE && !subscriptions)
        {
            opbx_cli(fd, FORMAT, opbx_inet_ntoa(iabuf, sizeof(iabuf), cur->sa.sin_addr), 
                opbx_strlen_zero(cur->username) ? ( opbx_strlen_zero(cur->cid_num) ? "(None)" : cur->cid_num ) : cur->username, 
                cur->callid, 
                cur->ocseq, cur->icseq, 
                opbx_getformatname(cur->owner ? cur->owner->nativeformats : 0), 
                opbx_test_flag(cur, SIP_CALL_ONHOLD) ? "Yes" : "No",
                opbx_test_flag(cur, SIP_NEEDDESTROY) ? "(d)" : "",
                cur->lastmsg );
            numchans++;
        }
        if (cur->subscribed != NONE && subscriptions)
        {
            opbx_cli(fd, FORMAT3, opbx_inet_ntoa(iabuf, sizeof(iabuf), cur->sa.sin_addr),
                    opbx_strlen_zero(cur->username) ? ( opbx_strlen_zero(cur->cid_num) ? "(None)" : cur->cid_num ) : cur->username, 
                    cur->callid, cur->exten, opbx_extension_state2str(cur->laststate), 
    ...
    opbx_mutex_unlock(&iflock);
    if (!subscriptions)
        opbx_cli(fd, "%d active SIP channel%s\n", numchans, (numchans != 1) ? "s" : "");
    else
        opbx_cli(fd, "%d active SIP subscription%s\n", numchans, (numchans != 1) ? "s" : "");
    return RESULT_SUCCESS;
#undef FORMAT
#undef FORMAT2
#undef FORMAT3
...
static char *complete_sip_peer(char *word, int state, int flags2)
{
    ...
    ASTOBJ_CONTAINER_TRAVERSE(&peerl, !result, do {
        ...
        {
            if (flags2 && !opbx_test_flag((&iterator->flags_page2), flags2))
                continue;
            if (++which > state)
            {
...
static char *complete_sipnotify(char *line, char *word, int pos, int state)
{
    ...
    if (pos == 2)
    {
        ...
        cat = opbx_category_browse(notify_types, NULL);
        while (cat)
        {
            ...
            cat = opbx_category_browse(notify_types, cat);
        }
        return c;
    }
...
static int sip_show_channel(int fd, int argc, char *argv[])
{
    ...
    while (cur)
    {
        if (!strncasecmp(cur->callid, argv[3],len))
        {
            ...
            if (cur->subscribed != NONE)
                opbx_cli(fd, "  * Subscription (type: %s)\n", subscription_type2str(cur->subscribed));
            else
                opbx_cli(fd, "  * SIP Call\n");
            opbx_cli(fd, "  Direction:              %s\n", opbx_test_flag(cur, SIP_OUTGOING)?"Outgoing":"Incoming");
            opbx_cli(fd, "  Call-ID:                %s\n", cur->callid);
            opbx_cli(fd, "  Our Codec Capability:   %d\n", cur->capability);
            opbx_cli(fd, "  Non-Codec Capability:   %d\n", cur->noncodeccapability);
            opbx_cli(fd, "  Their Codec Capability:   %d\n", cur->peercapability);
            opbx_cli(fd, "  Joint Codec Capability:   %d\n", cur->jointcapability);
            opbx_cli(fd, "  Format                  %s\n", opbx_getformatname(cur->owner ? cur->owner->nativeformats : 0) );
            opbx_cli(fd, "  Theoretical Address:    %s:%d\n", opbx_inet_ntoa(iabuf, sizeof(iabuf), cur->sa.sin_addr), ntohs(cur->sa.sin_port));
            ...
            if (!opbx_strlen_zero(cur->peername))
                opbx_cli(fd, "  Peername:               %s\n", cur->peername);
            if (!opbx_strlen_zero(cur->uri))
                opbx_cli(fd, "  Original uri:           %s\n", cur->uri);
            if (!opbx_strlen_zero(cur->cid_num))
                opbx_cli(fd, "  Caller-ID:              %s\n", cur->cid_num);
            opbx_cli(fd, "  Need Destroy:           %d\n", opbx_test_flag(cur, SIP_NEEDDESTROY));
    ...
    opbx_mutex_unlock(&iflock);
    if (!found) 
        opbx_cli(fd, "No such SIP Call ID starting with '%s'\n", argv[3]);
    return RESULT_SUCCESS;
}
...
static int sip_show_history(int fd, int argc, char *argv[])
{
    ...
    while (cur)
    {
        if (!strncasecmp(cur->callid, argv[3], len))
        {
            ...
            if (!x)
                opbx_cli(fd, "Call '%s' has no history\n", cur->callid);
            found++;
        }
        cur = cur->next;
    }
...
void sip_dump_history(struct sip_pvt *dialog)
{
    ...
    if (dialog->subscribed)
        opbx_log(OPBX_LOG_DEBUG, "  * Subscription\n");
    else
        opbx_log(OPBX_LOG_DEBUG, "  * SIP Call\n");
    x = 0;
    hist = dialog->history;
    while (hist)
    {
        x++;
        opbx_log(OPBX_LOG_DEBUG, "  %d. %s\n", x, hist->event);
        hist = hist->next;
    }
    if (!x)
        opbx_log(OPBX_LOG_DEBUG, "Call '%s' has no history\n", dialog->callid);
...
static void handle_request_info(struct sip_pvt *p, struct sip_request *req)
{
        ...
        !strcasecmp(get_header(req, "Content-Type"), "application/vnd.nortelnetworks.digits"))
    {
        ...
        {
            opbx_log(OPBX_LOG_WARNING, "Unable to retrieve DTMF signal from INFO message from %s\n", p->callid);
            transmit_response(p, "200 OK", req); /* Should return error */
            return;
        }
        else
        {
            opbx_copy_string(buf, c, sizeof(buf));
        }
    ...
    else if (!strcasecmp(get_header(req, "Content-Type"), "application/media_control+xml"))
    {
        ...
        if (p->owner)
            opbx_queue_control(p->owner, OPBX_CONTROL_VIDUPDATE);
        transmit_response(p, "200 OK", req);
        return;
    }
    else if ((c = get_header(req, "X-ClientCode")))
    {
        ...
        if (opbx_test_flag(p, SIP_USECLIENTCODE))
        {
            if (p->owner && p->owner->cdr)
                opbx_cdr_setuserfield(p->owner, c);
            if (p->owner && opbx_bridged_channel(p->owner) && opbx_bridged_channel(p->owner)->cdr)
    ...
    opbx_log(OPBX_LOG_WARNING, "Unable to parse INFO message from %s. Content %s\n", p->callid, buf);
...
static int sip_do_debug_ip(int fd, int argc, char *argv[])
{
    ...
    if (port == 0)
        opbx_cli(fd, "SIP Debugging Enabled for IP: %s\n", opbx_inet_ntoa(iabuf, sizeof(iabuf), debugaddr.sin_addr));
    else
        opbx_cli(fd, "SIP Debugging Enabled for IP: %s:%d\n", opbx_inet_ntoa(iabuf, sizeof(iabuf), debugaddr.sin_addr), port);
...
static int sip_do_debug_peer(int fd, int argc, char *argv[])
{
    ...
    if (peer)
    {
        if (peer->addr.sin_addr.s_addr)
        {
            ...
            opbx_cli(fd, "SIP Debugging Enabled for IP: %s:%d\n", opbx_inet_ntoa(iabuf, sizeof(iabuf), debugaddr.sin_addr), ntohs(debugaddr.sin_port));
            sipdebug |= SIP_DEBUG_CONSOLE;
        }
        else
            opbx_cli(fd, "Unable to get IP address of peer '%s'\n", argv[3]);
        ASTOBJ_UNREF(peer,sip_destroy_peer);
    }
    else
        opbx_cli(fd, "No such peer '%s'\n", argv[3]);
    return RESULT_SUCCESS;
}
...
static int sip_do_debug(int fd, int argc, char *argv[])
{
    ...
    if (oldsipdebug)
        opbx_cli(fd, "SIP Debugging re-enabled\n");
    else
        opbx_cli(fd, "SIP Debugging enabled\n");
    return RESULT_SUCCESS;
}
...
static int sip_notify(int fd, int argc, char *argv[])
{
    ...
    if (!notify_types)
    {
        opbx_cli(fd, "No %s file found, or no types listed there\n", notify_config);
        return RESULT_FAILURE;
    }

    varlist = opbx_variable_browse(notify_types, argv[2]);

    if (!varlist)
    {
        opbx_cli(fd, "Unable to find notify type '%s'\n", argv[2]);
        return RESULT_FAILURE;
    }
    ...
    for (i = 3;  i < argc;  i++)
    {
        ...
        if (!p)
        {
            opbx_log(OPBX_LOG_WARNING, "Unable to build sip pvt data for notify\n");
            return RESULT_FAILURE;
        }
        ...
        if (create_addr(p, argv[i]))
        {
            ...
            sip_destroy(p);
            opbx_cli(fd, "Could not create address for '%s'\n", argv[i]);
            continue;
        }
        ...
        build_via(p, p->via, sizeof(p->via));
        build_callid(p->callid, sizeof(p->callid), p->ourip, p->fromdomain);
        opbx_cli(fd, "Sending NOTIFY of type '%s' to '%s'\n", argv[2], argv[i]);
...
static int sip_do_history(int fd, int argc, char *argv[])
{
    ...
    recordhistory = 1;
    opbx_cli(fd, "SIP History Recording Enabled (use 'sip show history')\n");
    return RESULT_SUCCESS;
}
...
static int sip_no_history(int fd, int argc, char *argv[])
{
    ...
    recordhistory = 0;
    opbx_cli(fd, "SIP History Recording Disabled\n");
    return RESULT_SUCCESS;
}
...

{
    if (argc != 3)
        return RESULT_SHOWUSAGE;
    sipdebug &= ~SIP_DEBUG_CONSOLE;
    opbx_cli(fd, "SIP Debugging Disabled\n");
    return RESULT_SUCCESS;
}
...
static int do_register_auth(struct sip_pvt *p, struct sip_request *req, char *header, char *respheader) 
{
    ...
    if (reply_digest(p, req, header, SIP_REGISTER, digest, sizeof(digest)))
    {
        /* There's nothing to use for authentication */
         ...
         if (sip_debug_test_pvt(p) && p->registry)
             opbx_verbose("No authentication challenge, sending blank registration to domain/host name %s\n", p->registry->hostname);
             /* No old challenge */
        return -1;
    }
    if (recordhistory)
    {
     ...
     if (sip_debug_test_pvt(p) && p->registry)
         opbx_verbose("Responding to challenge, registration to domain/host name %s\n", p->registry->hostname);
...
static int do_proxy_auth(struct sip_pvt *p, struct sip_request *req, char *header, char *respheader, int sipmethod, int init) 
{
    ...
    p->authtries++;
    if (option_debug > 1)
        opbx_log(OPBX_LOG_DEBUG, "Auth attempt %d on %s\n", p->authtries, sip_methods[sipmethod].text);
    memset(digest, 0, sizeof(digest));
    ...
    char *header, int sipmethod,  char *digest, int digest_len)
{
    ...
    opbx_copy_string(tmp, get_header(req, header), sizeof(tmp));
    if (opbx_strlen_zero(tmp)) 
        return -1;
    if (strncasecmp(tmp, "Digest ", strlen("Digest ")))
    {
        opbx_log(OPBX_LOG_WARNING, "missing Digest.\n");
        return -1;
    }
    c = tmp + strlen("Digest ");
    for (i = keys; i->key != NULL; i++)
        i->dst[0] = '\0';    /* init all to empty strings */
    opbx_copy_string(oldnonce, p->nonce, sizeof(oldnonce));
    while (c && *(c = opbx_skip_blanks(c)))
    {
        /* lookup for keys */
        for (i = keys; i->key != NULL; i++)
        {
            ...
            opbx_copy_string(i->dst, src, i->dstlen);
            break;
        }
    ...
    if (p->registry)
    {
        ...
        {
            opbx_copy_string(r->realm, p->realm, sizeof(r->realm));
            opbx_copy_string(r->nonce, p->nonce, sizeof(r->nonce));
            opbx_copy_string(r->domain, p->domain, sizeof(r->domain));
            opbx_copy_string(r->opaque, p->opaque, sizeof(r->opaque));
            opbx_copy_string(r->qop, p->qop, sizeof(r->qop));
            r->noncecount = 0;
        }
    }
...
static int build_reply_digest(struct sip_pvt *p, int method, char* digest, int digest_len)
{
    ...
    if (!opbx_strlen_zero(p->domain))
        opbx_copy_string(uri, p->domain, sizeof(uri));
    else if (!opbx_strlen_zero(p->uri))
        opbx_copy_string(uri, p->uri, sizeof(uri));
    else
    ...
    snprintf(cnonce, sizeof(cnonce), "%08x", thread_safe_opbx_random());
     ...
     if ((auth = find_realm_authentication(authl, p->realm)))
     {
        ...
        if (sipdebug)
             opbx_log(OPBX_LOG_DEBUG,"Using realm %s authentication for call %s\n", p->realm, p->callid);
     }
     else
     {
    ...
    if (!opbx_strlen_zero(md5secret))
        opbx_copy_string(a1_hash, md5secret, sizeof(a1_hash));
    else
        opbx_md5_hash(a1_hash,a1);
    opbx_md5_hash(a2_hash,a2);

    p->noncecount++;
    if (!opbx_strlen_zero(p->qop))
...
static int function_sippeer(struct opbx_channel *chan, int argc, char **argv, char *buf, size_t len)
{
    ...
    {
        opbx_copy_string(buf, peer->language, len);
    }
    else  if (!strcasecmp(colname, "regexten"))
    {
        opbx_copy_string(buf, peer->regexten, len);
    }
    ...
    {
        opbx_copy_string(buf, peer->useragent, len);
    }
    ...
    {
        opbx_copy_string(buf, (opbx_test_flag(&peer->flags_page2, SIP_PAGE2_DYNAMIC) ? "yes" : "no"), len);
    }
...
static void parse_moved_contact(struct sip_pvt *p, struct sip_request *req)
{
    char tmp[256];
    char *s, *e;
    opbx_copy_string(tmp, get_header(req, "Contact"), sizeof(tmp));
    s = get_in_brackets(tmp);
    e = strchr(s, ';');
    if (e)
        *e = '\0';
    if (opbx_test_flag(p, SIP_PROMISCREDIR))
    {
        ...
        if (e)
            *e = '\0';
        opbx_log(OPBX_LOG_DEBUG, "Found promiscuous redirection to 'SIP/%s'\n", s);
        if (p->owner)
    ...
    else
    {
        ...
        if (!strncasecmp(s, "sip:", 4))
            s += 4;
        opbx_log(OPBX_LOG_DEBUG, "Found 302 Redirect to extension '%s'\n", s);
        if (p->owner)
            opbx_copy_string(p->owner->call_forward, s, sizeof(p->owner->call_forward));
    }
}
...
static void check_pendings(struct sip_pvt *p)
{
    ...
    if (opbx_test_flag(p, SIP_PENDINGBYE))
    {
	...
	if (!opbx_test_flag(p, SIP_CAN_BYE))
	    ...
	    transmit_request_with_auth(p, SIP_BYE, 0, 1, 1);
	opbx_clear_flag(p, SIP_PENDINGBYE);	
	sip_scheddestroy(p, 32000);
        //transmit_request_with_auth(p, SIP_BYE, 0, 1, 1);
        //opbx_set_flag(p, SIP_NEEDDESTROY);    
        //opbx_clear_flag(p, SIP_NEEDREINVITE);    
    }
    else if (opbx_test_flag(p, SIP_NEEDREINVITE))
    {
        opbx_log(OPBX_LOG_DEBUG, "Sending pending reinvite on '%s'\n", p->callid);
...
{
    int outgoing = opbx_test_flag(p, SIP_OUTGOING);
    
    if (option_debug > 3)
    {
        int reinvite = (p->owner && p->owner->_state == OPBX_STATE_UP);
        if (reinvite)
            opbx_log(OPBX_LOG_DEBUG, "SIP response %d to RE-invite on %s call %s\n", resp, outgoing ? "outgoing" : "incoming", p->callid);
        else
            opbx_log(OPBX_LOG_DEBUG, "SIP response %d to standard invite\n", resp);
    }

    if (opbx_test_flag(p, SIP_ALREADYGONE))
    {
        /* This call is already gone */
        opbx_log(OPBX_LOG_DEBUG, "Got response on call that is already terminated: %s (ignoring)\n", p->callid);
        return;
    }
    ...
    */
    ...
    case 180:
        ...
        if (!ignore  &&  p->owner)
        {
            opbx_queue_control(p->owner, OPBX_CONTROL_RINGING);
...
//        if (!strcasecmp(get_header(req, "Content-Type"), "application/sdp"))
    ...
    case 200:    /* 200 OK on invite - someone's answering our call */
        ...
        if (p->owner && (p->owner->_state == OPBX_STATE_UP))
        {
            ...
            if ((bridgepeer = opbx_bridged_channel(p->owner)))
            {
                ...
                else
                {
                        ...
                        opbx_log(OPBX_LOG_WARNING, "Strange... The other side of the bridge is not a SIP channel\n");
                        p->t38state = SIP_T38_STATUS_UNKNOWN;
                        opbx_log(OPBX_LOG_DEBUG, "T38 state changed to %d on channel %s\n",p->t38state, p->owner ? p->owner->name : "<none>");
                }
            }
            ...
            {
                opbx_log(OPBX_LOG_DEBUG, "Channel Bridge information is non-existant. T38 Termination requested.\n");
            }
        }
        ...
        if (!ignore  &&  p->owner)
        {
            if (p->owner->_state != OPBX_STATE_UP)
            {
                ...
                time(&p->ospstart);
#endif
                opbx_queue_control(p->owner, OPBX_CONTROL_ANSWER);
            }
            else
            {
                /* RE-invite */
    ...
    case 401: /* Www auth */
        ...
        if (!ignore)
        {
            ...
            {
                opbx_log(OPBX_LOG_NOTICE, "Failed to authenticate on INVITE to '%s'\n", get_header(&p->initreq, "From"));
                opbx_set_flag(p, SIP_NEEDDESTROY);    
                opbx_set_flag(p, SIP_ALREADYGONE);    
                if (p->owner)
                    opbx_queue_control(p->owner, OPBX_CONTROL_CONGESTION);
    ...
    case 403: /* Forbidden */
        /* First we ACK */
        transmit_request(p, SIP_ACK, seqno, 0, 0);
        opbx_log(OPBX_LOG_WARNING, "Forbidden - wrong password on authentication for INVITE to '%s'\n", get_header(&p->initreq, "From"));
        if (!ignore && p->owner)
            opbx_queue_control(p->owner, OPBX_CONTROL_CONGESTION);
        opbx_set_flag(p, SIP_NEEDDESTROY);    
        opbx_set_flag(p, SIP_ALREADYGONE);    
    ...
    case 481: /* Call leg does not exist */
        ...
        opbx_log(OPBX_LOG_WARNING, "Re-invite to non-existing call leg on other UA. SIP dialog '%s'. Giving up.\n", p->callid);
        transmit_request(p, SIP_ACK, seqno, 0, 0);
        break;
...
static int handle_response_register(struct sip_pvt *p, int resp, char *rest, struct sip_request *req, int ignore, int seqno)
{
    ...
    case 401:
        ...
        {
            opbx_log(OPBX_LOG_NOTICE, "Failed to authenticate on REGISTER to '%s@%s' (Tries %d)\n", p->registry->username, p->registry->hostname, p->authtries);
            opbx_set_flag(p, SIP_NEEDDESTROY);    
        }
        break;
    case 403:
    ...
    case 404:
        /* Not found */
        opbx_log(OPBX_LOG_WARNING, "Got 404 Not found on SIP register to service %s@%s, giving up\n", p->registry->username,p->registry->hostname);
        if (global_regattempts_max)
            p->registry->regattempts = global_regattempts_max+1;
        opbx_set_flag(p, SIP_NEEDDESTROY);    
        r->call = NULL;
    ...
    case 407:
        ...
        {
            opbx_log(OPBX_LOG_NOTICE, "Failed to authenticate on REGISTER to '%s' (tries '%d')\n", get_header(&p->initreq, "From"), p->authtries);
            opbx_set_flag(p, SIP_NEEDDESTROY);    
        }
        break;
    case 479:
        /* SER: Not able to process the URI - address is wrong in register*/
        opbx_log(OPBX_LOG_WARNING, "Got error 479 on register to %s@%s, giving up (check config)\n", p->registry->username,p->registry->hostname);
        if (global_regattempts_max)
            p->registry->regattempts = global_regattempts_max+1;
        opbx_set_flag(p, SIP_NEEDDESTROY);    
        r->call = NULL;
    ...
    case 200:
        ...
        if (!r)
        {
            opbx_log(OPBX_LOG_WARNING, "Got 200 OK on REGISTER that isn't a register\n");
            opbx_set_flag(p, SIP_NEEDDESTROY);    
            return 0;
        }
        ...
        if (r->expire > -1) {
            opbx_sched_del(sched, r->expire);
	    r->expire = -1;
	}
        /* according to section 6.13 of RFC, contact headers override
           expires headers, so check those first */
        expires = 0;
        if (!opbx_strlen_zero(get_header(req, "Contact")))
        {
            char *contact = NULL;
            char *tmptmp = NULL;
            int start = 0;
        ...
        if (sipdebug)
            opbx_log(OPBX_LOG_NOTICE, "Outbound Registration: Expiry for %s is %d sec (Scheduling reregistration in %d s)\n", r->hostname, expires, expires_ms/1000); 
...
static int handle_response_peerpoke(struct sip_pvt *p, int resp, char *rest, struct sip_request *req, int ignore, int seqno, int sipmethod)
{
    ...
    if (resp != 100)
    {
        ...
        if ((peer->lastms < 0)  || (peer->lastms > peer->maxms))
        {
            ...
            {
		if (option_verbose > 3)
			opbx_log(OPBX_LOG_NOTICE, "Peer '%s' is now REACHABLE! (%dms / %dms)\n", peer->name, pingtime, peer->maxms);
                statechanged = 1;
                newstate = 1;
            }
        }
        else if ((peer->lastms > 0) && (peer->lastms <= peer->maxms))
        {
            ...
            {
		if (option_verbose > 3)
			opbx_log(OPBX_LOG_NOTICE, "Peer '%s' is now TOO LAGGED! (%dms / %dms)\n", peer->name, pingtime, peer->maxms);
                statechanged = 1;
                newstate = 2;
            }
        }
...
static void handle_response(struct sip_pvt *p, int resp, char *rest, struct sip_request *req, int ignore, int seqno)
{
    ...
    if (opbx_strlen_zero(p->theirtag) || (resp >= 200))
    {
    ...
    else if (opbx_test_flag(p, SIP_OUTGOING))
    {
        ...
        case 200:    /* 200 OK */
            ...
            else if (sipmethod == SIP_NOTIFY)
            {
                ...
                if (p->owner)
                {
                    opbx_log(OPBX_LOG_WARNING, "Notify answer on an owned channel?\n");
                    opbx_queue_hangup(p->owner);
                }
                else
                {
        ...
        case 401: /* Not www-authorized on SIP method */
            ...
            {
                opbx_log(OPBX_LOG_WARNING, "Got authentication request (401) on unknown %s to '%s'\n", sip_methods[sipmethod].text, get_header(req, "To"));
                opbx_set_flag(p, SIP_NEEDDESTROY);    
            }
            break;
        case 403: /* Forbidden - we failed authentication */
            ...
            {
                opbx_log(OPBX_LOG_WARNING, "Forbidden - wrong password on authentication for %s\n", msg);
            }
            break;
        case 404: /* Not found */
        ...
        case 407: /* Proxy auth required */
            ...
            {
                if (opbx_strlen_zero(p->authname))
                    opbx_log(OPBX_LOG_WARNING, "Asked to authenticate %s, to %s:%d but we have no matching peer!\n",
                            msg, opbx_inet_ntoa(iabuf, sizeof(iabuf), p->recv.sin_addr), ntohs(p->recv.sin_port));
                    opbx_set_flag(p, SIP_NEEDDESTROY);    
                if ((p->authtries == MAX_AUTHTRIES) || do_proxy_auth(p, req, "Proxy-Authenticate", "Proxy-Authorization", sipmethod, 0))
                {
                    opbx_log(OPBX_LOG_NOTICE, "Failed to authenticate on %s to '%s'\n", msg, get_header(&p->initreq, "From"));
                    opbx_set_flag(p, SIP_NEEDDESTROY);    
                }
            }
            else if (p->registry && sipmethod == SIP_REGISTER)
	...
	case 487:
        ...
        case 491: /* Pending */
            if (sipmethod == SIP_INVITE)
                ...
                opbx_log(OPBX_LOG_WARNING, "Host '%s' does not implement '%s'\n", opbx_inet_ntoa(iabuf, sizeof(iabuf), p->sa.sin_addr), msg);
            break;
        case 501: /* Not Implemented */
            if (sipmethod == SIP_INVITE)
                handle_response_invite(p, resp, rest, req, ignore, seqno);
        ...
        default:
            if ((resp >= 300) && (resp < 700))
            {
                if ((option_verbose > 2) && (resp != 487))
                    opbx_verbose(VERBOSE_PREFIX_3 "Got SIP response %d \"%s\" back from %s\n", resp, rest, opbx_inet_ntoa(iabuf, sizeof(iabuf), p->sa.sin_addr));
                if (p->rtp)
                {
                ...
                case 482: /* SIP is incapable of performing a hairpin call, which
                    ...
                    opbx_log(OPBX_LOG_DEBUG, "Hairpin detected, setting up call forward for what it's worth\n");
                    if (p->owner)
            ...
            else if ((resp >= 100) && (resp < 200))
            {
                if (sipmethod == SIP_INVITE)
                {
                    ...
                    if (!opbx_strlen_zero(get_header(req, "Content-Type")))
                        process_sdp(p, req);
                    if (p->owner)
                    {
                        /* Queue a progress frame */
                ...
                opbx_log(OPBX_LOG_NOTICE, "Dont know how to handle a %d %s response from %s\n", resp, rest, p->owner ? p->owner->name : opbx_inet_ntoa(iabuf, sizeof(iabuf), p->sa.sin_addr));
        }
    }
    else
    {    
        ...
        if (req->debug)
            opbx_verbose("SIP Response message for INCOMING dialog %s arrived\n", msg);
        if (resp == 200)
        {
        ...
        case 200:
            ...
            {
                opbx_log(OPBX_LOG_DEBUG, "Got 200 OK on CANCEL\n");
            }
        ...
        default:    /* Errors without handlers */
            ...
            {
                if ((option_verbose > 2) && (resp != 487))
                    opbx_verbose(VERBOSE_PREFIX_3 "Incoming call: Got SIP response %d \"%s\" back from %s\n", resp, rest, opbx_inet_ntoa(iabuf, sizeof(iabuf), p->sa.sin_addr));
                switch (resp)
                {
                case 488: /* Not acceptable here - codec error */
                case 603: /* Decline */
...
static void *sip_park_thread(void *stuff)
{
    ...
    free(d);
    opbx_mutex_lock(&chan1->lock);
    opbx_do_masquerade(chan1);
    opbx_mutex_unlock(&chan1->lock);
    res = opbx_park_call(chan1, chan2, 0, &ext);
    /* Then hangup */
    opbx_hangup(chan2);
    opbx_log(OPBX_LOG_DEBUG, "Parked on extension '%d'\n", ext);
    return NULL;
}

/*! \brief  sip_park: Park a call */
static int sip_park(struct opbx_channel *chan1, struct opbx_channel *chan2, struct sip_request *req)
{
    struct sip_dual *d;
    ...
    if (d)
    {
        ...
        d->chan1 = chan1m;
        d->chan2 = chan2m;
        if (!opbx_pthread_create(&th, &global_attr_detached, sip_park_thread, d))
            return 0;
        free(d);
    }
    return -1;
...
static int attempt_transfer(struct sip_pvt *p1, struct sip_pvt *p2)
{
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "Transfer attempted without dual ownership?\n");
        return -1;
    }
    chana = p1->owner;
    chanb = p2->owner;
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "Neither bridgea nor bridgeb?\n");
    }
    ...
    if (peera && peerb && peerc && (peerb != peerc))
    {
        ...
        {
            peerb->cdr = opbx_cdr_append(peerb->cdr, peerc->cdr);
        }
        else if (peerc->cdr)
        {
            peerb->cdr = peerc->cdr;
        ...
        if (opbx_channel_masquerade(peerb, peerc))
        {
    ...
    {
        opbx_log(OPBX_LOG_NOTICE, "Transfer attempted with no appropriate bridged calls to transfer\n");
        if (chana)
            opbx_softhangup_nolock(chana, OPBX_SOFTHANGUP_DEV);
        if (chanb)
            opbx_softhangup_nolock(chanb, OPBX_SOFTHANGUP_DEV);
        return -1;
    }
    return 0;
}
...
static char *gettag(struct sip_request *req, char *header, char *tagbuf, int tagbufsize) 
{
    ...
    {
        thetag += 5;
        opbx_copy_string(tagbuf, thetag, tagbufsize);
        sep = strchr(tagbuf, ';');
        if (sep)
            *sep = '\0';
    }
...
static int handle_request_invite(struct sip_pvt *p, struct sip_request *req, int debug, int ignore, int seqno, struct sockaddr_in *sin, int *recount, char *e)
{
    ...
    /* This happens since we do not properly support SIP domain
       handling yet... -oej */
    if (opbx_test_flag(p, SIP_OUTGOING) && p->owner && (p->owner->_state != OPBX_STATE_UP))
    {
    ...
    if (!ignore)
    {
        /* Use this as the basis */
        if (debug)
            opbx_verbose("Using INVITE request as basis request - %s\n", p->callid);
        sip_cancel_destroy(p);
        /* This call is no longer outgoing if it ever was */
        opbx_clear_flag(p, SIP_OUTGOING);
        ...
        if (p->owner)
        {
            ...
            {
                p->jointcapability = p->capability;
                opbx_log(OPBX_LOG_DEBUG, "Hm....  No sdp for the moment\n");
            }
        }
    }
    else if (debug)
	...
	if (res > 0)
        ...
        {
	    if (res == -4) {
		opbx_log(OPBX_LOG_NOTICE, "Sending fake auth rejection for user %s\n", get_header(req, "From"));
		transmit_fake_auth_response(p, req, p->randdata, sizeof(p->randdata), 1);
	    } else {
		opbx_log(OPBX_LOG_NOTICE, "Failed to authenticate user %s\n", get_header(req, "From"));
        ...
        /* Queue NULL frame to prod opbx_rtp_bridge if appropriate */
        if (p->owner)
            opbx_queue_frame(p->owner, &af);
        ...
        opbx_log(OPBX_LOG_DEBUG, "Checking SIP call limits for device %s\n", p->username);
        res = update_call_counter(p, INC_CALL_LIMIT);
        if (res)
        {
            if (res < 0)
            {
                opbx_log(OPBX_LOG_NOTICE, "Failed to place call for user %s, too many calls\n", p->username);
    ...
    {
        if (option_debug > 1  &&  sipdebug)
            opbx_log(OPBX_LOG_DEBUG, "Got a SIP re-invite for call %s\n", p->callid);
        c = p->owner;
    }
    if (!ignore  &&  p)
        p->lastinvite = seqno;
    if (c)
    {
        ...
        case OPBX_STATE_DOWN:
            ...
            if (strcmp(p->exten, opbx_pickup_ext()))
            {
                ...
                case OPBX_PBX_FAILED:
                    opbx_log(OPBX_LOG_WARNING, "Failed to start PBX :(\n");
                    if (ignore)
                ...
                case OPBX_PBX_CALL_LIMIT:
                    opbx_log(OPBX_LOG_WARNING, "Failed to start PBX (call limit reached) \n");
                    if (ignore)
            ...
            {
                opbx_mutex_unlock(&c->lock);
                if (opbx_pickup_call(c))
                {
                    opbx_log(OPBX_LOG_NOTICE, "Nothing to pick up\n");
                    if (ignore)
                        transmit_response(p, "503 Unavailable", req);
                    else
                        transmit_response_reliable(p, "503 Unavailable", req, 1);
                ...
                if ( (bridgepeer=opbx_bridged_channel(p->owner) ) && !strcasecmp(bridgepeer->type,"SIP") ) 
		{
            ...
            break;
        default:
            opbx_log(OPBX_LOG_WARNING, "Don't know how to handle INVITE in state %d\n", c->_state);
            transmit_response(p, "100 Trying", req);
        }
    }
    else
    {
        if (p && !opbx_test_flag(p, SIP_NEEDDESTROY) && !ignore)
        {
            if (!p->jointcapability)
            {
            ...
            {
                opbx_log(OPBX_LOG_NOTICE, "Unable to create/find channel\n");
                transmit_response_reliable(p, "503 Unavailable", req, 1);
            }
            opbx_set_flag(p, SIP_NEEDDESTROY);    
        }
...
static int handle_request_refer(struct sip_pvt *p, struct sip_request *req, int debug, int ignore, int seqno, int *nounlock)
{
    struct opbx_channel *c=NULL;
    int res;
    struct opbx_channel *transfer_to;
    char *transfercontext = NULL;

    if (option_debug > 2)
        opbx_log(OPBX_LOG_DEBUG, "SIP call transfer received for call %s (REFER)!\n", p->callid);
    ...
    else
    {
        ...
        if (!ignore)
        {
            if (p->refer_call)
            {
                opbx_log(OPBX_LOG_DEBUG,"202 Accepted (supervised)\n");
            ...
            {
                opbx_log(OPBX_LOG_DEBUG,"202 Accepted (blind)\n");
                c = p->owner;
                if (c)
                {
                    transfer_to = opbx_bridged_channel(c);
                    if (transfer_to)
                    {
                        opbx_log(OPBX_LOG_DEBUG, "Got SIP blind transfer, applying to '%s'\n", transfer_to->name);
                        opbx_moh_stop(transfer_to);
                        if (!strcmp(p->refer_to, opbx_parking_ext()))
                        {
                        ...
                        else
                        {
                            ...
                            *nounlock = 1;
                            opbx_mutex_unlock(&c->lock);
                            opbx_async_goto(transfer_to,transfercontext, p->refer_to,1);
                        }
                    }
                    else
                    {
                        opbx_log(OPBX_LOG_DEBUG, "Got SIP blind transfer but nothing to transfer to.\n");
                        opbx_queue_hangup(p->owner);
                    }
                }
                opbx_set_flag(p, SIP_GOTREFER);    
...
static int handle_request_bye(struct sip_pvt *p, struct sip_request *req, int debug, int ignore)
{
    struct opbx_channel *c=NULL;
    int res;
    struct opbx_channel *bridged_to;
    char iabuf[INET_ADDRSTRLEN];
    
    if (p->pendinginvite && !opbx_test_flag(p, SIP_OUTGOING) && !ignore && !p->owner)
        transmit_response_reliable(p, "487 Request Terminated", &p->initreq, 1);
    ...
    {
        opbx_log(OPBX_LOG_NOTICE, "Client '%s' using deprecated BYE/Also transfer method.  Ask vendor to support REFER instead\n",
            opbx_inet_ntoa(iabuf, sizeof(iabuf), p->recv.sin_addr));
        if (opbx_strlen_zero(p->context))
            strcpy(p->context, default_context);
        ...
        if (!res)
        {
            ...
            if (c)
            {
                bridged_to = opbx_bridged_channel(c);
                if (bridged_to)
                {
                    /* Don't actually hangup here... */
                    opbx_moh_stop(bridged_to);
                    opbx_async_goto(bridged_to, p->context, p->refer_to,1);
                }
                else
                    opbx_queue_hangup(p->owner);
            }
        ...
        {
            opbx_log(OPBX_LOG_WARNING, "Invalid transfer information from '%s'\n", opbx_inet_ntoa(iabuf, sizeof(iabuf), p->recv.sin_addr));
            opbx_queue_hangup(p->owner);
        }
    }
    else if (p->owner)
...
static int handle_request_message(struct sip_pvt *p, struct sip_request *req, int debug, int ignore)
{
    ...
    {
        if (debug)
            opbx_verbose("Receiving message!\n");
...
static int handle_request_subscribe(struct sip_pvt *p, struct sip_request *req, int debug, int ignore, struct sockaddr_in *sin, int seqno, char *e)
{
    int gotdest;
    int res = 0;
    int firststate = OPBX_EXTENSION_REMOVED;
    ...
    {    
        ...
        if (p->initreq.method != SIP_SUBSCRIBE)
        {
            ...
            opbx_log(OPBX_LOG_DEBUG, "Got a subscription within the context of another call, can't handle that - %s (Method %s)\n", p->callid, sip_methods[p->initreq.method].text);
            return 0;
        }
        else
        {
            if (debug)
                opbx_log(OPBX_LOG_DEBUG, "Got a re-subscribe on existing subscription %s\n", p->callid);
        }
    }
    if (!ignore && !p->initreq.headers)
    {
        /* Use this as the basis */
        if (debug)
            opbx_verbose("Using latest SUBSCRIBE request as basis request\n");
        /* This call is no longer outgoing if it ever was */
        opbx_clear_flag(p, SIP_OUTGOING);
        copy_request(&p->initreq, req);
        check_via(p, req);
	...
	if (res > 0)
		...
		if (res == -4) {
			...
			opbx_log(OPBX_LOG_NOTICE, "Failed to authenticate user %s for SUBSCRIBE\n", get_header(req, "From"));
			if (ignore)
	...
	*/
        ...
        else
        {
			...
			opbx_log(OPBX_LOG_WARNING,"SUBSCRIBE failure: no Accept header: pvt: stateid: %d, laststate: %d, dialogver: %d, subscribecont: '%s'\n",
		...
		if (option_debug > 2) {
		    ...
		    if ( st != NULL )
			opbx_log(OPBX_LOG_DEBUG, "Subscription type: Event: %s Format: %s\n",  st->event, st->mediatype);
		}		    
            ...
            else
            {
                ...
                transmit_response(p, "489 Bad Event", req);
                if (option_debug > 1)
                    opbx_log(OPBX_LOG_DEBUG, "Received SIP subscribe for unknown event package: %s\n", event);
                opbx_set_flag(p, SIP_NEEDDESTROY);    
                return 0;
            }
            if (p->subscribed != NONE)
                p->stateid = opbx_extension_state_add(p->context, p->exten, cb_extensionstate, p);
        }
    }

    if (!ignore && p)
        p->lastinvite = seqno;
    if (p  &&  !opbx_test_flag(p, SIP_NEEDDESTROY))
    {
        ...
        if (sipdebug  ||  option_debug > 1)
            opbx_log(OPBX_LOG_DEBUG, "Adding subscription for extension %s context %s for peer %s\n", p->exten, p->context, p->username);
        if (p->autokillid > -1)
            sip_cancel_destroy(p);    /* Remove subscription expiry for renewals */
        sip_scheddestroy(p, (p->expiry + 10) * 1000);    /* Set timer for destruction of call at expiration */

        if ((firststate = opbx_extension_state(NULL, p->context, p->exten)) < 0)
        {
            opbx_log(OPBX_LOG_ERROR, "Got SUBSCRIBE for extensions without hint. Please add hint to %s in context %s\n", p->exten, p->context);
            transmit_response(p, "404 Not found", req);
            opbx_set_flag(p, SIP_NEEDDESTROY);    
            return 0;
        }
        else
        {
            ...
            append_history(p, "Subscribestatus", opbx_extension_state2str(firststate));
	    ...
	    */
		...
		if (p_old->subscribed == NONE)
		    continue;
		opbx_mutex_lock(&p_old->lock);
		if (!strcmp(p_old->username, p->username)) {
		    if (!strcmp(p_old->exten, p->exten) &&
		        !strcmp(p_old->context, p->context)) 
		    {
			opbx_set_flag(p_old, SIP_NEEDDESTROY);
			opbx_mutex_unlock(&p_old->lock);
			break;
		    }
		}
		opbx_mutex_unlock(&p_old->lock);
...
static int handle_request_register(struct sip_pvt *p, struct sip_request *req, int debug, int ignore, struct sockaddr_in *sin, char *e)
{
    ...
    /* Use this as the basis */
    if (debug)
        opbx_verbose("Using latest REGISTER request as basis request\n");
    copy_request(&p->initreq, req);
    check_via(p, req);
    if ((res = register_verify(p, sin, req, e, ignore)) < 0) 
        opbx_log(OPBX_LOG_NOTICE, "Registration from '%s' failed for '%s' - %s\n", get_header(req, "To"), opbx_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr), (res == -1) ? "Wrong password" : (res == -2 ? "Username/auth name mismatch" : "Not a local SIP domain"));
    if (res < 1)
    {
...
static int handle_request(struct sip_pvt *p, struct sip_request *req, struct sockaddr_in *sin, int *recount, int *nounlock)
{
    ...
    /* Must have Cseq */
    if (opbx_strlen_zero(cmd) || opbx_strlen_zero(cseq))
    {
        opbx_log(OPBX_LOG_ERROR, "Missing Cseq. Dropping this SIP message, it's incomplete.\n");
        error = 1;
    }
    if (!error && sscanf(cseq, "%d%n", &seqno, &len) != 1)
    {
        opbx_log(OPBX_LOG_ERROR, "No seqno in '%s'. Dropping incomplete message.\n", cmd);
        error = 1;
    }
    if (error)
    {
        if (!p->initreq.header)    /* New call */
            opbx_set_flag(p, SIP_NEEDDESTROY);    /* Make sure we destroy this dialog */
        return -1;
    }
    /* Get the command XXX */
    ...
    if (req->method == SIP_RESPONSE)
    {
        ...
        {
            opbx_log(OPBX_LOG_DEBUG, "Ignoring out of order response %d (expecting %d)\n", seqno, p->ocseq);
            return -1;
        }
        else if (p->ocseq && (p->ocseq != seqno))
        {
        ...
        {
            opbx_log(OPBX_LOG_WARNING, "Invalid response: '%s'\n", e);
        }
        else
        {
    ...
    if (option_debug > 2)
        opbx_log(OPBX_LOG_DEBUG, "**** Received %s (%d) - Command in SIP %s\n", sip_methods[p->method].text, sip_methods[p->method].id, cmd); 

    if (p->icseq && (p->icseq > seqno))
    {
	if (option_debug)
    	    opbx_log(OPBX_LOG_DEBUG, "Ignoring too old SIP packet packet %d (expecting >= %d)\n", seqno, p->icseq);
    ...
    else if (p->icseq && (p->icseq == seqno) && req->method != SIP_ACK &&(p->method != SIP_CANCEL|| opbx_test_flag(p, SIP_ALREADYGONE)))
    {
        ...
        ignore = 2;
        if (option_debug > 2)
            opbx_log(OPBX_LOG_DEBUG, "Ignoring SIP message because of retransmit (%s Seqno %d, ours %d)\n", sip_methods[p->method].text, p->icseq, seqno);
    }
    ...
    if (pedanticsipchecking)
    {
        ...
        if (!p->initreq.headers && opbx_test_flag(req, SIP_PKT_WITH_TOTAG))
        {
    ...
    case SIP_ACK:
        ...
        if (!p->lastinvite && opbx_strlen_zero(p->randdata))
            opbx_set_flag(p, SIP_NEEDDESTROY);    
        break;
    default:
        transmit_response_with_allow(p, "501 Method Not Implemented", req, 0);
        opbx_log(OPBX_LOG_NOTICE, "Unknown SIP command '%s' from '%s'\n", 
                 cmd, opbx_inet_ntoa(iabuf, sizeof(iabuf), p->sa.sin_addr));
...
static int sipsock_read(int *id, int fd, short events, void *ignore)
{
    ...
    if (res < 0)
    {
        ...
        if (errno == EAGAIN)
            opbx_log(OPBX_LOG_NOTICE, "SIP: Received packet with bad UDP checksum\n");
        else 
#endif
    ...
    if (req.method == SIP_UNKNOWN)
    {
        ...
        if (stun_me.msgtype == STUN_BINDRESP)
        {
            ...
            {
                opbx_log(OPBX_LOG_NOTICE, "STUN: externip not set. Setting it to stun mapped address %s\n", opbx_inet_ntoa(iabuf, sizeof(iabuf), msin.sin_addr) );
                externip = msin;
            }
            else if (memcmp(&externip.sin_addr,&msin.sin_addr,sizeof(externip.sin_addr)))
            {
                opbx_log(OPBX_LOG_NOTICE, "STUN: externip changed. Setting it to stun mapped address %s\n", opbx_inet_ntoa(iabuf, sizeof(iabuf), msin.sin_addr) );
                externip = msin;
            }
            return 1;
        }
    ...
    {
        /*
        opbx_verbose("--- (%d headers %d lines)", req.headers, req.lines);
        if (req.headers + req.lines == 0) 
            opbx_verbose(" Nat keepalive ");
        opbx_verbose("---\n");
        */
        opbx_verbose("--- (%d headers %d lines)%s ---\n", req.headers, req.lines, (req.headers + req.lines == 0) ? " Nat keepalive" : "");
    }
...
retrylock:
    ...
    if (p)
    {
        ...
        {
            opbx_log(OPBX_LOG_DEBUG, "Failed to grab lock, trying again...\n");
            opbx_mutex_unlock(&p->lock);
	...
	if (!lockretry) {
	    opbx_log(OPBX_LOG_ERROR, "We could NOT get the channel lock for %s - Call ID %s! \n", p->owner->name, p->callid);
	    opbx_log(OPBX_LOG_ERROR, "SIP MESSAGE JUST IGNORED: %s \n", req.data);
	    return 1;
	}
        memcpy(&p->recv, &sin, sizeof(p->recv));
        if (recordhistory)
        ...
        {
            /* Request failed */
            opbx_log(OPBX_LOG_DEBUG, "SIP message could not be handled, bad request: %-70.70s\n", p->callid[0] ? p->callid : "<no callid>");
        }
...
static int sip_send_mwi_to_peer(struct sip_peer *peer)
{
    ...
    /* Check for messages */
    opbx_app_messagecount(peer->mailbox, &newmsgs, &oldmsgs);
    ...
    if (!p)
    {
        opbx_log(OPBX_LOG_WARNING, "Unable to build sip pvt data for MWI\n");
        return -1;
    }
...
static void *do_monitor(void *data)
{
    ...
    if (sipsock > -1) 
        sipsock_read_id = opbx_io_add(io, sipsock, sipsock_read, OPBX_IO_IN, NULL);
    ...
    for (;;)
    {
        ...
        {
            if (option_verbose > 0)
                opbx_verbose(VERBOSE_PREFIX_1 "Reloading SIP\n");
            sip_do_reload();

            /* Change the I/O fd of our UDP socket */
            if (sipsock > -1)
                sipsock_read_id = opbx_io_change(io, sipsock_read_id, sipsock, NULL, 0, NULL);
        }
...
restartsearch:        
        ...
        while (!fastrestart && sip)
        {
            ...
            if (sip->rtp && sip->owner && (sip->owner->_state == OPBX_STATE_UP) && !sip->redirip.sin_addr.s_addr)
            {
                ...
                if (sip->lastrtprx && (sip->rtptimeout || sip->rtpholdtimeout) && t > sip->lastrtprx + sip->rtptimeout)
                {
                              ...
                              (t > sip->lastrtprx + sip->rtpholdtimeout)))
                    {
                        ...
                        if (sip->rtptimeout)
                        {
                            while (sip->owner && opbx_mutex_trylock(&sip->owner->lock))
                            {
                            ...
                            if (sip->owner)
                            {
                                opbx_log(OPBX_LOG_NOTICE, "Disconnecting call '%s' for lack of RTP activity in %ld seconds\n", sip->owner->name, (long)(t - sip->lastrtprx));
                                /* Issue a softhangup */
                                opbx_softhangup(sip->owner, OPBX_SOFTHANGUP_DEV);
                                opbx_mutex_unlock(&sip->owner->lock);
            ...
            if (opbx_test_flag(sip, SIP_NEEDDESTROY) && !sip->packets && !sip->owner)
            {
                    ...
                    ( sip->stun_needed==1 && ( opbx_stun_find_request(&sip->stun_transid)==NULL )))
                {
                    ...
                    opbx_log(OPBX_LOG_NOTICE, "Delaying call destroy (stun active) on call '%s' [%d]\n", sip->callid,sip->stun_needed);
            }
            opbx_mutex_unlock(&sip->lock);
            sip = sip->next;
        }
	...
	if (res >= 0)  {
        ...
        ASTOBJ_CONTAINER_TRAVERSE(&peerl, !peer, do {
            if ((curpeernum > lastpeernum) && !opbx_strlen_zero(iterator->mailbox) && ((t - iterator->lastmsgcheck) > global_mwitime))
            {
                fastrestart = 1;
...
static int sip_devicestate(void *data)
{
    ...
    host = opbx_strdupa(data);
    if ((tmp = strchr(host, '@')))
        host = tmp + 1;

    if (option_debug > 2) 
        opbx_log(OPBX_LOG_DEBUG, "Checking device state for peer %s\n", host);
    ...
    {
        if ((hp = opbx_gethostbyname(host, &ahp)))
            res = OPBX_DEVICE_UNKNOWN;
    }
...
static struct opbx_channel *sip_request_call(const char *type, int format, void *data, int *cause)
{
    int oldformat;
    struct sip_pvt *p;
    struct opbx_channel *tmpc = NULL;
    char *ext, *host;
    char tmp[256];
    char *dest = data;

    oldformat = format;
    format &= ((OPBX_FORMAT_MAX_AUDIO << 1) - 1);
    if (!format)
    {
    ...
    if (!p)
    {
        opbx_log(OPBX_LOG_WARNING, "Unable to build sip pvt data for '%s'\n", (char *)data);
        return NULL;
    }
    ...
    {
	sip_destroy(p);
	opbx_log(OPBX_LOG_ERROR, "Unable to build option SIP data structure - Out of memory\n");
	*cause = OPBX_CAUSE_SWITCH_CONGESTION;
        return NULL;
    }
    ...
    if (opbx_strlen_zero(p->peername) && ext)
        opbx_copy_string(p->peername, ext, sizeof(p->peername));
    ...
    p->prefcodec = format;
    opbx_mutex_lock(&p->lock);
    tmpc = sip_new(p, OPBX_STATE_DOWN, host);    /* Place the call */
    opbx_mutex_unlock(&p->lock);
    if (!tmpc)
        sip_destroy(p);
    restart_monitor();
...
static int handle_common_options(struct opbx_flags *flags, struct opbx_flags *mask, struct opbx_variable *v)
{
    int res = 0;
    ...
    else if (!strcasecmp(v->name, "useclientcode"))
    {
        ...
        opbx_set2_flag(flags, opbx_true(v->value), SIP_USECLIENTCODE);
        res = 1;
    }
    else if (!strcasecmp(v->name, "dtmfmode"))
    {
        opbx_set_flag(mask, SIP_DTMF);
        opbx_clear_flag(flags, SIP_DTMF);
        if (!strcasecmp(v->value, "inband"))
            opbx_set_flag(flags, SIP_DTMF_INBAND);
        else if (!strcasecmp(v->value, "rfc2833"))
            opbx_set_flag(flags, SIP_DTMF_RFC2833);
        else if (!strcasecmp(v->value, "info"))
            opbx_set_flag(flags, SIP_DTMF_INFO);
        else if (!strcasecmp(v->value, "auto"))
            opbx_set_flag(flags, SIP_DTMF_AUTO);
        else
        {
            opbx_log(OPBX_LOG_WARNING, "Unknown dtmf mode '%s' on line %d, using rfc2833\n", v->value, v->lineno);
            opbx_set_flag(flags, SIP_DTMF_RFC2833);
        }
    }
    else if (!strcasecmp(v->name, "nat"))
    {
        opbx_set_flag(mask, SIP_NAT);
        opbx_clear_flag(flags, SIP_NAT);
    ...
    {
        opbx_set_flag(mask, SIP_REINVITE);
        opbx_clear_flag(flags, SIP_REINVITE);
        if (!strcasecmp(v->value, "update"))
            opbx_set_flag(flags, SIP_REINVITE_UPDATE | SIP_CAN_REINVITE);
        else
            opbx_set2_flag(flags, opbx_true(v->value), SIP_CAN_REINVITE);
    }
    else if (!strcasecmp(v->name, "insecure"))
    {
        opbx_set_flag(mask, SIP_INSECURE_PORT | SIP_INSECURE_INVITE);
        opbx_clear_flag(flags, SIP_INSECURE_PORT | SIP_INSECURE_INVITE);
        if (!strcasecmp(v->value, "very"))
            opbx_set_flag(flags, SIP_INSECURE_PORT | SIP_INSECURE_INVITE);
        else if (opbx_true(v->value))
            opbx_set_flag(flags, SIP_INSECURE_PORT);
        else if (!opbx_false(v->value))
        {
            char buf[64];
            char *word, *next;

            opbx_copy_string(buf, v->value, sizeof(buf));
            next = buf;
            while ((word = strsep(&next, ",")))
            {
                if (!strcasecmp(word, "port"))
                    ...
                    opbx_set_flag(flags, SIP_INSECURE_INVITE);
                else
                    opbx_log(OPBX_LOG_WARNING, "Unknown insecure mode '%s' on line %d\n", v->value, v->lineno);
            }
        }
    }
    else if (!strcasecmp(v->name, "progressinband"))
    {
        opbx_set_flag(mask, SIP_PROG_INBAND);
        opbx_clear_flag(flags, SIP_PROG_INBAND);
        if (opbx_true(v->value))
            opbx_set_flag(flags, SIP_PROG_INBAND_YES);
        else if (strcasecmp(v->value, "never"))
            opbx_set_flag(flags, SIP_PROG_INBAND_NO);
    }
    ...
    {
        opbx_set_flag(mask, SIP_OSPAUTH);
        opbx_clear_flag(flags, SIP_OSPAUTH);
        if (!strcasecmp(v->value, "proxy"))
            opbx_set_flag(flags, SIP_OSPAUTH_PROXY);
        else if (!strcasecmp(v->value, "gateway"))
            opbx_set_flag(flags, SIP_OSPAUTH_GATEWAY);
        else if(!strcasecmp (v->value, "exclusive"))
             opbx_set_flag(flags, SIP_OSPAUTH_EXCLUSIVE);
#endif
    }
    else if (!strcasecmp(v->name, "promiscredir"))
    {
        opbx_set_flag(mask, SIP_PROMISCREDIR);
        opbx_set2_flag(flags, opbx_true(v->value), SIP_PROMISCREDIR);
        res = 1;
    }
...
static int add_sip_domain(const char *domain, const enum domain_mode mode, const char *context)
{
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "Zero length domain.\n");
        return 1;
    }
    ...
    if (!d)
    {
        opbx_log(OPBX_LOG_ERROR, "Allocation of domain structure failed, Out of memory\n");
        return 0;
    }
    ...
    OPBX_LIST_LOCK(&domain_list);
    OPBX_LIST_INSERT_TAIL(&domain_list, d, list);
    OPBX_LIST_UNLOCK(&domain_list);

     if (sipdebug)    
        opbx_log(OPBX_LOG_DEBUG, "Added local SIP domain '%s'\n", domain);
...
static int check_sip_domain(const char *domain, char *context, size_t len)
{
    ...
    OPBX_LIST_TRAVERSE(&domain_list, d, list)
    {
        ...
        if (len && !opbx_strlen_zero(d->context))
            opbx_copy_string(context, d->context, len);
...
static void clear_sip_domains(void)
{
    ...
    OPBX_LIST_LOCK(&domain_list);
    while ((d = OPBX_LIST_REMOVE_HEAD(&domain_list, list)))
        free(d);
    OPBX_LIST_UNLOCK(&domain_list);
}
...
static struct sip_auth *add_realm_authentication(struct sip_auth *authlist, char *configuration, int lineno)
{
    ...
    opbx_log(OPBX_LOG_DEBUG, "Auth config ::  %s\n", configuration);

    opbx_copy_string(authcopy, configuration, sizeof(authcopy));
    stringp = authcopy;
    ...
    {
        opbx_log(OPBX_LOG_WARNING, "Format for authentication entry is user[:secret]@realm at line %d\n", lineno);
        return authlist;
    }
    stringp = username;
    username = strsep(&stringp, ":");
    ...
    if (auth)
    {
        ...
        opbx_copy_string(auth->realm, realm, sizeof(auth->realm));
        opbx_copy_string(auth->username, username, sizeof(auth->username));
        if (secret)
            opbx_copy_string(auth->secret, secret, sizeof(auth->secret));
        if (md5secret)
            opbx_copy_string(auth->md5secret, md5secret, sizeof(auth->md5secret));
    }
    else
    {
        opbx_log(OPBX_LOG_ERROR, "Allocation of auth structure failed, Out of memory\n");
        return authlist;
    }
    ...
    if (option_verbose > 2)
        opbx_verbose("Added authentication for realm %s\n", realm);
...
static struct sip_user *build_user(const char *name, struct opbx_variable *v, int realtime)
{
    ...
    struct opbx_flags userflags = {(0)};
    struct opbx_flags mask = {(0)};
    ...
    while (v)
    {
        ...
        {
            opbx_copy_string(user->context, v->value, sizeof(user->context));
        }
        else if (!strcasecmp(v->name, "subscribecontext"))
        {
            opbx_copy_string(user->subscribecontext, v->value, sizeof(user->subscribecontext));
        }
        ...
        {
            opbx_copy_string(user->md5secret, v->value, sizeof(user->md5secret));
        }
        ...
        {
            user->callgroup = opbx_get_group(v->value);
        }
        else if (!strcasecmp(v->name, "pickupgroup"))
        {
            user->pickupgroup = opbx_get_group(v->value);
        }
        ...
        {
            opbx_copy_string(user->musicclass, v->value, sizeof(user->musicclass));
        }
    ...
    opbx_copy_flags(user, &userflags, mask.flags);
    opbx_free_ha(oldha);
    return user;
}
...
static struct sip_peer *temp_peer(const char *name)
{
    ...
    opbx_set_flag(peer, SIP_SELFDESTRUCT);
    opbx_set_flag(&peer->flags_page2, SIP_PAGE2_DYNAMIC);
    peer->prefs = prefs;
    reg_source_db(peer);
...
static struct sip_peer *build_peer(const char *name, struct opbx_variable *v, int realtime)
{
    ...
    else
    {
        ...
        {
            opbx_log(OPBX_LOG_WARNING, "Can't allocate SIP peer memory\n");
        }
    }
    ...
    strcpy(peer->language, default_language);
    strcpy(peer->musicclass, global_musicclass);
    opbx_copy_flags(peer, &global_flags, SIP_USEREQPHONE);
    peer->secret[0] = '\0';
    peer->md5secret[0] = '\0';
    peer->cid_num[0] = '\0';
    peer->cid_name[0] = '\0';
    ...
    while (v)
    {
        ...
        else if (realtime && !strcasecmp(v->name, "ipaddr") && !opbx_strlen_zero(v->value))
        {
        ...
        {
            opbx_callerid_split(v->value, peer->cid_name, sizeof(peer->cid_name), peer->cid_num, sizeof(peer->cid_num));
        }
        ...
        {
            opbx_copy_string(peer->subscribecontext, v->value, sizeof(peer->subscribecontext));
        }
        else if (!strcasecmp(v->name, "fromdomain"))
            opbx_copy_string(peer->fromdomain, v->value, sizeof(peer->fromdomain));
        ...
        else if (!strcasecmp(v->name, "host") || !strcasecmp(v->name, "outboundproxy"))
        {
            if (!strcasecmp(v->value, "dynamic"))
            {
                ...
                {
                    opbx_log(OPBX_LOG_WARNING, "You can't have a dynamic outbound proxy, you big silly head at line %d.\n", v->lineno);
                }
                else
                {
        ...
        {
            if (!realtime && opbx_test_flag(&peer->flags_page2, SIP_PAGE2_DYNAMIC))
        ...
        {
            peer->callingpres = opbx_parse_caller_presentation(v->value);
        ...
        {
            opbx_copy_string(peer->language, v->value, sizeof(peer->language));
        }
        ...
        else if (!strcasecmp(v->name, "amaflags"))
        {
            ...
            if (format < 0)
            {
                opbx_log(OPBX_LOG_WARNING, "Invalid AMA Flags for peer: %s at line %d\n", v->value, v->lineno);
            }
            else
            {
        ...
        {
            opbx_copy_string(peer->accountcode, v->value, sizeof(peer->accountcode));
        }
        else if (!strcasecmp(v->name, "musiconhold"))
        {
            opbx_copy_string(peer->musicclass, v->value, sizeof(peer->musicclass));
        }
        ...
        {
            opbx_copy_string(peer->vmexten, v->value, sizeof(peer->vmexten));
        }
        else if (!strcasecmp(v->name, "callgroup"))
        {
            peer->callgroup = opbx_get_group(v->value);
        }
        else if (!strcasecmp(v->name, "pickupgroup"))
        {
            peer->pickupgroup = opbx_get_group(v->value);
        }
        else if (!strcasecmp(v->name, "allow"))
        {
        ...
        else if (!strcasecmp(v->name, "rtptimeout"))
        {
            ...
            {
                opbx_log(OPBX_LOG_WARNING, "'%s' is not a valid RTP hold time at line %d.  Using default.\n", v->value, v->lineno);
                peer->rtptimeout = global_rtptimeout;
            }
        }
        ...
        else if (!strcasecmp(v->name, "rtpkeepalive"))
        {
            ...
            {
                opbx_log(OPBX_LOG_WARNING, "'%s' is not a valid RTP keepalive time at line %d.  Using default.\n", v->value, v->lineno);
                peer->rtpkeepalive = global_rtpkeepalive;
            }
        }
        else if (!strcasecmp(v->name, "setvar"))
        ...
        else if (!strcasecmp(v->name, "qualify"))
        {
            ...
            {
                opbx_log(OPBX_LOG_WARNING, "Qualification of peer '%s' should be 'yes', 'no', or a number of milliseconds at line %d of sip.conf\n", peer->name, v->lineno);
                peer->maxms = 0;
            }
        }
        /* else if (strcasecmp(v->name,"type"))
    ...
    if (!opbx_test_flag((&global_flags_page2), SIP_PAGE2_IGNOREREGEXPIRE) && opbx_test_flag(&peer->flags_page2, SIP_PAGE2_DYNAMIC)  &&  realtime)
    {
        time_t nowtime;
        ...
        if ((nowtime - regseconds) > 0)
        {
            ...
            memset(&peer->addr, 0, sizeof(peer->addr));
            if (option_debug)
                opbx_log(OPBX_LOG_DEBUG, "Bah, we're expired (%ld/%ld/%ld)!\n", nowtime - regseconds, regseconds, nowtime);
        }
    }
    opbx_copy_flags(peer, &peerflags, mask.flags);
    if (!found && opbx_test_flag(&peer->flags_page2, SIP_PAGE2_DYNAMIC) && !opbx_test_flag(peer, SIP_REALTIME))
        reg_source_db(peer);
    ASTOBJ_UNMARK(peer);
    opbx_free_ha(oldha);
    return peer;
...
static int reload_config(void)
{
    ...
    if (!cfg)
    {
        opbx_log(OPBX_LOG_NOTICE, "Unable to load config %s\n", config);
        return -1;
    }
    ...
    externexpire = 0;
    externrefresh = 10;
    opbx_copy_string(default_useragent, DEFAULT_USERAGENT, sizeof(default_useragent));
    opbx_copy_string(default_notifymime, DEFAULT_NOTIFYMIME, sizeof(default_notifymime));
    global_notifyringing = 1;
    global_alwaysauthreject = 0;
    opbx_copy_string(global_realm, DEFAULT_REALM, sizeof(global_realm));
    opbx_copy_string(global_musicclass, "default", sizeof(global_musicclass));
    opbx_copy_string(default_callerid, DEFAULT_CALLERID, sizeof(default_callerid));
    ...
    opbx_clear_flag(&global_flags_page2, OPBX_FLAGS_ALL);
    opbx_set_flag(&global_flags, SIP_DTMF_RFC2833);
    opbx_set_flag(&global_flags, SIP_NAT_RFC3581);
    opbx_set_flag(&global_flags, SIP_CAN_REINVITE);
    opbx_set_flag(&global_flags_page2, SIP_PAGE2_RTUPDATE);
    ...
    while (v)
    {
        ...
        if (!strcasecmp(v->name, "context"))
        {
            opbx_copy_string(default_context, v->value, sizeof(default_context));
        }
        else if (!strcasecmp(v->name, "realm"))
        {
            opbx_copy_string(global_realm, v->value, sizeof(global_realm));
        }
        else if (!strcasecmp(v->name, "useragent"))
        {
            opbx_copy_string(default_useragent, v->value, sizeof(default_useragent));
            opbx_log(OPBX_LOG_DEBUG, "Setting User Agent Name to %s\n",
                default_useragent);
        }
        ...
        else if (!strcasecmp(v->name, "rtautoclear"))
        {
            ...
            if (i > 0)
                ...
                i = 0;
            opbx_set2_flag((&global_flags_page2), i || opbx_true(v->value), SIP_PAGE2_RTAUTOCLEAR);
        }
        else if (!strcasecmp(v->name, "usereqphone"))
        {
            opbx_set2_flag((&global_flags), opbx_true(v->value), SIP_USEREQPHONE);    
        }
        else if (!strcasecmp(v->name, "relaxdtmf"))
        {
            relaxdtmf = opbx_true(v->value);
        }
        else if (!strcasecmp(v->name, "checkmwi"))
        {
            if ((sscanf(v->value, "%d", &global_mwitime) != 1) || (global_mwitime < 0))
            {
                opbx_log(OPBX_LOG_WARNING, "'%s' is not a valid MWI time setting at line %d.  Using default (10).\n", v->value, v->lineno);
                global_mwitime = DEFAULT_MWITIME;
            }
        }
        else if (!strcasecmp(v->name, "vmexten"))
        {
            opbx_copy_string(global_vmexten, v->value, sizeof(global_vmexten));
        }
        else if (!strcasecmp(v->name, "rtptimeout"))
        {
        ...
        {
            videosupport = opbx_true(v->value);
        }
        else if (!strcasecmp(v->name, "t38udptlsupport"))
        {
            t38udptlsupport = opbx_true(v->value);
        }
        else if (!strcasecmp(v->name, "t38rtpsupport"))
        {
            t38rtpsupport = opbx_true(v->value);
        }
        else if (!strcasecmp(v->name, "t38tcpsupport"))
        {
            t38tcpsupport = opbx_true(v->value);
        }
        else if (!strcasecmp(v->name, "compactheaders"))
        {
            compactheaders = opbx_true(v->value);
        }
        else if (!strcasecmp(v->name, "notifymimetype"))
        {
            opbx_copy_string(default_notifymime, v->value, sizeof(default_notifymime));
        }
        else if (!strcasecmp(v->name, "notifyringing"))
        {
            global_notifyringing = opbx_true(v->value);
        } 
	else if (!strcasecmp(v->name, "alwaysauthreject")) {
	    global_alwaysauthreject = opbx_true(v->value);
	}
        else if (!strcasecmp(v->name, "musicclass") || !strcasecmp(v->name, "musiconhold"))
        {
            opbx_copy_string(global_musicclass, v->value, sizeof(global_musicclass));
        }
        else if (!strcasecmp(v->name, "language"))
        {
            opbx_copy_string(default_language, v->value, sizeof(default_language));
        }
        ...
        {
            opbx_copy_string(default_callerid, v->value, sizeof(default_callerid));
        }
        else if (!strcasecmp(v->name, "fromdomain"))
        {
            opbx_copy_string(default_fromdomain, v->value, sizeof(default_fromdomain));
        }
        ...
        {
            srvlookup = opbx_true(v->value);
        }
        else if (!strcasecmp(v->name, "pedantic"))
        {
            pedanticsipchecking = opbx_true(v->value);
        }
        ...
        {
            dumphistory = opbx_true(v->value);
        }
        else if (!strcasecmp(v->name, "recordhistory"))
        {
            recordhistory = opbx_true(v->value);
        }
        ...
        {
            if ((hp = opbx_gethostbyname(v->value, &ahp)) == NULL)
        ...
        else if (!strcasecmp(v->name, "localnet"))
        {
            ...
            if (!(na = opbx_append_ha("d", v->value, localaddr)))
        ...
        {
            opbx_log(OPBX_LOG_WARNING, "Use of localmask is no long supported -- use localnet with mask syntax\n");
        }
        ...
        {
            opbx_copy_string(externhost, v->value, sizeof(externhost));
            if ((hp = opbx_gethostbyname(externhost, &ahp)) == NULL)
                opbx_log(OPBX_LOG_WARNING, "Invalid address for externhost keyword: %s\n", externhost);
            else
                memcpy(&externip.sin_addr, hp->h_addr, sizeof(externip.sin_addr));
            time(&externexpire);
        }
        ...
        {
            opbx_copy_string(stunserver_host, v->value, sizeof(stunserver_host));
            if ((hp = opbx_gethostbyname(stunserver_host, &ahp)) == NULL)
            {
                opbx_log(OPBX_LOG_NOTICE, "Invalid address for stunserver_host keyword: %s\n", stunserver_host);
            }
            else
            {
        ...
        else if (!strcasecmp(v->name, "stunserver_port"))
        {
            if (sscanf(v->value, "%d", &stunserver_portno) == 1)
            {
                ...
                if (stundebug) 
                    opbx_log(OPBX_LOG_WARNING, "STUN: stunserver_portno is: %d\n", stunserver_portno);
            }
            else
            {
                if (stundebug) 
                    opbx_log(OPBX_LOG_WARNING, "Invalid stun port number '%s' at line %d of %s\n", v->value, v->lineno, config);
            }
        }
        else if (!strcasecmp(v->name, "allow"))
        {
            opbx_parse_allow_disallow(&prefs, &global_capability, v->value, 1);
        }
        else if (!strcasecmp(v->name, "disallow"))
        {
            opbx_parse_allow_disallow(&prefs, &global_capability, v->value, 0);
        }
        else if (!strcasecmp(v->name, "allowexternaldomains"))
        {
            allow_external_domains = opbx_true(v->value);
        }
        else if (!strcasecmp(v->name, "autodomain"))
        {
            auto_sip_domains = opbx_true(v->value);
        }
        else if (!strcasecmp(v->name, "domain"))
        {
            char *domain = opbx_strdupa(v->value);
            ...
            if (opbx_strlen_zero(domain))
                opbx_log(OPBX_LOG_WARNING, "Empty domain specified at line %d\n", v->lineno);
            else if (opbx_strlen_zero(context))
                opbx_log(OPBX_LOG_WARNING, "Empty context specified at line %d for domain '%s'\n", v->lineno, domain);
            else
        ...
        else if (!strcasecmp(v->name, "qualify"))
        {
            ...
            {
                opbx_log(OPBX_LOG_WARNING, "Qualification default should be 'yes', 'no', or a number of milliseconds at line %d of sip.conf\n", v->lineno);
                default_qualify = 0;
            }
        }
    ...
    if (!allow_external_domains && OPBX_LIST_EMPTY(&domain_list))
    {
        opbx_log(OPBX_LOG_WARNING, "To disallow external domains, you need to configure local SIP domains.\n");
        allow_external_domains = 1;
    }
    
    /* Build list of authentication to various SIP realms, i.e. service providers */
    v = opbx_variable_browse(cfg, "authentication");
    while (v)
    {
    ...
    while (cat)
    {
        if (strcasecmp(cat, "general") && strcasecmp(cat, "authentication"))
        {
            ...
            if (utype)
            {
                ...
                {
                    opbx_log(OPBX_LOG_WARNING, "Unknown type '%s' for '%s' in %s\n", utype, cat, "sip.conf");
                }
            }
            else
                opbx_log(OPBX_LOG_WARNING, "Section '%s' lacks type\n", cat);
        }
        cat = opbx_category_browse(cfg, cat);
    }
    if (opbx_find_ourip(&__ourip, bindaddr))
    {
        opbx_log(OPBX_LOG_WARNING, "Unable to get own IP address, SIP disabled\n");
        return 0;
    }
    ...
    if (sipsock < 0)
    {
        ...
        if (sipsock < 0)
        {
            opbx_log(OPBX_LOG_WARNING, "Unable to create SIP socket: %s\n", strerror(errno));
        }
        else
        {
            ...
            opbx_enable_packet_fragmentation(sipsock);

            if (opbx_udpfromto_init(sipsock) < 0)
            {
                opbx_log(OPBX_LOG_ERROR, "Failed to set socket parameters");
                close(sipsock);
                sipsock = -1;
            }
            ...
            else
            {
                if (option_verbose > 1)
                { 
                    opbx_verbose(VERBOSE_PREFIX_2 "SIP Listening on %s:%d\n", 
                    opbx_inet_ntoa(iabuf, sizeof(iabuf), bindaddr.sin_addr), ntohs(bindaddr.sin_port));
    ...
    if (auto_sip_domains)
    {
        ...
        {
            opbx_inet_ntoa(temp, sizeof(temp), bindaddr.sin_addr);
            add_sip_domain(temp, SIP_DOMAIN_AUTO, NULL);
        }
        else
        {
            opbx_log(OPBX_LOG_NOTICE, "Can't add wildcard IP address to domain list, please add IP address to domain manually.\n");
        }
...
//TODO add stun ip to domains if needed
        ...
        if (!opbx_strlen_zero(externhost))
    ...
    if (notify_types)
        opbx_config_destroy(notify_types);
    notify_types = opbx_config_load(notify_config);

    return 0;
}

/*! \brief  sip_get_rtp_peer: Returns null if we can't reinvite (part of RTP interface) */
static struct opbx_rtp *sip_get_rtp_peer(struct opbx_channel *chan)
{
    struct sip_pvt *p;
    struct opbx_rtp *rtp = NULL;
    p = chan->tech_pvt;
    if (!p)
        return NULL;
    opbx_mutex_lock(&p->lock);
    if (p->rtp && opbx_test_flag(p, SIP_CAN_REINVITE))
        rtp =  p->rtp;
    opbx_mutex_unlock(&p->lock);
    return rtp;
}
...
static struct opbx_rtp *sip_get_vrtp_peer(struct opbx_channel *chan)
{
    struct sip_pvt *p;
    struct opbx_rtp *rtp = NULL;
    p = chan->tech_pvt;
    ...
    opbx_mutex_lock(&p->lock);
    if (p->vrtp && opbx_test_flag(p, SIP_CAN_REINVITE))
        rtp = p->vrtp;
    opbx_mutex_unlock(&p->lock);
    return rtp;
}
...
static int sip_set_rtp_peer(struct opbx_channel *chan, struct opbx_rtp *rtp, struct opbx_rtp *vrtp, int codecs, int nat_active)
{
    struct sip_pvt *p;
    ...
    opbx_mutex_lock(&p->lock);
    if (rtp)
        opbx_rtp_get_peer(rtp, &p->redirip);
    else
        memset(&p->redirip, 0, sizeof(p->redirip));
    if (vrtp)
        opbx_rtp_get_peer(vrtp, &p->vredirip);
    else
    ...
    if (!opbx_test_flag(p, SIP_GOTREFER))
    {
        if (!p->pendinginvite)
        {
            ...
            {
                char iabuf[INET_ADDRSTRLEN];
                opbx_log(OPBX_LOG_DEBUG, "Sending reinvite on SIP '%s' - It's audio soon redirected to IP %s\n", p->callid, opbx_inet_ntoa(iabuf, sizeof(iabuf), rtp ? p->redirip.sin_addr : p->ourip));
            }
            transmit_reinvite_with_sdp(p);
        }
        else if (!opbx_test_flag(p, SIP_PENDINGBYE))
        {
            ...
            {
                char iabuf[INET_ADDRSTRLEN];
                opbx_log(OPBX_LOG_DEBUG, "Deferring reinvite on SIP '%s' - It's audio will be redirected to IP %s\n", p->callid, opbx_inet_ntoa(iabuf, sizeof(iabuf), rtp ? p->redirip.sin_addr : p->ourip));
            }
...
static struct opbx_udptl *sip_get_udptl_peer(struct opbx_channel *chan)
{
    ...
    opbx_mutex_lock(&p->lock);
    if (p->udptl && opbx_test_flag(p, SIP_CAN_REINVITE))
        udptl = p->udptl;
    opbx_mutex_unlock(&p->lock);
    return udptl;
}
...
static int sip_set_udptl_peer(struct opbx_channel *chan, struct opbx_udptl *udptl)
{
    ...
    if (!opbx_test_flag(p, SIP_GOTREFER))
    {
        if (!p->pendinginvite)
        {
            ...
            {
                char iabuf[INET_ADDRSTRLEN];
                opbx_log(OPBX_LOG_DEBUG, "Sending reinvite on SIP '%s' - It's UDPTL soon redirected to IP %s:%d\n", p->callid, opbx_inet_ntoa(iabuf, sizeof(iabuf), udptl ? p->udptlredirip.sin_addr : p->ourip), udptl ? ntohs(p->udptlredirip.sin_port) : 0);
            }
        ...
        else if (!opbx_test_flag(p, SIP_PENDINGBYE))
        {
            ...
            {
                char iabuf[INET_ADDRSTRLEN];
                opbx_log(OPBX_LOG_DEBUG, "Deferring reinvite on SIP '%s' - It's UDPTL will be redirected to IP %s:%d\n", p->callid, opbx_inet_ntoa(iabuf, sizeof(iabuf), udptl ? p->udptlredirip.sin_addr : p->ourip), udptl ? ntohs(p->udptlredirip.sin_port) : 0);
            }
            opbx_set_flag(p, SIP_NEEDREINVITE);    
        }
    }
...
#if 0
static struct opbx_tpkt *sip_get_tpkt_peer(struct opbx_channel *chan)
{
    struct sip_pvt *p;
    struct opbx_tpkt *tpkt = NULL;
    ...
    opbx_mutex_lock(&p->lock);
    if (p->tpkt && opbx_test_flag(p, SIP_CAN_REINVITE))
        tpkt = p->tpkt;
    opbx_mutex_unlock(&p->lock);
    return tpkt;
}

static int sip_set_tpkt_peer(struct opbx_channel *chan, struct opbx_tpkt *tpkt)
{
    struct sip_pvt *p;
    ...
    opbx_mutex_lock(&p->lock);
    if (tpkt)
        opbx_tpkt_get_peer(tpkt, &p->redirip);
    else
        memset(&p->redirip, 0, sizeof(p->redirip));
    if (!opbx_test_flag(p, SIP_GOTREFER))
    {
        if (!p->pendinginvite)
        {
            if (option_debug > 2)
            {
                ...
                opbx_log(OPBX_LOG_DEBUG, "Sending reinvite on SIP '%s' - It's TPKT soon redirected to IP %s\n", p->callid, opbx_inet_ntoa(iabuf, sizeof(iabuf), tpkt ? p->redirip.sin_addr : p->ourip));
            }
            transmit_reinvite_with_t38_sdp(p);
        }
        else if (!opbx_test_flag(p, SIP_PENDINGBYE))
        {
            if (option_debug > 2)
            {
                ...
                opbx_log(OPBX_LOG_DEBUG, "Deferring reinvite on SIP '%s' - It's TPKT will be redirected to IP %s\n", p->callid, opbx_inet_ntoa(iabuf, sizeof(iabuf), tpkt ? p->redirip.sin_addr : p->ourip));
            }
            opbx_set_flag(p, SIP_NEEDREINVITE);    
        }
    }
...
static int sip_handle_t38_reinvite(struct opbx_channel *chan, struct sip_pvt *pvt, int reinvite)
{
    struct sip_pvt *p;
    int flag = 0;
    ...
    if (reinvite)
    {
        ...
        if (opbx_test_flag(p, SIP_CAN_REINVITE) && opbx_test_flag(pvt, SIP_CAN_REINVITE))
        {
            opbx_udptl_get_peer(pvt->udptl, &p->udptlredirip);
            flag =1;
        }
        else
        {
        ...
        if (!opbx_test_flag(p, SIP_GOTREFER))
        {
            if (!p->pendinginvite)
            {
                if (option_debug > 2)
                {
                    char iabuf[INET_ADDRSTRLEN];
                    if (flag)
                        opbx_log(OPBX_LOG_DEBUG, "Sending reinvite on SIP '%s' - It's UDPTL soon redirected to IP %s:%d\n", p->callid, opbx_inet_ntoa(iabuf, sizeof(iabuf), p->udptlredirip.sin_addr), ntohs(p->udptlredirip.sin_port));
                    else
                        opbx_log(OPBX_LOG_DEBUG, "Sending reinvite on SIP '%s' - It's UDPTL soon redirected to us (IP %s)\n", p->callid, opbx_inet_ntoa(iabuf, sizeof(iabuf), p->ourip));
                }
                transmit_reinvite_with_t38_sdp(p);
            }
            else if (!opbx_test_flag(p, SIP_PENDINGBYE))
            {
                if (option_debug > 2)
                {
                    ...
                    if (flag)
                        ...
                        opbx_log(OPBX_LOG_DEBUG, "Deferring reinvite on SIP '%s' - It's UDPTL will be redirected to us (IP %s)\n", p->callid, opbx_inet_ntoa(iabuf, sizeof(iabuf), p->ourip));
                }
                opbx_set_flag(p, SIP_NEEDREINVITE);    
            }
        }
    ...
    else
    {
        ...
        if (option_debug > 2)
        {
            char iabuf[INET_ADDRSTRLEN];
            if (flag)
                opbx_log(OPBX_LOG_DEBUG, "Responding 200 OK on SIP '%s' - It's UDPTL soon redirected to IP %s:%d\n", p->callid, opbx_inet_ntoa(iabuf, sizeof(iabuf), p->udptlredirip.sin_addr), ntohs(p->udptlredirip.sin_port));
            else
                opbx_log(OPBX_LOG_DEBUG, "Responding 200 OK on SIP '%s' - It's UDPTL soon redirected to us (IP %s)\n", p->callid, opbx_inet_ntoa(iabuf, sizeof(iabuf), p->ourip));
        }
...
*/
       ...
       )
    {
        if (!opbx_test_flag(p, SIP_GOTREFER))
        {
            ...
            {
                if (option_debug > 2)
                    opbx_log(OPBX_LOG_DEBUG, "Forcing reinvite on SIP (%s) for T.38 negotiation.\n",chan->name);
        ...
        {
            if (option_debug > 2)
                opbx_log(OPBX_LOG_DEBUG, "Deferring forced reinvite on SIP (%s) - it will be re-negotiated for T.38\n",chan->name);
            opbx_set_flag(p, SIP_NEEDREINVITE);
        }
    }
    else {
...
static int sip_dtmfmode(struct opbx_channel *chan, int argc, char **argv, char *buf, size_t len)
{
    ...
    if (chan->type != channeltype)
    {
        opbx_log(OPBX_LOG_WARNING, "Call this application only on SIP incoming calls\n");
        opbx_mutex_unlock(&chan->lock);
        return 0;
    }
    p = chan->tech_pvt;
    ...
    { 
        ...
        opbx_log(OPBX_LOG_WARNING, "I don't know about this dtmf mode: %s\n", argv[0]);
    if (opbx_test_flag(p, SIP_DTMF) == SIP_DTMF_INBAND)
    {
        if (!p->vad)
        {
            p->vad = opbx_dsp_new();
            opbx_dsp_set_features(p->vad, DSP_FEATURE_DTMF_DETECT);
        }
    }
    else
    {
...
static int sip_addheader(struct opbx_channel *chan, int argc, char **argv, char *buf, size_t len)
{
    ...
    if (ok)
    {
        ...
        if (sipdebug)
            opbx_log(OPBX_LOG_DEBUG,"SIP Header added \"%s\" as %s\n", argv[0], varbuf);
    }
    else
    {
...
static int sip_sipredirect(struct sip_pvt *p, const char *dest)
{
    ...
    if (!extension)
    {
        opbx_log(OPBX_LOG_ERROR, "Missing mandatory argument: extension\n");
        return -1;
    }
	...
	char *scan;
      ...
      if (!opbx_db_get("SIP/Registry", extension, data, sizeof(data))) {
        ...
        if (!strlen(tmp))
        {
            opbx_log(OPBX_LOG_ERROR, "Cannot retrieve the 'To' header from the original SIP request!\n");
            return -1;
        }
        if ((localtmp = strstr(tmp, "sip:")) && (localtmp = strchr(localtmp, '@')))
        {
            ...
            if (!strlen(lhost))
            {
                opbx_log(OPBX_LOG_ERROR, "Can't find the host address\n");
                return -1;
            }
...
static int sip_get_codec(struct opbx_channel *chan)
{
...
static struct opbx_rtp_protocol sip_rtp =
{
    type: channeltype,
...
static struct opbx_tpkt_protocol sip_tpkt =
{
...
static void sip_send_all_registers(void)
{
    int ms;
    int regspacing;
    int shift = 1 + (int) (100.0 * (thread_safe_opbx_random() / (RAND_MAX + 1.0)));
    ...
    ASTOBJ_CONTAINER_TRAVERSE(&regl, 1, do {
        ...
        if (iterator->expire > -1)
            opbx_sched_del(sched, iterator->expire);
        ms += regspacing;
        iterator->expire = opbx_sched_add(sched, ms, sip_reregister, iterator);
        ASTOBJ_UNLOCK(iterator);
    } while (0)
    );
}
...
static int sip_do_reload(void)
{
	...
	if (iterator->call) {
		if (option_debug > 2)
			opbx_log(OPBX_LOG_DEBUG, "Destroying active SIP dialog for registry %s@%s\n", iterator->username, iterator->hostname);
...
static int load_module(void)
{
    ...
    {
        opbx_log(OPBX_LOG_ERROR, "Unable to register channel type %s\n", channeltype);
        return -1;
    }
    ...
    opbx_manager_register2("SIPshowpeer", EVENT_FLAG_SYSTEM, manager_sip_show_peer,
...
static int unload_module(void)
{
    ...
    opbx_manager_unregister("SIPshowpeer");
