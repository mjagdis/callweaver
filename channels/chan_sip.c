...
static inline int sip_debug_test_pvt(struct sip_pvt *p) 
{
	if (sipdebug == 0)
		return 0;
	return sip_debug_test_addr(((opbx_test_flag(p, SIP_NAT) & SIP_NAT_ROUTE) ? &p->recv : &p->sa));
}
...
static int opbx_sip_ouraddrfor(struct in_addr *them, struct in_addr *us, struct sip_pvt *p)
{
	...
	if ( 
	    (localaddr && opbx_apply_ha(localaddr, &theirs) )
...
static int retrans_pkt(void *data)
{
	...
	if (pkt->retrans < MAX_RETRANS) {
		...
		if (pkt->owner && sip_debug_test_pvt(pkt->owner)) {
			if (opbx_test_flag(pkt->owner, SIP_NAT) & SIP_NAT_ROUTE)
...
static int __sip_autodestruct(void *data)
{
	...
	if (p->subscribed) {
		...
		transmit_state_notify(p, OPBX_EXTENSION_DEACTIVATED, 1, 1);	/* Send first notification */
		p->subscribed = NONE;
...
static void sip_rebuild_payload(struct sip_pvt *p, struct sip_request *req,int have_stun) {
	...
	while (tmpsdl!=NULL) {
	    ...
	    if (tmpsdl->type==SIP_DL_HEAD_CONTENTLENGHT) { // Must recalculate content length
		int sdplen=0;
...
static int if_callid_exists(char *callid) {
    ...
    cur = iflist;
    while(cur) {
	if (!strcmp(cur->callid, callid)) return 1;
	cur=cur->next;
    }
    return 0;
}
...
static int send_response(struct sip_pvt *p, struct sip_request *req, int reliable, int seqno)
{
	...
	if (stun_active && p->stun_needed==1) {
	    ...
	    if (stundebug) opbx_log(LOG_DEBUG,"This transmit response has stun at %d\n",p->stun_needed);
...
static int create_addr(struct sip_pvt *dialog, char *opeer)
{
	...
	if (!p) {
		...
		hp = opbx_gethostbyname(hostn, &ahp);
		if (hp) {
...
//	if (p->stun_needed>0) stun_remove_request(&p->stun_transid);
...
static int sip_hangup(struct opbx_channel *ast)
{
	...
	opbx_set_flag(&locflags, SIP_NEEDDESTROY);	
	...
	opbx_copy_flags(p, (&locflags), SIP_NEEDDESTROY);	
	opbx_mutex_unlock(&p->lock);
	return 0;
}
...
static int sip_answer(struct opbx_channel *ast)
{
	...
	if (ast->_state != OPBX_STATE_UP) {
		...
		if (codec) {
			...
			} else opbx_log(LOG_NOTICE, "Ignoring ${SIP_CODEC} variable because of unrecognized/not configured codec (check allow/disallow in sip.conf): %s\n",codec);
		}
...
static int process_sdp(struct sip_pvt *p, struct sip_request *req)
{
	...
	if (strcasecmp(get_header(req, "Content-Type"), "application/sdp")) {
		opbx_log(LOG_NOTICE, "Content is '%s', not 'application/sdp'\n", get_header(req, "Content-Type"));
		return -1;
	}
...
static int add_header(struct sip_request *req, const char *var, const char *value, short int type)
{
	...
	line=malloc( sizeof(struct sip_data_line) );
	if (line ) {
	    ...
	    if (req->head_lines==NULL) req->head_lines=line;
	    else {
...
static int add_line(struct sip_request *req, const char *dline, short int type)
{
	...
	if (line) {
	    ...
	    if (req->sdp_lines==NULL) req->sdp_lines=line;
	    else {
		while (pointer->next!=NULL) pointer=pointer->next;
		pointer->next=line;
		//req->lines++;
...
static int add_digit(struct sip_request *req, char digit)
{
	...
	snprintf(tmp, sizeof(tmp), "Signal=%c", digit);
...
static int add_sdp(struct sip_request *resp, struct sip_pvt *p)
{
	...
	for (x = 1; x <= ((videosupport && p->vrtp) ? OPBX_FORMAT_MAX_VIDEO : OPBX_FORMAT_MAX_AUDIO); x <<= 1) {
		...
		if (x <= OPBX_FORMAT_MAX_AUDIO)
			add_codec_to_sdp(p, resp, x, 8000,
					 debug);
		else
			add_codec_to_sdp(p, resp, x, 90000,
					 debug);
	}
...
static int parse_ok_contact(struct sip_pvt *pvt, struct sip_request *req)
{
	...
	if (!(opbx_test_flag(pvt, SIP_NAT) & SIP_NAT_ROUTE)) {
		/* XXX This could block for a long time XXX */
		/* We should only do this if it's a name, not an IP */
		hp = opbx_gethostbyname(n, &ahp);
		if (!hp)  {
...
static enum parse_register_result parse_register_contact(struct sip_pvt *pvt, struct sip_peer *p, struct sip_request *req)
{
	...
	if (opbx_strlen_zero(expires)) {	/* No expires header */
...
static int get_destination(struct sip_pvt *p, struct sip_request *oreq)
{
	...
	if ((a = strchr(uri, '@'))) {
		...
		*a = '\0';
		...
		if (colon)
			*colon = '\0';
		opbx_copy_string(p->domain, a, sizeof(p->domain));
	}
	/* Skip any options */
	if ((a = strchr(uri, ';'))) {
		*a = '\0';
...
#define FORMAT  "%-25.25s  %-15.15s %-3.3s %-3.3s %-3.3s %-8d %-10s\n"
	...
	ASTOBJ_CONTAINER_TRAVERSE(&peerl, 1, do {
		...
		if (!s)  {/* Normal CLI list */
			...
			opbx_test_flag(iterator, SIP_DYNAMIC) ? "yes" : "no",  /* Dynamic or not? */
...
static int _sip_show_peer(int type, int fd, struct mansession *s, struct message *m, int argc, char *argv[])
{
	...
	if (peer && type==0 ) { /* Normal listing */
		...
		opbx_cli(fd, "  Dynamic      : %s\n", (opbx_test_flag(peer, SIP_DYNAMIC)?"Yes":"No"));
		...
		opbx_cli(fd, "LastMsgsSent: %d\r\n", peer->lastmsgssent);
		opbx_cli(fd, "Call limit: %d\r\n", peer->call_limit);
		opbx_cli(fd, "Dynamic: %s\r\n", (opbx_test_flag(peer, SIP_DYNAMIC)?"Y":"N"));
	...
	char *header, int sipmethod,  char *digest, int digest_len)
{
	...
	while (c && *(c = opbx_skip_blanks(c))) {	/* lookup for keys */
		for (i = keys; i->key != NULL; i++) {
			char *src, *separator;
...
static char *function_sippeer(struct opbx_channel *chan, char *cmd, char *data, char *buf, size_t len)
{
	...
	if (!strcasecmp(colname, "ip")) {
		...
		opbx_copy_string(buf, (opbx_test_flag(peer, SIP_DYNAMIC) ? "yes" : "no"), len);
...
static void handle_response_invite(struct sip_pvt *p, int resp, char *rest, struct sip_request *req, int ignore, int seqno)
{
	...
	if (opbx_test_flag(p, SIP_ALREADYGONE)) { /* This call is already gone */
		...
		case OPBX_STATE_UP:
			...
			if (p->t38state == 4) {
				...
				if ((bridgepeer=opbx_bridged_channel(p->owner))) {
					...
					if (!strcasecmp(bridgepeer->type,"SIP")) { /* If we are bridged to SIP channel */
						bridgepvt = (struct sip_pvt*)bridgepeer->tech_pvt;
...
static int handle_request_bye(struct sip_pvt *p, struct sip_request *req, int debug, int ignore)
{
	...
	if (p->pendinginvite && !opbx_test_flag(p, SIP_OUTGOING) && !ignore)
		transmit_response_reliable(p, "487 Request Terminated", &p->initreq, 1);
		...
		if (gotdest) {
			...
			if (!strcmp(event, "presence") || !strcmp(event, "dialog")) { /* Presence, RFC 3842 */
				...
				*/
				...
				return 0;
			} else { /* At this point, OpenPBX does not understand the specified event */
				transmit_response(p, "489 Bad Event", req);
				if (option_debug > 1)
...
/*
				...
				&& !strncmp(sip->owner->appl,"TxFAX", sizeof("TxFAX") ) 
			       ) {
				    opbx_log(LOG_DEBUG, "We are on a TxFAX session sending to another RxFAX instance over RTP. Should reinvite with T38. %s\n",sip->callid);

				    if ( 
					!(opbx_bridged_channel(sip->owner)) &&
					!(opbx_test_flag(sip, SIP_GOTREFER)) &&
					!sip->pendinginvite
    				      ) {
					sip->t38state = 4;
					transmit_reinvite_with_t38_sdp(sip);
					opbx_log(LOG_DEBUG, "T38 state changed to %d on channel %s\n",sip->t38state,sip->owner->name);
				    }
...
static struct sip_peer *temp_peer(const char *name)
{
	...
	opbx_set_flag(peer, SIP_DYNAMIC);
	peer->prefs = prefs;
	reg_source_db(peer);
...
static struct sip_peer *build_peer(const char *name, struct opbx_variable *v, int realtime)
{
		...
		if (realtime && !strcasecmp(v->name, "regseconds")) {
			...
			if (!strcasecmp(v->value, "dynamic")) {
				...
				if (peer->expire > -1)
					opbx_sched_del(sched, peer->expire);
				peer->expire = -1;
				opbx_clear_flag(peer, SIP_DYNAMIC);	
	...
	if (!opbx_test_flag((&global_flags_page2), SIP_PAGE2_IGNOREREGEXPIRE) && opbx_test_flag(peer, SIP_DYNAMIC) && realtime) {
		time_t nowtime;
	...
	if (!found && opbx_test_flag(peer, SIP_DYNAMIC) && !opbx_test_flag(peer, SIP_REALTIME))
		reg_source_db(peer);
	ASTOBJ_UNMARK(peer);
	opbx_free_ha(oldha);
	return peer;
...
static int reload_config(void)
{
	...
	while(v) {
		...
		if (!strcasecmp(v->name, "context")) {
			...
			if (!(hp = opbx_gethostbyname(stunserver_host, &ahp))) 
...
int unload_module()
{
	...
	if (!opbx_mutex_lock(&iflock)) {
		...
		while (p) {
			...
			opbx_mutex_destroy(&pl->lock);
			if (pl->chanvars) {
				opbx_variables_destroy(pl->chanvars);
				pl->chanvars = NULL;
			}
			free(pl);
		}
