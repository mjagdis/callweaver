	...
	if (fr) {
		...
		if ((a = strchr(fr, '@'))) {
			*a = '\0';
			...
			opbx_copy_string(p->fromdomain, fr, sizeof(p->fromdomain));
	}
	if (pedanticsipchecking)
		opbx_uri_decode(c);
	if (sip_debug_test_pvt(p))
		opbx_verbose("Looking for %s in %s\n", c, p->context);
	if (opbx_exists_extension(NULL, p->context, c, 1, fr) ||
		!strcmp(c, opbx_pickup_ext())) {
		if (!oreq)
			opbx_copy_string(p->exten, c, sizeof(p->exten));
		return 0;
	}

	if (opbx_canmatch_extension(NULL, p->context, c, 1, fr) ||
	    !strncmp(c, opbx_pickup_ext(),strlen(c))) {
		return 1;
	}
...
static int handle_request_register(struct sip_pvt *p, struct sip_request *req, int debug, int ignore, struct sockaddr_in *sin, char *e)
{
	...
	if ((res = register_verify(p, sin, req, e, ignore)) < 0) 
		opbx_log(LOG_NOTICE, "Registration from '%s' failed for '%s' - %s\n", get_header(req, "To"), opbx_inet_ntoa(iabuf, sizeof(iabuf), sin->sin_addr), (res == -1) ? "Wrong password" : "Username/auth name mismatch");
	if (res < 1) {
...
// static struct opbx_cli_entry  cli_sip_reload =
