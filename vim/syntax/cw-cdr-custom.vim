" Vim syntax file
" Language:	CallWeaver cdr_custom.conf file
...
syn region  cwSection       matchgroup=cwSectionName start="^\s*\%(;\s*\)\?\[mappings\]\%(([^)]*)\)\?" end="^\s*\%(;\s*\)\?\[.\{-}\]\%(([^)]*)\)\?" keepend contained contains=cwPreProc,cwKeySubstValue,cwComment,cwError
...
let b:current_syntax = "cw-cdr-custom"
