...
static int handle_add_indication(struct cw_dynstr *ds_p, int argc, char *argv[])
{
    ...
    if (!tz) {
        ...
        cw_log(CW_LOG_NOTICE, "Country '%s' does not exist, creating it.\n",argv[2]);
        ...
        {
            cw_log(CW_LOG_WARNING, "Unable to register new country\n");
            free(tz);
            return -1;
        }
        created_country = 1;
    ...
    {
        cw_log(CW_LOG_WARNING, "Unable to register indication %s/%s\n",argv[2],argv[3]);
        if (created_country)
...
static int handle_remove_indication(struct cw_dynstr *ds_p, int argc, char *argv[])
{
    ...
    if (argc == 3)
    {
        ...
        {
            cw_log(CW_LOG_WARNING, "Unable to unregister indication country %s\n",argv[2]);
            return -1;
        }
        return 0;
    }
    ...
    tz = cw_get_indication_zone(argv[2]);
    if (!tz) {
        cw_log(CW_LOG_WARNING, "Unable to unregister indication %s/%s, country does not exists\n",argv[2],argv[3]);
        return -1;
    }
    if (cw_unregister_indication(tz,argv[3])) {
        cw_log(CW_LOG_WARNING, "Unable to unregister indication %s/%s\n",argv[2],argv[3]);
        return -1;
    }
    return 0;
}
...
static int handle_show_indications(struct cw_dynstr *ds_p, int argc, char *argv[])
{
    ...
    if (cw_mutex_lock(&tzlock)) {
        cw_log(CW_LOG_WARNING, "Unable to lock tone_zones list\n");
        return 0;
    }
    if (argc == 2) {
        ...
        while (v)
        {
            ...
            else if (!strcasecmp(v->name,"ringcadence") || !strcasecmp(v->name,"ringcadance"))
            {
                ...
                while (ring)
                {
                    ...
                    {
                        cw_log(CW_LOG_WARNING,"Invalid ringcadence given '%s' at line %d.\n",ring,v->lineno);
                        ring = strsep(&c,",");
                        continue;
                    }
            ...
            else if (!strcasecmp(v->name,"alias"))
            {
                ...
                while (country)
                {
                    ...
                    {
                        cw_log(CW_LOG_WARNING, "Unable to register indication alias at line %d.\n",v->lineno);
                        free(tones);
                    }
                    /* next item */
                    country = strsep(&c,",");
            ...
            else
            {
                ...
                for (ps = NULL, ts = tones->tones;  ts;  ps = ts, ts = ts->next)
                {
                    ...
                    {
                        /* already there */
                        cw_log(CW_LOG_NOTICE,"Duplicate entry '%s', skipped.\n",v->name);
                        goto out;
                    }
                }
...
out:            v = v->next;
        ...
        if (tones->description[0] || tones->alias[0] || tones->tones) {
            if (cw_register_indication_country(tones)) {
                cw_log(CW_LOG_WARNING, "Unable to register indication at line %d.\n",v->lineno);
                free(tones);
            }
        } else free(tones);
    ...
    if (!country || !*country || cw_set_indication_country(country))
        cw_log(CW_LOG_WARNING,"Unable to set the default country (for indication tones)\n");
 ...
 */
static struct cw_clicmd add_indication_cli = {
...
static struct cw_clicmd remove_indication_cli = {
...
static struct cw_clicmd show_indications_cli = {
...
static int unload_module(void)
{
    ...
    res |= cw_unregister_function(playtones_app);
    res |= cw_unregister_function(stopplaytones_app);
    return res;
}
...
static int load_module(void)
{
    ...
    playtones_app = cw_register_function("PlayTones", handle_playtones, "Play a tone list", NULL, playtones_desc);
    stopplaytones_app = cw_register_function("StopPlayTones", handle_stopplaytones, "Stop playing a tone list", NULL, "Stop playing a tone list");
