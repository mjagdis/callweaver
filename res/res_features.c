...
static void do_bridge_masquerade(struct opbx_channel *chan, struct opbx_channel *tmpchan)
{
	opbx_moh_stop(chan);
	...
	opbx_explicit_goto(tmpchan, chan->context, chan->exten, chan->priority + 1);
...
static int action_bridge(struct mansession *s, const struct message *m)
{
	...
	char buf[BUFSIZ];
	struct opbx_channel *chana = NULL, *chanb = NULL;
	struct opbx_channel *tmpchana = NULL, *tmpchanb = NULL;
	struct opbx_bridge_thread_obj *tobj = NULL;
	int res = 0;
	int playtone = 0;
	char sActionid[1025];

	/* Find out whether we're supposed to play a tone or not */
	playtone = opbx_true(pt);
	
	/* if ActionID specified, build a new one with \r\n included */
	if ( !opbx_strlen_zero(actionid) )
		snprintf(sActionid, sizeof(sActionid), "ActionID: %s\r\n", actionid);
	else
		strcpy(sActionid, "");

	/* make sure valid channels were specified */
	if ( !opbx_strlen_zero(channela) && !opbx_strlen_zero(channelb) ) {
		chana = opbx_get_channel_by_name_locked(channela);
		chanb = opbx_get_channel_by_name_locked(channelb);
		if ( chana )
			opbx_mutex_unlock(&chana->lock);
		if ( chanb )
			opbx_mutex_unlock(&chanb->lock);
	...
	if ( OPBX_STATE_UP != chana->_state )
		opbx_answer(chana);
	if ( OPBX_STATE_UP != chanb->_state )
		opbx_answer(chanb);

	/* create the placeholder channels and grab the other channels */
	tmpchana = opbx_channel_alloc(0, OPBX_STATE_DOWN, 0, 0, "Bridge/%s", chana->name);
	tmpchanb = opbx_channel_alloc(0, OPBX_STATE_DOWN, 0, 0, "Bridge/%s", chanb->name);

	if ( tmpchana )
		do_bridge_masquerade(chana, tmpchana);
	else {
		astman_send_error(s, m, "Unable to create temporary channels!");
		opbx_hangup(tmpchana);
		opbx_hangup(chana);
		return 1;
	}
	...
	if ( tmpchanb )
		...
		astman_send_error(s, m, "Unable to create temporary channels!");
		opbx_hangup(tmpchanb);
		opbx_hangup(chanb);
		return 1;
	}
	
	/* make the channels compatible, send error if we fail doing so */
	res = opbx_channel_make_compatible(tmpchana, tmpchanb);
	if ( res ) {
		opbx_log(LOG_WARNING, "Could not make channels %s and %s compatible for manager bridge\n", tmpchana->name, tmpchanb->name);
	...
	if ( tobj ) {
		...
		if ( playtone ) 
			if ( !opbx_strlen_zero(xfersound) && !opbx_streamfile(tmpchanb, xfersound, tmpchanb->language) )
				if ( opbx_waitstream(tmpchanb, "") < 0 )
					opbx_log(LOG_WARNING, "Failed to play a courtesy tone on chan %s\n", tmpchanb->name);
		opbx_bridge_call_thread_launch(tobj);
		astman_send_ack(s, m, "Launched bridge thread with success");
		return 0;
	} else {
		opbx_log(LOG_WARNING, "Unable to spawn a new bridge thread on %s and %s: %s\n", tmpchana->name, tmpchanb->name, strerror(errno));
