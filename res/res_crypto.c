...
OPENPBX_FILE_VERSION(__FILE__, "$Revision: 1.26 $")
	...
	if (key->rsa) {
		...
		if (!notice) {
			if (!option_initcrypto) 
				ast_log(LOG_NOTICE, "Add the '-i' flag to the openpbx command line if you want to automatically initialize passcodes at launch.\n");
			notice++;
		}
		/* Keep it anyway */
		key->delme = 0;
