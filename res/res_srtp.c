...
static int res_srtp_protect(struct cw_srtp *srtp, void **buf, size_t *len);
...
res_srtp_unprotect(struct cw_srtp *srtp, void *buf, int *len)
{
	...
	for (i = 0; i < 2; i++) {
		if ((res = srtp_unprotect(srtp->session, buf, len)) != err_status_no_ctx
		|| !srtp->cb || !srtp->cb->no_ctx
		|| srtp->cb->no_ctx(srtp->rtp, ntohl(((srtp_hdr_t *)buf)->ssrc), srtp->data) < 0)
			break;
	}
...
static int
res_srtp_protect(struct cw_srtp *srtp, void **buf, size_t *len)
{
	if ((*len + SRTP_MAX_TRAILER_LEN) <= sizeof(srtp->buf)) {
		...
		if ((res = srtp_protect(srtp->session, srtp->buf, &l)) == err_status_ok) {
			*buf = srtp->buf;
			*len = l;
			return 0;
		}
...
MODULE_INFO(load_module, NULL, unload_module, NULL, "Secure RTP (SRTP)")
