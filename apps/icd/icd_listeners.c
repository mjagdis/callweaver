...
icd_listeners *create_icd_listeners(void)
{
    ...
    if (listeners == NULL) {
        cw_log(CW_LOG_ERROR, "No memory available to create a new ICD Listeners collection\n");
        return NULL;
    }
...
icd_status icd_listeners__dump(icd_listeners * that, cw_dynstr_t *ds_p)
{
    CW_UNUSED(that);
    CW_UNUSED(ds_p);
        ...
        icd_event * event, void *extra), void *extra)
{
    ...
    if (newnode == NULL) {
        cw_log(CW_LOG_ERROR, "No memory available to create a new ICD Listener Node\n");
        return NULL;
    }
