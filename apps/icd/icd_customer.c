...
icd_customer *create_icd_customer(icd_config * data)
{
    ...
    if (customer == NULL) {
        cw_log(CW_LOG_ERROR, "No memory available to create a new ICD Customer\n");
        return NULL;
    }
...
icd_plugable_fn *icd_customer_get_plugable_fns(icd_caller * that)
{
    ...
    if (plugable_fns == NULL) {
        if (icd_verbose > 4)
            cw_log(CW_LOG_NOTICE, "Customer Caller %d [%s] has no plugable fn aborting ala crash\n",
...
icd_customer *icd_customer__generate_queued_call(char *id, char *queuename, char *dialstr, char *vars, char delim, icd_plugable_fn *(*plug)(icd_caller *))
{
    ...
    if (queue == NULL) {
        cw_log(CW_LOG_ERROR, "CUSTOMER FAILURE! Customer assigned to undefined Queue [%s]\n", queuename);
        return NULL;
    }
