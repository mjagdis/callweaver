...
void icd_plugable__create_standard_fns(icd_plugable_fn_list * that, icd_config * data)
{
    ...
    cw_log(CW_LOG_NOTICE, "QueueLIST[%s]\n", queuelist);
    while (queuelist != NULL) {
        ...
        if (currqueue != NULL && strlen(currqueue) > 0) {
            ...
            if (queue != NULL) {
                ...
                cw_log(CW_LOG_NOTICE, "CurrQueue-distname[%s]\n", dist_name);
                ...
                if (plugable_fns == NULL) {
                    ...
                    if (plugable_fns != NULL) {
                        ...
                        cw_log(CW_LOG_NOTICE, "Add Plugable funcs for Callers dist[%s]\n", dist_name);
                    } else {
                        cw_log(CW_LOG_NOTICE, "Create_icd_plugable_fns returned null [%s]\n", dist_name);
                    }
                } else {
                    cw_log(CW_LOG_NOTICE, "icd_plugable_fn_list__fetch_fns think it found [%s]\n", dist_name);
                }
            }
        }
    ...
    if (icd_plugable_fn_list_count(that) == 0) {
        ...
        if (plugable_fns != NULL) {
            icd_plugable_fn_list__add_fns(that, plugable_fns);
            cw_log(CW_LOG_NOTICE, "No Dists found Adding Plugable funcs for Callers dist[Default]\n");
        }
    }
...
icd_plugable_fn *create_icd_plugable_fns(icd_config * data, char *name)
{
    ...
    if (plugable_fns == NULL) {
        cw_log(CW_LOG_ERROR, "No memory available to create a new ICD plugable fns\n");
        return NULL;
    }
