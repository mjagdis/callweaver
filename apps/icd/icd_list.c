...
icd_list *create_icd_list(icd_config * data)
{
    ...
    if (list == NULL) {
        cw_log(CW_LOG_ERROR, "No memory available to create a new ICD List\n");
        return NULL;
    }
    list->allocated = 1;
...
icd_status destroy_icd_list(icd_list ** listp)
{
    ...
    if (vetoed == ICD_EVETO) {
        cw_log(CW_LOG_NOTICE, "Destruction of ICD List %s has been vetoed\n", icd_list__get_name(*listp));
        return ICD_EVETO;
    }
...
icd_status init_icd_list(icd_list * that, icd_config * data)
{
    ...
    if (that->cache == NULL) {
        cw_log(CW_LOG_ERROR, "No memory available to create an ICD List cache\n");
...
icd_status icd_list__clear(icd_list * that)
{
    ...
    if (vetoed == ICD_EVETO) {
        cw_log(CW_LOG_WARNING, "Clearing of ICD List %s has been vetoed\n", icd_list__get_name(that));
        return ICD_EVETO;
    }
    ...
    count = icd_list__count(that);
    if (count != 0) {
        cw_log(CW_LOG_WARNING, "Was unable to clear ICD List %s\n", icd_list__get_name(that));
        return ICD_EGENERAL;
    }
    ...
    cw_log(CW_LOG_WARNING, "Unable to get a lock on ICD List %s in order to clear it\n", icd_list__get_name(that));
    return ICD_ELOCK;
}
...
icd_status icd_list__push(icd_list * that, void *element)
{
    ...
    if (vetoed == ICD_EVETO) {
        cw_log(CW_LOG_NOTICE, "Adding Node to ICD List %s has been vetoed\n", icd_list__get_name(that));
        return ICD_EVETO;
    }
    ...
    if (icd_list__lock(that) == ICD_SUCCESS) {
        ...
        if (new_node == NULL) {
            cw_log(CW_LOG_WARNING, "No room in ICD List %s to push an element\n", icd_list__get_name(that));
            icd_list__unlock(that);
            return ICD_ERESOURCE;
        }
    ...
    cw_log(CW_LOG_WARNING, "Unable to get a lock on ICD List %s in order to push onto it\n",
        icd_list__get_name(that));
    return ICD_ELOCK;
}
...
void *icd_list__pop_locked(icd_list * that)
{
    ...
    if (vetoed == ICD_EVETO) {
        cw_log(CW_LOG_NOTICE, "Removing Node from ICD List %s has been vetoed\n", icd_list__get_name(that));
        return NULL;
    }
    ...
    cw_log(CW_LOG_WARNING, "Unable to get a lock on ICD List %s in order to pop off of it\n",
        icd_list__get_name(that));
    return NULL;
...
icd_status icd_list__lock(icd_list * that)
{
   ...
   if (icd_debug)
            cw_log(CW_LOG_DEBUG, "List [%s] try to lock\n", that->name);
    retval = cw_mutex_lock(&that->lock);
    if (retval == 0) {
       if (icd_debug)
            cw_log(CW_LOG_DEBUG, "List [%s] locked\n", that->name);
        return ICD_SUCCESS;
    }
    if (icd_debug)
            cw_log(CW_LOG_DEBUG, "List [%s] lock failed\n", that->name);
    return ICD_ELOCK;
}
...
icd_status icd_list__unlock(icd_list * that)
{
    ...
    if (icd_debug)
            cw_log(CW_LOG_DEBUG, "List [%s] try to unlock\n", that->name);
    retval = cw_mutex_unlock(&that->lock);
    if (retval == 0) {
        if (icd_debug)
            cw_log(CW_LOG_DEBUG, "List [%s] unlocked\n", that->name);
        return ICD_SUCCESS;
    }
    if (icd_debug)
            cw_log(CW_LOG_DEBUG, "List [%s] unlock failed\n", that->name);
    return ICD_ELOCK;
}
...
icd_list_iterator *icd_list__get_iterator(icd_list * that)
{
    ...
    if (iter == NULL) {
        cw_log(CW_LOG_ERROR, "No memory available to create an iterator on ICD list %s\n", icd_list__get_name(that));
        return NULL;
    }
...
icd_status icd_list__drop_node(icd_list * that, const void *key, int (*match_fn) (const void *key, void *payload))
{
    ...
    while (icd_list_iterator__has_more_nolock(iter)) {
        ...
        if (match_fn(key, payload)) {
            ...
            if (vetoed == ICD_EVETO) {
                cw_log(CW_LOG_NOTICE, "Removal of Node from ICD List %s has been vetoed\n", icd_list__get_name(that));
                icd_list__unlock(that);
                return ICD_EVETO;
            }
