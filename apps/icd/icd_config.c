...
icd_status icd_config__set_value(icd_config * that, char *key, char *setting)
{
    ...
    if (regnode == NULL) {
        if (that->registry->validate == 0) {
            ...
            cw_log(CW_LOG_WARNING, "Could not find key %s in registry %s for configuration %s\n", key,
    ...
    if (regnode->type == ICD_CONFIG_REGNODE_EXACT) {
        ...
        if (child_regnode == NULL) {
            cw_log(CW_LOG_WARNING, "Could not find child key %s in registry %s for configuration %s\n", child_key,
                correct_null_str(that->registry->name), correct_null_str(that->name));
        ...
        cw_log(CW_LOG_ERROR, "Config Registry %s error, invalid registry type for key %s\n",
...
icd_config_registry *create_icd_config_registry(char *name)
{
    ...
    if (registry == NULL) {
        cw_log(CW_LOG_ERROR, "No memory available to create a new ICD Config Registry\n");
        return NULL;
    }
...
icd_status icd_config_registry__register(icd_config_registry * that, char *key)
{
    ...
    if (node != NULL) {
        cw_log(CW_LOG_WARNING, "Configuration %s Registry key '%s' is duplicated\n", correct_null_str(that->name),
            key);
        return ICD_EGENERAL;
    }
    ICD_MALLOC(node, sizeof(icd_config_registry_node));
    if (node == NULL) {
        cw_log(CW_LOG_ERROR, "No memory available to create a new entry in ICD Config Registry %s\n",
            correct_null_str(that->name));
...
icd_status icd_config_registry__register_ptr(icd_config_registry * that, char *key, char *keysetting, void *value)
{
    ...
    if (child_node != NULL) {
        cw_log(CW_LOG_WARNING, "Configuration %s Registry key '%s' for '%s' is duplicated.\n",
    ...
    if (parent_node == NULL) {
        ...
        if (parent_node == NULL) {
            cw_log(CW_LOG_ERROR, "No memory available to create a parent entry in ICD Config Registry %s\n",
                correct_null_str(that->name));
            return ICD_ERESOURCE;;
        }
        parent_node->key = key;
