...
/***************************************************************************
    ...
    copyright            : (C) 2003 by Anthony Minessale II
    email                : anthmct at yahoo dot com
***************************************************************************/
...
/*
  Conference support for ICD working pretty well after 3 days work! 
...
static int open_pseudo_zap()
{
    ...
    ast_log(LOG_WARNING, "gonna open pseudo channel:\n");
...
icd_status icd_conference__clear(icd_caller * that)
{
    ...
    if (conf) {
        if (conf->owner && (conf->owner == that)) {
            ast_log(LOG_NOTICE, "Goodbye Conf %d\n", conf->ztc.confno);
            close(conf->fd);
            ICD_STD_FREE(conf);
            conf = NULL;
        }
...
icd_status icd_conference__associate(icd_caller * that, icd_conference * conf, int owner)
{
    ...
    if (owner) {
        if (conf->owner) {
            ast_log(LOG_WARNING, "Error setting conference owner, one already exists...\n");
        } else {
            conf->owner = that;
...
icd_status icd_conference__join(icd_caller * that)
{
    ...
    ast_log(LOG_NOTICE, "Joining conference....%d\n", conf->ztc.confno);
