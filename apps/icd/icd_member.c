...
struct icd_member {
    ...
    icd_plugable_fn *(*get_plugable_fn) (icd_caller * caller);
      icd_status(*dump_fn) (icd_member * member, int verbosity, cw_dynstr_t *ds_p, void *extra);
    void *dump_fn_extra;
...
icd_status icd_member__dump(icd_member * that, int verbosity, cw_dynstr_t *ds_p)
{
    assert(that != NULL);
...
icd_status icd__member__set_dump_func(icd_member * that, icd_status(*dump_fn) (icd_member * list, int verbosity,
        cw_dynstr_t *ds_p, void *extra), void *extra)
{
    assert(that != NULL);
...
icd_status icd_member__standard_dump(icd_member * that, int verbosity, cw_dynstr_t *ds_p, void *extra)
{
    /* TBD */
    CW_UNUSED(that);
    CW_UNUSED(verbosity);
