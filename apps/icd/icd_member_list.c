...
icd_member_list *create_icd_member_list(char *name, icd_config * data)
{
    ...
    if (list == NULL) {
        cw_log(CW_LOG_ERROR, "No memory available to create a new ICD Member List\n");
        return NULL;
    }
    list->allocated = 1;
...
icd_status destroy_icd_member_list(icd_member_list ** listp)
{
    ...
    if (vetoed == ICD_EVETO) {
        cw_log(CW_LOG_NOTICE, "Destruction of ICD Member List %s has been vetoed\n",
...
icd_status icd_member_list__pushback(icd_member_list * that, icd_member * new_member)
{
    ...
    if (vetoed == ICD_EVETO) {
        cw_log(CW_LOG_NOTICE, "Pushing Back to ICD Member List %s has been vetoed\n",
    ...
    if (icd_member_list__lock(that) == ICD_SUCCESS) {
        ...
        if (new_node == NULL) {
            cw_log(CW_LOG_WARNING, "No room in ICD Member List %s to push back an element\n",
                icd_member_list__get_name(that));
    ...
    cw_log(CW_LOG_WARNING, "Unable to get a lock on ICD Member List %s in order to push onto it\n",
        icd_member_list__get_name(that));
    return ICD_ELOCK;
}
...
icd_status icd_member_list__dump(icd_member_list * that, int verbosity, cw_dynstr_t *ds_p)
{
    assert(that != NULL);
...
icd_status icd_member_list__standard_dump(icd_list * list, int verbosity, cw_dynstr_t *ds_p, void *extra)
{
