...
static struct opbx_conference *conflist = NULL ;

// mutex for synchronizing access to conflist
OPBX_MUTEX_DEFINE_STATIC(conflist_lock);
...
void conference_exec( struct opbx_conference *conf ) 
{
	...
	opbx_log( OPBX_CONF_DEBUG, "[ $Revision: 1.7 $ ] entered conference_exec, name => %s\n", conf->name ) ;
	...
	// count to OPBX_CONF_FRAMES_PER_SECOND
	int tf_count = 0 ;
	long tf_diff = 0 ;
	...
	while ( 42 == 42 )
	{
		...
		time_sleep = OPBX_CONF_FRAME_INTERVAL - time_diff ;
		...
		else
		{
			...
			if ( 
				since_last_slept == 0
				&& time_diff > OPBX_CONF_CONFERENCE_SLEEP * 2 
			)
			{
				opbx_log( 
					OPBX_CONF_DEBUG, 
					"long scheduling delay, time_diff => %ld, OPBX_CONF_FRAME_INTERVAL => %d\n",
					time_diff, OPBX_CONF_FRAME_INTERVAL 
				) ;
			}
		...
		add_milliseconds( &base, OPBX_CONF_FRAME_INTERVAL ) ;
		
		//
		// check thread frequency
		//
	
		if ( ++tf_count >= OPBX_CONF_FRAMES_PER_SECOND )
		{
			...
			if ( 
				( tf_frequency <= ( float )( OPBX_CONF_FRAME_INTERVAL - 1 ) )
				|| ( tf_frequency >= ( float )( OPBX_CONF_FRAME_INTERVAL + 1 ) )
			)
			{
				opbx_log( 
		...
		TIMELOG(opbx_mutex_lock( &conf->lock ),1,"conf thread conf lock");
		...
		while ( member != NULL )
		{
			...
			TIMELOG(opbx_mutex_lock( &member->lock ),1,"conf thread member lock") ;

			// check for dead members
			if ( member->remove_flag == 1 ) 
			{
				opbx_log( LOG_NOTICE, "found member slated for removal, channel => %s\n", member->channel_name ) ;
...
// !!! TESTING !!!
			...
			{
				opbx_log( OPBX_CONF_DEBUG, "got incoming conf_frame with null opbx_frame\n" ) ;
...
opbx_mutex_unlock( &member->lock ) ;
		...
		if ( ( usecdiff( &curr, &notify ) / OPBX_CONF_NOTIFICATION_SLEEP ) >= 1 )
		{
			// send the notifications
			send_state_change_notifications( conf->memberlist ) ;
		
			// increment the notification timer base
			add_milliseconds( &notify, OPBX_CONF_NOTIFICATION_SLEEP ) ;
		}
...
struct opbx_conference* start_conference( struct opbx_conf_member* member ) 
{
	...
	if ( conf == NULL )
	{
		...
		if ( conf == NULL ) 
		{
			...
			opbx_mutex_unlock( &start_stop_conf_lock ) ; // release mutex
			return NULL ;
		}
	}
	else
...
struct opbx_conference* create_conf( char* name, struct opbx_conf_member* member )
{
	...
	// zero stats
	memset(	&conf->stats, 0x0, sizeof( opbx_conference_stats ) ) ;
	...
	while ( conf_current != NULL ) 
	{
		...
		if ( conf_current == conf ) 
		{
			...
			{				
				...
				{
					opbx_translator_free_path( conf_current->from_slinear_paths[ c ] ) ;
			...
			opbx_log( LOG_NOTICE, "conference accounting, fi => %ld, fo => %ld, fm => %ld, tt => %ld\n",
...
void add_member( struct opbx_conf_member *member, struct opbx_conference *conf ) 
{
	if ( conf == NULL ) 
	{
...
int remove_member( struct opbx_conf_member* member, struct opbx_conference* conf ) 
{
	// check for member
	if ( member == NULL )
	{
	...
	while ( member_list != NULL ) 
	{
		if ( member_list == member ) 
		{
			...
			opbx_log( OPBX_CONF_DEBUG, "removed member from conference, name => %s, remaining => %d\n", conf->name, conf->membercount ) ;
...
int count_member( struct opbx_conf_member* member, struct opbx_conference* conf, short add_member )
{
	if ( member == NULL || conf == NULL )
	{
		opbx_log( LOG_WARNING, "unable to count member\n" ) ;
		return -1 ;
	}
...
)
{
	...
	if ( conf == NULL )
	{
		opbx_log( LOG_WARNING, "unable to queue speaker frame with null conference\n" ) ;
		return -1 ;
	}
	
	if ( member == NULL )
	{
		opbx_log( LOG_WARNING, "unable to queue speaker frame with null member\n" ) ;
		return -1 ;
	}
	...
	for ( ; frame != NULL ; frame = frame->next ) 
	{
		...
		if ( frame->fr == NULL )
		{
			opbx_log( LOG_WARNING, "unable to queue speaker frame with null data\n" ) ;
			continue ;
		}
		...
		// short-cut pointer to the opbx_frame
		qf = frame->fr ;

		// acquire member lock
		TIMELOG(opbx_mutex_lock( &member->lock ),1,"queue_frame_for_speaker: memberlock") ;
		...
		else
		{
			...
			// ( calling obpx_frdup() to make sure the translator's copy sticks around )
			//
			qf = convert_frame_from_slinear( member->from_slinear, opbx_frdup( qf ) ) ;
			...
			{
				opbx_log( LOG_WARNING, "unable to translate outgoing speaker frame, channel => %s\n", member->channel_name ) ;
			}
		}
...
)
{
	...
	if ( conf == NULL )
	{
		opbx_log( LOG_WARNING, "unable to queue listener frame with null conference\n" ) ;
		return -1 ;
	}
	
	if ( member == NULL )
	{
		opbx_log( LOG_WARNING, "unable to queue listener frame with null member\n" ) ;
		return -1 ;
	}
	...
	for ( ; frame != NULL ; frame = frame->next ) 
	{
		...
		if ( frame->fr == NULL )
		{
			opbx_log( LOG_WARNING, "unknown error queueing frame for listener, frame->fr == NULL\n" ) ;
			continue ;
		}

		// acquire member lock
		TIMELOG(opbx_mutex_lock( &member->lock ),1,"queue_frame_for_listener") ;
...
//ast_log (AST_CONF_DEBUG, "storing converted frame into index=>%d, qf->frametype=>%d, qf->subclass=%d, qf->datalen=%d \n",member->write_format_index, qf->frametype, qf->subclass, qf->datalen);
		...
		{
			opbx_log( LOG_WARNING, "unable to translate outgoing listener frame, channel => %s\n", member->channel_name ) ;
		}
...
{	
	...
	if ( conf == NULL )
	{
		opbx_log( OPBX_CONF_DEBUG, "unable to queue silent frame for null conference\n" ) ;
		return -1 ;
	}
	
	if ( member == NULL )
	{
		opbx_log( OPBX_CONF_DEBUG, "unable to queue silent frame for null member\n" ) ;
		return -1 ;
	}
	...
	static struct opbx_frame* qf = NULL ;

	if ( silent_frame == NULL )
	{
		if ( ( silent_frame = get_silent_frame() ) == NULL )
		{
			opbx_log( LOG_WARNING, "unable to initialize static silent frame\n" ) ;
			return -1 ;
		}
	}	

	// acquire member lock
	TIMELOG(opbx_mutex_lock( &member->lock ),1,"queue_silent_frame") ;
	...
	if ( qf == NULL )
	{
		...
		struct opbx_trans_pvt* trans = opbx_translator_build_path( member->write_format, OPBX_FORMAT_SLINEAR ) ;
		...
		if ( trans != NULL )
		{
			...
			{
				// translate the frame
				qf = opbx_translate( trans, silent_frame->fr, 0 ) ;
	...
	{
		opbx_log( LOG_ERROR, "unable to translate outgoing silent frame, channel => %s\n", member->channel_name ) ;
	}
...
int get_conference_stats( opbx_conference_stats* stats, int requested )
{	
	// no conferences exist
	if ( conflist == NULL ) 
	{
		opbx_log( OPBX_CONF_DEBUG, "conflist has not yet been initialize\n" ) ;
		return 0 ;
	}
...
int get_conference_stats_by_name( opbx_conference_stats* stats, const char* name )
{	
	// no conferences exist
	if ( conflist == NULL ) 
	{
...
struct opbx_conf_member *find_member ( char *chan, int lock) 
{
	struct opbx_conf_member *found = NULL;
	struct opbx_conf_member *member;	
	struct opbx_conference *conf;
