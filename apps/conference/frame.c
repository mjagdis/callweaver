 ...
 * app_conference
 *
 * A channel independent conference application for xAsterisk
 *
...
conf_frame* mix_single_speaker( conf_frame* frames_in )
{
	...
	if ( frames_in == NULL )
	{
		opbx_log( OPBX_CONF_DEBUG, "unable to mix single spoken frame with null frame\n" ) ;
		return NULL ;
	}

	if ( frames_in->fr == NULL )
	{
		opbx_log( OPBX_CONF_DEBUG, "unable to mix single spoken frame with null data\n" ) ;
		return NULL ;
	}

	if ( frames_in->member == NULL )
	{
		opbx_log( OPBX_CONF_DEBUG, "unable to mix single spoken frame with null member\n" ) ;
		return NULL ;
	}
	...
	frames_in->converted[ frames_in->member->read_format_index ] = opbx_frdup( frames_in->fr ) ;
...
/*
	...
	{
		struct opbx_conf_member* m = NULL ;
...
)
{
	...
	{
		opbx_log( OPBX_CONF_DEBUG, "passed spoken frame list was NULL\n" ) ;
		return NULL ;
	}
	...
	if ( speakers < 2 )
	{
		opbx_log( OPBX_CONF_DEBUG, "mix_multiple_speakers() called with less than two speakers\n" ) ;
		return NULL ;
	}
#endif // APP_CONFERENCE_DEBUG
	...
	while ( cf_spoken != NULL )
	{
		...
		{
			opbx_log( LOG_WARNING, "unable to determine frame member\n" ) ;
		}
		else
		{
			...
			{
				opbx_log( LOG_WARNING, "unable to convert frame to slinear\n" ) ;
			}
			else
			{
	...
	for ( cf_send = cf_sendFrames ; cf_send != NULL ; cf_send = cf_send->next )
	{
		...
		char* cp_listenerBuffer = malloc( OPBX_CONF_BUFFER_SIZE ) ;
		memset( cp_listenerBuffer, 0x0, OPBX_CONF_BUFFER_SIZE ) ;
		
		// point past the friendly offset right to the data
		cp_listenerData = cp_listenerBuffer + OPBX_FRIENDLY_OFFSET ;
		...
		for ( ; cf_spoken != NULL ; cf_spoken = cf_spoken->next )
		{
			...
			{
				opbx_log( LOG_WARNING, "unable to mix conf_frame with null opbx_frame\n" ) ;
			}
			else
			{			
...
struct opbx_frame* convert_frame_to_slinear( struct opbx_trans_pvt* trans, struct opbx_frame* fr )
{
	// check for null frame
	if ( fr == NULL )
	{
		opbx_log( LOG_ERROR, "unable to translate null frame to slinear\n" ) ;
		return NULL ;
	}
	...
	if ( fr->subclass == OPBX_FORMAT_SLINEAR ) 
		return fr ;

	// check for null translator ( after we've checked that we need to translate )
	if ( trans == NULL )
	{
		opbx_log( LOG_ERROR, "unable to translate frame with null translation path\n" ) ;
		return fr ;
	}
...
struct opbx_frame* convert_frame_from_slinear( struct opbx_trans_pvt* trans, struct opbx_frame* fr )
{
	// check for null translator ( after we've checked that we need to translate )
	if ( trans == NULL )
	{
	...
	if ( fr == NULL )
	{
		opbx_log( LOG_ERROR, "unable to translate null slinear frame\n" ) ;
		return NULL ;
	}
	...
	if ( fr->subclass != OPBX_FORMAT_SLINEAR )
	{
		opbx_log( LOG_ERROR, "unable to translate non-slinear frame\n" ) ;
		return NULL ;
	}
...
struct opbx_frame* convert_frame( struct opbx_trans_pvt* trans, struct opbx_frame* fr )
{
	if ( trans == NULL )
	{
		opbx_log( LOG_WARNING, "unable to convert frame with null translator\n" ) ;
		return NULL ;
	}

	if ( fr == NULL )
	{
		opbx_log( LOG_WARNING, "unable to convert null frame\n" ) ;
		return NULL ;
	}

	// convert the frame
	struct opbx_frame* translated_frame = opbx_translate( trans, fr, 1 ) ;

	// check for errors
	if ( translated_frame == NULL ) 
	{
		opbx_log( LOG_ERROR, "unable to translate frame\n" ) ;
		return NULL ;
	}
...
conf_frame* delete_conf_frame( conf_frame* cf )
{
	...
	if ( cf == NULL )
	{
		opbx_log( OPBX_CONF_DEBUG, "unable to delete null conf frame\n" ) ;
		return NULL ;
	}
...
conf_frame* create_conf_frame( struct opbx_conf_member* member, conf_frame* next, const struct opbx_frame* fr )
{
	...
	if ( cf == NULL )
	{
		opbx_log( LOG_ERROR, "unable to allocate memory for conf frame\n" ) ;
		return NULL ;
	}
...
//	}
	...
	memset( (struct opbx_frame*)( cf->converted ), 0x0, ( sizeof( struct opbx_frame* ) * AC_SUPPORTED_FORMATS ) ) ;
	...
	cf->fr = ( fr == NULL ) ? NULL : opbx_frdup( ( struct opbx_frame* )( fr ) ) ;
...
conf_frame* copy_conf_frame( conf_frame* src )
{
	...
	if ( src == NULL )
	{
		opbx_log( OPBX_CONF_DEBUG, "unable to copy null conf frame\n" ) ;
		return NULL ;
	}
	...
	if ( cfr == NULL )
	{
		opbx_log( OPBX_CONF_DEBUG, "unable to create new conf frame for copy\n" ) ;
		return NULL ;
	}
...
conf_frame* get_silent_frame( void ) 
{
	...
	if ( static_silent_frame == NULL )
	{
		...
		if ( static_silent_frame == NULL )
		{
			opbx_log( LOG_WARNING, "unable to create cached silent frame\n" ) ;
			return NULL ;
		}
...
struct opbx_frame* get_silent_slinear_frame( void )
{
	static struct opbx_frame* f = NULL ;
	
	// we'll let this leak until the application terminates
	if ( f == NULL )
	{
		char* data = malloc( OPBX_CONF_BUFFER_SIZE ) ;
		memset( data, 0x0, OPBX_CONF_BUFFER_SIZE ) ;
