...
static int process_incoming(struct opbx_conf_member *member, struct opbx_frame *f) 
{
	...
	{			
		...
		)
		{
			...
			else
			{
				...
				member->ignore_speex_count = OPBX_CONF_SKIP_SPEEX_PREPROCESS ;
			}
		}
#endif
  ...
  //					opbx_log( OPBX_CONF_DEBUG, "RECEIVED SILENT FRAME, channel => %s, frames_in => %ld, s => %ld, ms => %ld\n", 
  ...
  //					opbx_log( OPBX_CONF_DEBUG, "RECEIVED VOICE FRAME, channel => %s, frames_in => %ld, s => %ld, ms => %ld\n", 
  //						member->channel_name, member->frames_in, tv.tv_sec, tv.tv_usec ) ;
  ...
  //					struct opbx_frame* af = opbx_frdup( f ) ;
...
static struct opbx_frame *get_next_soundframe(struct opbx_conf_member *member, struct opbx_frame
    *exampleframe) {
    struct opbx_frame *f;

again:
    f=opbx_readframe(member->soundq->stream); 

    if(!f) { // we're done with this sound; remove it from the queue, and try again
	struct opbx_conf_soundq *toboot = member->soundq;

	opbx_closestream(toboot->stream);
...
// or requested sounds
static int process_outgoing(struct opbx_conf_member *member) 
{
	conf_frame* cf ; // frame read from the output queue
	struct opbx_frame *f;
	struct opbx_frame *realframe = NULL;
	...
	{	
		...
		if ( ( delivery_diff != OPBX_CONF_FRAME_INTERVAL     ) && 
		     ( delivery_diff != 2 * OPBX_CONF_FRAME_INTERVAL )  &&
		     ( delivery_diff != 3 * OPBX_CONF_FRAME_INTERVAL ) )
		{		
			opbx_log( OPBX_CONF_DEBUG, "unanticipated delivery time, delivery_diff => %d, delivery.tv_usec => %ld\n", 
		...
		{
			opbx_log( LOG_WARNING, "queued frame timestamped in the past, %ld.%ld <= %ld.%ld\n",
...
#ifdef DEBUG_USE_TIMELOG
		TIMELOG( opbx_write( member->chan, f ), 10, "member: opbx_write");
#else
		// send the voice frame
		if ( opbx_write( member->chan, f ) == 0 )
		{
			// struct timeval tv ;
			// gettimeofday( &tv, NULL ) ;
		...
		else
		{
			...
			opbx_log( LOG_ERROR, "unable to write voice frame to channel, channel => %s\n", member->channel_name ) ;
...
//	gettimeofday( &start, NULL ) ;
	...
	struct opbx_frame *f ; // frame received from ast_read()

	int left = 0 ;
	int res;
	
	opbx_log( OPBX_CONF_DEBUG, "[ $Revision: 1.9 $ ] begin processing member thread, channel => %s\n", chan->name ) ;
...
//		id, flags, priority ) ;
	...
	opbx_log( OPBX_CONF_DEBUG, "CHANNEL INFO, CHANNEL => %s, DNID => %s, CALLER_ID => %s, ANI => %s\n", 
		chan->name, chan->cid.cid_dnid, chan->cid.cid_num, chan->cid.cid_ani ) ;

	opbx_log( OPBX_CONF_DEBUG, "CHANNEL CODECS, CHANNEL => %s, NATIVE => %d, READ => %d, WRITE => %d\n", 
		chan->name, chan->nativeformats, member->read_format, member->write_format ) ;

	if ( opbx_set_read_format( chan, member->read_format ) < 0 )
	{
		opbx_log( LOG_ERROR, "unable to set read format to signed linear\n" ) ;
		delete_member( member ) ;
		return -1 ;
	} 

	// for right now, we'll send everything as slinear
	if ( opbx_set_write_format( chan, member->write_format ) < 0 ) // AST_FORMAT_SLINEAR, chan->nativeformats
	{
		opbx_log( LOG_ERROR, "unable to set write format to signed linear\n" ) ;
		delete_member( member ) ;
		return -1 ;
	}
	...
	if ( member->incoming_fh == NULL )
	{
		opbx_log( OPBX_CONF_DEBUG, "incoming_fh is null, incoming_fn => %s\n", incoming_fn ) ;
	}
	else
	{
		opbx_log( OPBX_CONF_DEBUG, "incoming_fh is not null, incoming_fn => %s\n", incoming_fn ) ;
	}
	...
	while ( 42 == 42 )
	{
		...
		left = opbx_waitfor( chan, OPBX_CONF_WAITFOR_LATENCY ) ;
		...
		else if ( left > 0 ) 
		{
			...
			if ( f == NULL ) 
			{
				opbx_log( LOG_NOTICE, "unable to read from channel, channel => %s\n", chan->name ) ;
				break ;
			}
	...
	opbx_log( OPBX_CONF_DEBUG, "end member event loop, time_entered => %ld\n", member->time_entered.tv_sec ) ;
...
struct opbx_conf_member* create_member( struct opbx_channel *chan, const char* data ) 
{
	...
	// ( not currently used )
	// member->samplesperframe = OPBX_CONF_BLOCK_SAMPLES ;
	...
	)
	{
		...
		member->dsp = speex_preprocess_state_init( OPBX_CONF_BLOCK_SAMPLES, OPBX_CONF_SAMPLE_RATE ) ;
		
		if ( member->dsp == NULL ) 
		{
			opbx_log( LOG_WARNING, "unable to initialize member dsp, channel => %s\n", chan->name ) ;
		}
		else
		{
			opbx_log( LOG_NOTICE, "member dsp initialized, channel => %s, v => %d, d => %d, a => %d\n", 
			...
			opbx_log( OPBX_CONF_DEBUG, "speech_prob_start => %f, speech_prob_continue => %f\n", 
	...
	member->read_format = ( member->dsp == NULL ) ? chan->nativeformats : OPBX_FORMAT_SLINEAR ;
	member->write_format = chan->nativeformats ;
	
	// translation paths ( opbx_translator_build_path() returns null if formats match )
	member->to_slinear = opbx_translator_build_path( OPBX_FORMAT_SLINEAR, member->read_format ) ;
	member->from_slinear = opbx_translator_build_path( member->write_format, OPBX_FORMAT_SLINEAR ) ;
	...
	opbx_log( OPBX_CONF_DEBUG, "created member, type => %c, priority => %d, readformat => %d\n", 	
...
struct opbx_conf_member* delete_member( struct opbx_conf_member* member ) 
{
	...
	// !!! DEBUGING !!!	
	opbx_log( OPBX_CONF_DEBUG, "deleting member input frames, name => %s\n", 
		member->channel_name ) ;
	...
	// !!! DEBUGING !!!	
	opbx_log( OPBX_CONF_DEBUG, "deleting member output frames, name => %s\n", 
		member->channel_name ) ;
	...
	if (member->outPacker != NULL)
		opbx_packer_free(member->outPacker);
	...
	{
		// !!! DEBUGING !!!	
		opbx_log( OPBX_CONF_DEBUG, "destroying member preprocessor, name => %s\n", 
			member->channel_name ) ;
	...
	// !!! DEBUGING !!!	
	opbx_log( OPBX_CONF_DEBUG, "freeing member translator paths, name => %s\n", 
		member->channel_name ) ;

	// free the mixing translators
	opbx_translator_free_path( member->to_slinear ) ;
	opbx_translator_free_path( member->from_slinear ) ;
	...
	// !!! DEBUGING !!!	
	opbx_log( OPBX_CONF_DEBUG, "freeing member channel name, name => %s\n", 
		member->channel_name ) ;
...
conf_frame* get_incoming_frame( struct opbx_conf_member *member )
{
	...
	if ( member == NULL )
	{
		opbx_log( LOG_WARNING, "unable to get frame from null member\n" ) ;
		return NULL ;
	}
...
#ifdef OPBX_CONF_CACHE_LAST_FRAME
	if ( member->inFramesCount == 0 )
	{
		...
		if ( member->inFramesRepeatLast >= OPBX_CONF_CACHE_LAST_FRAME )
		{
			// already used this frame OPBX_CONF_CACHE_LAST_FRAME times
		...
		{			 
			opbx_log( OPBX_CONF_DEBUG, "repeating cached frame, channel => %s, inFramesRepeatLast => %d\n",
	...
	{
		opbx_log( OPBX_CONF_DEBUG, "enabling cached frame, channel => %s, incoming => %d, outgoing => %d\n",
	...
	if ( member->inFramesCount == 0 )
		return NULL ;
#endif // OPBX_CONF_CACHE_LAST_FRAME
...
*/
	...
	{
		if ( member->inFramesCount > OPBX_CONF_QUEUE_DROP_THRESHOLD ) 
		{
			struct timeval curr ;
			gettimeofday( &curr, NULL ) ;
			...
			long time_limit = 1000 - ( ( member->inFramesCount - OPBX_CONF_QUEUE_DROP_THRESHOLD ) * 100 ) ;
...
*/
	...
	if ( member->inFramesCount >= OPBX_CONF_MAX_QUEUE ) 
	{
		// count sequential drops
		member->sequential_drops++ ;
	...
	if (member->inSmoother == NULL ){
		...
		if ( cfr == NULL ) 
		{
			opbx_log( LOG_ERROR, "unable to malloc conf_frame\n" ) ;
			return -1 ;
		}
		...
		opbx_smoother_feed( member->inSmoother, fr );
...
conf_frame* get_outgoing_frame( struct opbx_conf_member *member )
{
	...
	if ( member->outFramesCount > OPBX_CONF_MIN_QUEUE ) 
	{
...
/*
	...
	if ( member->outFramesCount > OPBX_CONF_QUEUE_DROP_THRESHOLD ) 
	{
		struct timeval curr ;
		gettimeofday( &curr, NULL ) ;
		...
		if ( diff >= OPBX_CONF_QUEUE_DROP_TIME_LIMIT )
		{
			opbx_log( 
				OPBX_CONF_DEBUG,
...
*/
	...
	// because it's easier ( and doesn't matter much anyway ).
	//
	if ( member->outFramesCount >= OPBX_CONF_MAX_QUEUE ) 
	{
		opbx_log( 
			OPBX_CONF_DEBUG,
	...
	if ( cfr == NULL ) 
	{
		opbx_log( LOG_ERROR, "unable to create new conf frame\n" ) ;
...
int queue_outgoing_frame( struct opbx_conf_member* member, const struct opbx_frame* fr, struct timeval delivery ) 
{
	...
	if ( ( member->outPacker == NULL ) && ( member->smooth_multiple > 1 ) && ( member->smooth_size_out > 0 ) ){
		...
		member->outPacker = opbx_packer_new( member->smooth_multiple * member->smooth_size_out);
	}
...
//ast_log (AST_CONF_DEBUG, "sending fr into outPacker, datalen=>%d, samples=>%d\n",fr->datalen, fr->samples);
		opbx_packer_feed( member->outPacker , fr );
		while( (sfr = opbx_packer_read( member->outPacker ) ) )
		{
...
#if 0
			opbx_log( OPBX_CONF_DEBUG, "member state changed, channel => %s, state => %d, incoming => %d, outgoing => %d\n",
...
struct opbx_packer {
	...
	char framedata[PACKER_SIZE + OPBX_FRIENDLY_OFFSET];
	int samples;
...
void opbx_packer_reset(struct opbx_packer *s, int framesize)
{
	memset(s, 0, sizeof(struct opbx_packer));
	s->framesize = framesize;
	s->packet_index=0;
	s->len=0;
}

struct opbx_packer *opbx_packer_new(int framesize)
{
	struct opbx_packer *s;
	if (framesize < 1)
		return NULL;
	s = malloc(sizeof(struct opbx_packer));
	if (s)
		opbx_packer_reset(s, framesize);
	return s;
}

int opbx_packer_get_flags(struct opbx_packer *s)
{
	return s->flags;
}

void opbx_packer_set_flags(struct opbx_packer *s, int flags)
{
	s->flags = flags;
}

int opbx_packer_feed(struct opbx_packer *s, const struct opbx_frame *f)
{
	if (f->frametype != OPBX_FRAME_VOICE) {
		opbx_log(LOG_WARNING, "Huh?  Can't pack a non-voice frame!\n");
		return -1;
	}
	if (!s->format) {
		s->format = f->subclass;
		...
		opbx_log(LOG_WARNING, "Packer was working on %d format frames, now trying to feed %d?\n", s->format, f->subclass);
		return -1;
	}
	if (s->len + f->datalen > PACKER_SIZE) {
		opbx_log(LOG_WARNING, "Out of packer space\n");
		return -1;
	}
	if (s->packet_index >= PACKER_QUEUE ){
		opbx_log(LOG_WARNING, "Out of packer queue space\n");
		return -1;
	}
...
struct opbx_frame *opbx_packer_read(struct opbx_packer *s)
{
	struct opbx_frame *opt;
	int len;
...
void opbx_packer_free(struct opbx_packer *s)
{
	free(s);
}
