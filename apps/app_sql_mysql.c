...
static void *find_identifier(int identifier,int identifier_type) {
	...
	if (CW_LIST_LOCK(headp)) {
		cw_log(CW_LOG_WARNING,"Unable to lock identifiers list\n");
	} else {
		...
		if (!found) {
			cw_log(CW_LOG_WARNING,"Identifier %d, identifier_type %d not found in identifier list\n",identifier,identifier_type);
		}
...
static int del_identifier(int identifier,int identifier_type) {
	...
	if (found==0) {
		cw_log(CW_LOG_WARNING,"Could not find identifier %d, identifier_type %d in list to delete\n",identifier,identifier_type);
		return(-1);
	} else {
		return(0);
	}
...
#if EXTRA_LOG
		cw_log(CW_LOG_WARNING,"MYSQL: setting var '%s' to value '%s'\n",varname,s);
#endif
	...
	if( connid_var && dbhost && dbuser && dbpass && dbname ) {
		...
		if (mysql) {
			if (mysql_real_connect(mysql,dbhost,dbuser,dbpass,dbname,0,NULL,0)) {
				...
				cw_log(CW_LOG_WARNING,"mysql_real_connect(mysql,%s,%s,dbpass,%s,...) failed\n",dbhost,dbuser,dbname);
			}
		}
		else {
			cw_log(CW_LOG_WARNING,"myslq_init returned NULL\n");
		}
 	}
	else {
		cw_log(CW_LOG_WARNING,"MYSQL(connect is missing some arguments\n");
	}
	...
	if (resultid_var && (connid>=0) && querystring) {
		if ((mysql=find_identifier(connid,CW_MYSQL_ID_CONNID))!=NULL) {
			...
			if ((mysqlres=mysql_use_result(mysql))!=NULL) {
				...
				cw_log(CW_LOG_WARNING,"aMYSQL_query: mysql_store_result() failed on query %s\n",querystring);
			}
		}
		else {
			cw_log(CW_LOG_WARNING,"aMYSQL_query: Invalid connection identifier %d passed in aMYSQL_query\n",connid);
		}
	}
	else {
		cw_log(CW_LOG_WARNING,"aMYSQL_query: missing some arguments\n");
	}
		...
		if ((mysqlres=find_identifier(resultid,CW_MYSQL_ID_RESID))!=NULL) {
			...
			if ((mysqlrow=mysql_fetch_row(mysqlres))!=NULL) {
				...
				for (j=0;j<numFields;j++) {
					s5=strsep(&data," ");
					if (s5==NULL) {
						cw_log(CW_LOG_WARNING,"cw_MYSQL_fetch: More fields (%d) than variables (%d)\n",numFields,j);
						break;
					}
					s6=mysqlrow[j];
...
#if EXTRA_LOG
				cw_log(CW_LOG_WARNING,"cw_MYSQL_fetch: numFields=%d\n",numFields);
#endif
				set_callweaver_int(chan,fetchid_var,1); // try more rows
			} else {
#if EXTRA_LOG
				cw_log(CW_LOG_WARNING,"cw_MYSQL_fetch : EOF\n");
#endif
			...
			cw_log(CW_LOG_WARNING,"aMYSQL_fetch: Invalid result identifier %d passed\n",resultid);
		}
	}
	else {
		cw_log(CW_LOG_WARNING,"aMYSQL_fetch: missing some arguments\n");
	}
...
static int aMYSQL_disconnect(struct cw_channel *chan, char *data) {
	...
	if ((mysql=find_identifier(id,CW_MYSQL_ID_CONNID))==NULL) {
		cw_log(CW_LOG_WARNING,"Invalid connection identifier %d passed in aMYSQL_disconnect\n",id);
	} else {
		mysql_close(mysql);
...
static int MYSQL_exec(struct cw_channel *chan, int argc, char **argv, char *result, size_t result_max)
{
	...
	if (strncasecmp("connect",argv[0],strlen("connect"))==0) {
		res=aMYSQL_connect(chan,cw_strdupa(argv[0]));
	} else 	if (strncasecmp("query",argv[0],strlen("query"))==0) {
		res=aMYSQL_query(chan,cw_strdupa(argv[0]));
	} else 	if (strncasecmp("fetch",argv[0],strlen("fetch"))==0) {
		res=aMYSQL_fetch(chan,cw_strdupa(argv[0]));
	} else 	if (strncasecmp("clear",argv[0],strlen("clear"))==0) {
		res=aMYSQL_clear(chan,cw_strdupa(argv[0]));
	} else 	if (strncasecmp("disconnect",argv[0],strlen("disconnect"))==0) {
		res=aMYSQL_disconnect(chan,cw_strdupa(argv[0]));
	} else {
		cw_log(CW_LOG_WARNING, "Unknown argument to MYSQL application : %s\n", argv[0]);
		res=-1;	
	}
...
static int load_module(void)
{
	...
	app = cw_register_function(name, MYSQL_exec, synopsis, syntax, descrip);
	return 0;
}
