...
int adsi_tl_add_tm(adsi_tx_state_t *s, char *msg)
{
    ...
    tx_len += 2;
    tx_len = adsi_tl_add_field(&tx_adsi, tx_msg, tx_len, DLL_PARM_PROVIDER_ID, "AST", 3);
        ...
        if (l > 0)
        {
            ...
            else
            {
                ...
                case DLL_SMS_P2_INFO_MO:
                    ...
                    adsi_put_message(&tx_adsi, tx_msg, tx_len);
                    /* Skip the TL length */
                    l += 2;
                    break;
...
static int sms_protocol2_exec(struct opbx_channel *chan, void *data)
{
    ...
    if (chan == NULL)
    {
        opbx_log(LOG_WARNING, "Channel is NULL. Giving up.\n");
        return -1;
    }
    vdata = data;
    ...
    if (!vdata)
    {
        ...
        opbx_log(LOG_WARNING, "SMS requires an argument (message)\n");
        return -1;
    }
    ...
    for (i = 0;  vdata[i]  &&  (vdata[i] != ':')  &&  (vdata[i] != ',');  i++)
    {
        ...
        if (i == strlen(vdata))
        {
            opbx_log(LOG_WARNING, "No extension found\n");
            return -1;
        }
        msg[i] = vdata[i];
    }
    ...
    if (!res)
    {
        ...
        while (opbx_waitfor(chan, -1) > -1)
        {
            ...
            if (inf->frametype == OPBX_FRAME_VOICE)
            {
                ...
                len = adsi_tx(&tx_adsi, (int16_t *) &buf[OPBX_FRIENDLY_OFFSET], inf->samples);
                if (len)
                {
